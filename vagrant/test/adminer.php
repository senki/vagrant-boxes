...
* @version 4.2.3
*/error_reporting(6135);$Mc=!preg_match('~^(unsafe_raw)?$~',ini_get("filter.default"));if($Mc||ini_get("filter.default_flags")){foreach(array('_GET','_POST','_COOKIE','_SERVER')as$X){$Jh=filter_input_array(constant("INPUT$X"),FILTER_UNSAFE_RAW);if($Jh)$$X=$Jh;}}if(function_exists("mb_internal_encoding"))mb_internal_encoding("8bit");if(isset($_GET["file"])){if($_SERVER["HTTP_IF_MODIFIED_SINCE"]){header("HTTP/1.1 304 Not Modified");exit;}header("Expires: ".gmdate("D, d M Y H:i:s",time()+365*24*60*60)." GMT");header("Last-Modified: ".gmdate("D, d M Y H:i:s")." GMT");if($_GET["file"]=="favicon.ico"){header("Content-Type: image/x-icon");echo
...
idf_unescape($t){$Td=substr($t,-1);return
str_replace($Td.$Td,$Td,substr($t,1,-1));}function
escape_string($X){return
substr(q($X),1,-1);}function
number($X){return
preg_replace('~[^0-9]+~','',$X);}function
remove_slashes($Lf,$Mc=false){if(get_magic_quotes_gpc()){while(list($x,$X)=each($Lf)){foreach($X
as$Jd=>$W){unset($Lf[$x][$Jd]);if(is_array($W)){$Lf[$x][stripslashes($Jd)]=$W;$Lf[]=&$Lf[$x][stripslashes($Jd)];}else$Lf[$x][stripslashes($Jd)]=($Mc?$W:stripslashes($W));}}}}function
bracket_escape($t,$Qa=false){static$wh=array(':'=>':1',']'=>':2','['=>':3');return
strtr($t,($Qa?array_flip($wh):$wh));}function
...
nl_br($P){return
str_replace("\n","<br>",$P);}function
checkbox($C,$Y,$gb,$Qd="",$Se="",$lb=""){$J="<input type='checkbox' name='$C' value='".h($Y)."'".($gb?" checked":"").($Se?' onclick="'.h($Se).'"':'').">";return($Qd!=""||$lb?"<label".($lb?" class='$lb'":"").">$J".h($Qd)."</label>":$J);}function
optionlist($Ye,$wg=null,$Rh=false){$J="";foreach($Ye
as$Jd=>$W){$Ze=array($Jd=>$W);if(is_array($W)){$J.='<optgroup label="'.h($Jd).'">';$Ze=$W;}foreach($Ze
as$x=>$X)$J.='<option'.($Rh||is_string($x)?' value="'.h($x).'"':'').(($Rh||is_string($x)?(string)$x:$X)===$wg?' selected':'').'>'.h($X);if(is_array($W))$J.='</optgroup>';}return$J;}function
html_select($C,$Ye,$Y="",$Re=true){if($Re)return"<select name='".h($C)."'".(is_string($Re)?' onchange="'.h($Re).'"':"").">".optionlist($Ye,$Y)."</select>";$J="";foreach($Ye
as$x=>$X)$J.="<label><input type='radio' name='".h($C)."' value='".h($x)."'".($x==$Y?" checked":"").">".h($X)."</label>";return$J;}function
select_input($Ma,$Ye,$Y="",$yf=""){return($Ye?"<select$Ma><option value=''>$yf".optionlist($Ye,$Y,true)."</select>":"<input$Ma size='10' value='".h($Y)."' placeholder='$yf'>");}function
confirm(){return" onclick=\"return confirm('".lang(0)."');\"";}function
print_fieldset($md,$Yd,$ci=false,$Se=""){echo"<fieldset><legend><a href='#fieldset-$md' onclick=\"".h($Se)."return !toggle('fieldset-$md');\">$Yd</a></legend><div id='fieldset-$md'".($ci?"":" class='hidden'").">\n";}function
bold($Ya,$lb=""){return($Ya?" class='active $lb'":($lb?" class='$lb'":""));}function
odd($J=' class="odd"'){static$s=0;if(!$J)$s=-1;return($s++%2?$J:'');}function
js_escape($P){return
addcslashes($P,"\r\n'\\/");}function
json_row($x,$X=null){static$Nc=true;if($Nc)echo"{";if($x!=""){echo($Nc?"":",")."\n\t\"".addcslashes($x,"\r\n\"\\/").'": '.($X!==null?'"'.addcslashes($X,"\r\n\"\\/").'"':'undefined');$Nc=false;}else{echo"\n}\n";$Nc=true;}}function
ini_bool($wd){$X=ini_get($wd);return(preg_match('~^(on|true|yes)$~i',$X)||(int)$X);}function
sid(){static$J;if($J===null)$J=(SID&&!($_COOKIE&&ini_bool("session.use_cookies")));return$J;}function
set_password($Yh,$N,$V,$G){$_SESSION["pwds"][$Yh][$N][$V]=($_COOKIE["adminer_key"]&&is_string($G)?array(encrypt_string($G,$_COOKIE["adminer_key"])):$G);}function
...
get_key_vals($H,$i=null,$mh=0){global$h;if(!is_object($i))$i=$h;$J=array();$i->timeout=$mh;$I=$i->query($H);$i->timeout=0;if(is_object($I)){while($K=$I->fetch_row())$J[$K[0]]=$K[1];}return$J;}function
get_rows($H,$i=null,$n="<p class='error'>"){global$h;$xb=(is_object($i)?$i:$h);$J=array();$I=$xb->query($H);if(is_object($I)){while($K=$I->fetch_assoc())$J[]=$K;}elseif(!$I&&!is_object($i)&&$n&&defined("PAGE_HEADER"))echo$n.error()."\n";return$J;}function
...
as$x=>$X){if($M&&!in_array(idf_escape($x),$M))continue;$Ja=convert_field($p[$x]);if($Ja)$J.=", $Ja AS ".idf_escape($x);}return$J;}function
cookie($C,$Y,$ae=2592000){global$ba;$F=array($C,(preg_match("~\n~",$Y)?"":$Y),($ae?time()+$ae:0),preg_replace('~\\?.*~','',$_SERVER["REQUEST_URI"]),"",$ba);if(version_compare(PHP_VERSION,'5.2.0')>=0)$F[]=true;return
call_user_func_array('setcookie',$F);}function
restart_session(){if(!ini_bool("session.use_cookies"))session_start();}function
stop_session(){if(!ini_bool("session.use_cookies"))session_write_close();}function&get_session($x){return$_SESSION[$x][DRIVER][SERVER][$_GET["username"]];}function
set_session($x,$X){$_SESSION[$x][DRIVER][SERVER][$_GET["username"]]=$X;}function
auth_url($Yh,$N,$V,$m=null){global$ac;preg_match('~([^?]*)\\??(.*)~',remove_from_uri(implode("|",array_keys($ac))."|username|".($m!==null?"db|":"").session_name()),$B);return"$B[1]?".(sid()?SID."&":"").($Yh!="server"||$N!=""?urlencode($Yh)."=".urlencode($N)."&":"")."username=".urlencode($V).($m!=""?"&db=".urlencode($m):"").($B[2]?"&$B[2]":"");}function
...
query_redirect($H,$A,$pe,$Vf=true,$zc=true,$Gc=false,$lh=""){global$h,$n,$b;if($zc){$Kg=microtime(true);$Gc=!$h->query($H);$lh=format_time($Kg);}$Ig="";if($H)$Ig=$b->messageQuery($H,$lh);if($Gc){$n=error().$Ig;return
false;}if($Vf)redirect($A,$pe.$Ig);return
true;}function
queries($H){global$h;static$Pf=array();static$Kg;if(!$Kg)$Kg=microtime(true);if($H===null)return
array(implode("\n",$Pf),format_time($Kg));$Pf[]=(preg_match('~;$~',$H)?"DELIMITER ;;\n$H;\nDELIMITER ":$H).";";return$h->query($H);}function
apply_queries($H,$S,$vc='table'){foreach($S
as$Q){if(!queries("$H ".$vc($Q)))return
false;}return
true;}function
queries_redirect($A,$pe,$Vf){list($Pf,$lh)=queries(null);return
query_redirect($Pf,$A,$pe,$Vf,false,!$Vf,$lh);}function
...
get_file($x,$Nb=false){$Kc=$_FILES[$x];if(!$Kc)return
null;foreach($Kc
as$x=>$X)$Kc[$x]=(array)$X;$J='';foreach($Kc["error"]as$x=>$n){if($n)return$n;$C=$Kc["name"][$x];$th=$Kc["tmp_name"][$x];$zb=file_get_contents($Nb&&preg_match('~\\.gz$~',$C)?"compress.zlib://$th":$th);if($Nb){$Kg=substr($zb,0,3);if(function_exists("iconv")&&preg_match("~^\xFE\xFF|^\xFF\xFE~",$Kg,$bg))$zb=iconv("utf-16","utf-8",$zb);elseif($Kg=="\xEF\xBB\xBF")$zb=substr($zb,3);$J.=$zb."\n\n";}else$J.=$zb;}return$J;}function
upload_error($n){$me=($n==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($n?lang(2).($me?" ".lang(3,$me):""):lang(4));}function
repeat_pattern($wf,$y){return
str_repeat("$wf{0,65535}",$y/65535)."$wf{0,".($y%65535)."}";}function
is_utf8($X){return(preg_match('~~u',$X)&&!preg_match('~[\\0-\\x8\\xB\\xC\\xE-\\x1F]~',$X));}function
shorten_utf8($P,$y=80,$Rg=""){if(!preg_match("(^(".repeat_pattern("[\t\r\n -\x{FFFF}]",$y).")($)?)u",$P,$B))preg_match("(^(".repeat_pattern("[\t\r\n -~]",$y).")($)?)",$P,$B);return
h($B[1]).$Rg.(isset($B[2])?"":"<i>...</i>");}function
format_number($X){return
strtr(number_format($X,0,".",lang(5)),preg_split('~~u',lang(6),-1,PREG_SPLIT_NO_EMPTY));}function
friendly_url($X){return
preg_replace('~[^a-z0-9_]~i','-',$X);}function
hidden_fields($Lf,$pd=array()){while(list($x,$X)=each($Lf)){if(!in_array($x,$pd)){if(is_array($X)){foreach($X
as$Jd=>$W)$Lf[$x."[$Jd]"]=$W;}else
echo'<input type="hidden" name="'.h($x).'" value="'.h($X).'">';}}}function
hidden_fields_get(){echo(sid()?'<input type="hidden" name="'.session_name().'" value="'.h(session_id()).'">':''),(SERVER!==null?'<input type="hidden" name="'.DRIVER.'" value="'.h(SERVER).'">':""),'<input type="hidden" name="username" value="'.h($_GET["username"]).'">';}function
table_status1($Q,$Hc=false){$J=table_status($Q,$Hc);return($J?$J:array("Name"=>$Q));}function
column_foreign_keys($Q){global$b;$J=array();foreach($b->foreignKeys($Q)as$q){foreach($q["source"]as$X)$J[$X][]=$q;}return$J;}function
enum_input($U,$Ma,$o,$Y,$pc=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$o["length"],$he);$J=($pc!==null?"<label><input type='$U'$Ma value='$pc'".((is_array($Y)?in_array($pc,$Y):$Y===0)?" checked":"")."><i>".lang(7)."</i></label>":"");foreach($he[1]as$s=>$X){$X=stripcslashes(str_replace("''","'",$X));$gb=(is_int($Y)?$Y==$s+1:(is_array($Y)?in_array($s+1,$Y):$Y===$X));$J.=" <label><input type='$U'$Ma value='".($s+1)."'".($gb?' checked':'').'>'.h($b->editVal($X,$o)).'</label>';}return$J;}function
input($o,$Y,$r){global$h,$Eh,$b,$w;$C=h(bracket_escape($o["field"]));echo"<td class='function'>";if(is_array($Y)&&!$r){$Ha=array($Y);if(version_compare(PHP_VERSION,5.4)>=0)$Ha[]=JSON_PRETTY_PRINT;$Y=call_user_func_array('json_encode',$Ha);$r="json";}$eg=($w=="mssql"&&$o["auto_increment"]);if($eg&&!$_POST["save"])$r=null;$Xc=(isset($_GET["select"])||$eg?array("orig"=>lang(8)):array())+$b->editFunctions($o);$Ma=" name='fields[$C]'";if($o["type"]=="enum")echo
nbsp($Xc[""])."<td>".$b->editInput($_GET["edit"],$o,$Ma,$Y);else{$Nc=0;foreach($Xc
as$x=>$X){if($x===""||!$X)break;$Nc++;}$Re=($Nc?" onchange=\"var f = this.form['function[".h(js_escape(bracket_escape($o["field"])))."]']; if ($Nc > f.selectedIndex) f.selectedIndex = $Nc;\" onkeyup='keyupChange.call(this);'":"");$Ma.=$Re;$fd=(in_array($r,$Xc)||isset($Xc[$r]));echo(count($Xc)>1?"<select name='function[$C]' onchange='functionChange(this);'".on_help("getTarget(event).value.replace(/^SQL\$/, '')",1).">".optionlist($Xc,$r===null||$fd?$r:"")."</select>":nbsp(reset($Xc))).'<td>';$yd=$b->editInput($_GET["edit"],$o,$Ma,$Y);if($yd!="")echo$yd;elseif($o["type"]=="set"){preg_match_all("~'((?:[^']|'')*)'~",$o["length"],$he);foreach($he[1]as$s=>$X){$X=stripcslashes(str_replace("''","'",$X));$gb=(is_int($Y)?($Y>>$s)&1:in_array($X,explode(",",$Y),true));echo" <label><input type='checkbox' name='fields[$C][$s]' value='".(1<<$s)."'".($gb?' checked':'')."$Re>".h($b->editVal($X,$o)).'</label>';}}elseif(preg_match('~blob|bytea|raw|file~',$o["type"])&&ini_bool("file_uploads"))echo"<input type='file' name='fields-$C'$Re>";elseif(($jh=preg_match('~text|lob~',$o["type"]))||preg_match("~\n~",$Y)){if($jh&&$w!="sqlite")$Ma.=" cols='50' rows='12'";else{$L=min(12,substr_count($Y,"\n")+1);$Ma.=" cols='30' rows='$L'".($L==1?" style='height: 1.2em;'":"");}echo"<textarea$Ma>".h($Y).'</textarea>';}elseif($r=="json")echo"<textarea$Ma cols='50' rows='12' class='jush-js'>".h($Y).'</textarea>';else{$oe=(!preg_match('~int~',$o["type"])&&preg_match('~^(\\d+)(,(\\d+))?$~',$o["length"],$B)?((preg_match("~binary~",$o["type"])?2:1)*$B[1]+($B[3]?1:0)+($B[2]&&!$o["unsigned"]?1:0)):($Eh[$o["type"]]?$Eh[$o["type"]]+($o["unsigned"]?0:1):0));if($w=='sql'&&$h->server_info>=5.6&&preg_match('~time~',$o["type"]))$oe+=7;echo"<input".((!$fd||$r==="")&&preg_match('~(?<!o)int~',$o["type"])?" type='number'":"")." value='".h($Y)."'".($oe?" maxlength='$oe'":"").(preg_match('~char|binary~',$o["type"])&&$oe>20?" size='40'":"")."$Ma>";}}}function
...
false;return$Y;}if(preg_match('~blob|bytea|raw|file~',$o["type"])&&ini_bool("file_uploads")){$Kc=get_file("fields-$t");if(!is_string($Kc))return
false;return
q($Kc);}return$b->processInput($o,$Y,$r);}function
fields_from_edit(){global$Zb;$J=array();foreach((array)$_POST["field_keys"]as$x=>$X){if($X!=""){$X=bracket_escape($X);$_POST["function"][$X]=$_POST["field_funs"][$x];$_POST["fields"][$X]=$_POST["field_vals"][$x];}}foreach((array)$_POST["fields"]as$x=>$X){$C=bracket_escape($x,1);$J[$C]=array("field"=>$C,"privileges"=>array("insert"=>1,"update"=>1),"null"=>1,"auto_increment"=>($x==$Zb->primary),);}return$J;}function
search_tables(){global$b,$h;$_GET["where"][0]["op"]="LIKE %%";$_GET["where"][0]["val"]=$_POST["query"];$Tc=false;foreach(table_status('',true)as$Q=>$R){$C=$b->tableName($R);if(isset($R["Engine"])&&$C!=""&&(!$_POST["tables"]||in_array($Q,$_POST["tables"]))){$I=$h->query("SELECT".limit("1 FROM ".table($Q)," WHERE ".implode(" AND ",$b->selectSearchProcess(fields($Q),array())),1));if(!$I||$I->fetch_row()){if(!$Tc){echo"<ul>\n";$Tc=true;}echo"<li>".($I?"<a href='".h(ME."select=".urlencode($Q)."&where[0][op]=".urlencode($_GET["where"][0]["op"])."&where[0][val]=".urlencode($_GET["where"][0]["val"]))."'>$C</a>\n":"$C: <span class='error'>".error()."</span>\n");}}}echo($Tc?"</ul>":"<p class='message'>".lang(9))."\n";}function
dump_headers($nd,$ye=false){global$b;$J=$b->dumpHeaders($nd,$ye);$kf=$_POST["output"];if($kf!="text")header("Content-Disposition: attachment; filename=".$b->dumpFilename($nd).".$J".($kf!="file"&&!preg_match('~[^0-9a-z]~',$kf)?".$kf":""));session_write_close();ob_flush();flush();return$J;}function
...
fetch_field(){$K=(object)$this->getColumnMeta($this->_offset++);$K->orgtable=$K->table;$K->orgname=$K->name;$K->charsetnr=(in_array("blob",(array)$K->flags)?63:0);return$K;}}}$ac=array();class
Min_SQL{var$_conn;function
__construct($h){$this->_conn=$h;}function
select($Q,$M,$Z,$ad,$af=array(),$z=1,$E=0,$Hf=false){global$b,$w;$Dd=(count($ad)<count($M));$H=$b->selectQueryBuild($M,$Z,$ad,$af,$z,$E);if(!$H)$H="SELECT".limit(($_GET["page"]!="last"&&+$z&&$ad&&$Dd&&$w=="sql"?"SQL_CALC_FOUND_ROWS ":"").implode(", ",$M)."\nFROM ".table($Q),($Z?"\nWHERE ".implode(" AND ",$Z):"").($ad&&$Dd?"\nGROUP BY ".implode(", ",$ad):"").($af?"\nORDER BY ".implode(", ",$af):""),($z!=""?+$z:null),($E?$z*$E:0),"\n");$Kg=microtime(true);$J=$this->_conn->query($H);if($Hf)echo$b->selectQuery($H,format_time($Kg));return$J;}function
delete($Q,$Qf,$z=0){$H="FROM ".table($Q);return
queries("DELETE".($z?limit1($H,$Qf):" $H$Qf"));}function
update($Q,$O,$Qf,$z=0,$yg="\n"){$Wh=array();foreach($O
as$x=>$X)$Wh[]="$x = $X";$H=table($Q)." SET$yg".implode(",$yg",$Wh);return
queries("UPDATE".($z?limit1($H,$Qf):" $H$Qf"));}function
insert($Q,$O){return
queries("INSERT INTO ".table($Q).($O?" (".implode(", ",array_keys($O)).")\nVALUES (".implode(", ",$O).")":" DEFAULT VALUES"));}function
insertUpdate($Q,$L,$Ff){return
false;}function
begin(){return
...
rollback(){return
queries("ROLLBACK");}}$ac["sqlite"]="SQLite 3";$ac["sqlite2"]="SQLite 2";if(isset($_GET["sqlite"])||isset($_GET["sqlite2"])){$Cf=array((isset($_GET["sqlite"])?"SQLite3":"SQLite"),"PDO_SQLite");define("DRIVER",(isset($_GET["sqlite"])?"sqlite":"sqlite2"));if(class_exists(isset($_GET["sqlite"])?"SQLite3":"SQLiteDatabase")){if(isset($_GET["sqlite"])){class
Min_SQLite{var$extension="SQLite3",$server_info,$affected_rows,$errno,$error,$_link;function
__construct($Lc){$this->_link=new
SQLite3($Lc);$Zh=$this->_link->version();$this->server_info=$Zh["versionString"];}function
...
query($H,$Fh=false){$ve=($Fh?"unbufferedQuery":"query");$I=@$this->_link->$ve($H,SQLITE_BOTH,$n);$this->error="";if(!$I){$this->error=$n;return
...
fetch_field(){$C=$this->_result->fieldName($this->_offset++);$wf='(\\[.*]|"(?:[^"]|"")*"|(.+))';if(preg_match("~^($wf\\.)?$wf\$~",$C,$B)){$Q=($B[3]!=""?$B[3]:idf_unescape($B[2]));$C=($B[5]!=""?$B[5]:idf_unescape($B[4]));}return(object)array("name"=>$C,"orgname"=>$C,"orgtable"=>$Q,);}}}}elseif(extension_loaded("pdo_sqlite")){class
Min_SQLite
extends
...
extends
Min_SQL{function
insertUpdate($Q,$L,$Ff){$Wh=array();foreach($L
as$O)$Wh[]="(".implode(", ",$O).")";return
...
array();}function
limit($H,$Z,$z,$D=0,$yg=" "){return" $H$Z".($z!==null?$yg."LIMIT $z".($D?" OFFSET $D":""):"");}function
...
fields($Q){global$h;$J=array();$Ff="";foreach(get_rows("PRAGMA table_info(".table($Q).")")as$K){$C=$K["name"];$U=strtolower($K["type"]);$Ob=$K["dflt_value"];$J[$C]=array("field"=>$C,"type"=>(preg_match('~int~i',$U)?"integer":(preg_match('~char|clob|text~i',$U)?"text":(preg_match('~blob~i',$U)?"blob":(preg_match('~real|floa|doub~i',$U)?"real":"numeric")))),"full_type"=>$U,"default"=>(preg_match("~'(.*)'~",$Ob,$B)?str_replace("''","'",$B[1]):($Ob=="NULL"?null:$Ob)),"null"=>!$K["notnull"],"privileges"=>array("select"=>1,"insert"=>1,"update"=>1),"primary"=>$K["pk"],);if($K["pk"]){if($Ff!="")$J[$Ff]["auto_increment"]=false;elseif(preg_match('~^integer$~i',$U))$J[$C]["auto_increment"]=true;$Ff=$C;}}$Ig=$h->result("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = ".q($Q));preg_match_all('~(("[^"]*+")+|[a-z0-9_]+)\s+text\s+COLLATE\s+(\'[^\']+\'|\S+)~i',$Ig,$he,PREG_SET_ORDER);foreach($he
as$B){$C=str_replace('""','"',preg_replace('~^"|"$~','',$B[1]));if($J[$C])$J[$C]["collation"]=trim($B[3],"'");}return$J;}function
indexes($Q,$i=null){global$h;if(!is_object($i))$i=$h;$J=array();$Ig=$i->result("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = ".q($Q));if(preg_match('~\bPRIMARY\s+KEY\s*\((([^)"]+|"[^"]*")++)~i',$Ig,$B)){$J[""]=array("type"=>"PRIMARY","columns"=>array(),"lengths"=>array(),"descs"=>array());preg_match_all('~((("[^"]*+")+)|(\S+))(\s+(ASC|DESC))?(,\s*|$)~i',$B[1],$he,PREG_SET_ORDER);foreach($he
as$B){$J[""]["columns"][]=idf_unescape($B[2]).$B[4];$J[""]["descs"][]=(preg_match('~DESC~i',$B[5])?'1':null);}}if(!$J){foreach(fields($Q)as$C=>$o){if($o["primary"])$J[""]=array("type"=>"PRIMARY","columns"=>array($C),"lengths"=>array(),"descs"=>array(null));}}$Jg=get_key_vals("SELECT name, sql FROM sqlite_master WHERE type = 'index' AND tbl_name = ".q($Q),$i);foreach(get_rows("PRAGMA index_list(".table($Q).")",$i)as$K){$C=$K["name"];$u=array("type"=>($K["unique"]?"UNIQUE":"INDEX"));$u["lengths"]=array();$u["descs"]=array();foreach(get_rows("PRAGMA index_info(".idf_escape($C).")",$i)as$og){$u["columns"][]=$og["name"];$u["descs"][]=null;}if(preg_match('~^CREATE( UNIQUE)? INDEX '.preg_quote(idf_escape($C).' ON '.idf_escape($Q),'~').' \((.*)\)$~i',$Jg[$C],$bg)){preg_match_all('/("[^"]*+")+( DESC)?/',$bg[2],$he);foreach($he[2]as$x=>$X){if($X)$u["descs"][$x]='1';}}if(!$J[""]||$u["type"]!="UNIQUE"||$u["columns"]!=$J[""]["columns"]||$u["descs"]!=$J[""]["descs"]||!preg_match("~^sqlite_~",$C))$J[$C]=$u;}return$J;}function
...
h($h->error);}function
check_sqlite_name($C){global$h;$Fc="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($Fc)\$~",$C)){$h->error=lang(21,str_replace("|",", ",$Fc));return
false;}return
true;}function
create_database($m,$d){global$h;if(file_exists($m)){$h->error=lang(22);return
false;}if(!check_sqlite_name($m))return
false;try{$_=new
Min_SQLite($m);}catch(Exception$xc){$h->error=$xc->getMessage();return
...
alter_table($Q,$C,$p,$Pc,$ub,$qc,$d,$Oa,$rf){$Qh=($Q==""||$Pc);foreach($p
as$o){if($o[0]!=""||!$o[1]||$o[2]){$Qh=true;break;}}$c=array();$if=array();foreach($p
as$o){if($o[1]){$c[]=($Qh?$o[1]:"ADD ".implode($o[1]));if($o[0]!="")$if[$o[0]]=$o[1][0];}}if(!$Qh){foreach($c
as$X){if(!queries("ALTER TABLE ".table($Q)." $X"))return
false;}if($Q!=$C&&!queries("ALTER TABLE ".table($Q)." RENAME TO ".table($C)))return
false;}elseif(!recreate_table($Q,$C,$c,$if,$Pc))return
false;if($Oa)queries("UPDATE sqlite_sequence SET seq = $Oa WHERE name = ".q($C));return
true;}function
recreate_table($Q,$C,$p,$if,$Pc,$v=array()){if($Q!=""){if(!$p){foreach(fields($Q)as$x=>$o){$p[]=process_field($o,$o);$if[$x]=idf_escape($x);}}$Gf=false;foreach($p
as$o){if($o[6])$Gf=true;}$dc=array();foreach($v
as$x=>$X){if($X[2]=="DROP"){$dc[$X[1]]=true;unset($v[$x]);}}foreach(indexes($Q)as$Md=>$u){$f=array();foreach($u["columns"]as$x=>$e){if(!$if[$e])continue
2;$f[]=$if[$e].($u["descs"][$x]?" DESC":"");}if(!$dc[$Md]){if($u["type"]!="PRIMARY"||!$Gf)$v[]=array($u["type"],$Md,$f);}}foreach($v
as$x=>$X){if($X[0]=="PRIMARY"){unset($v[$x]);$Pc[]="  PRIMARY KEY (".implode(", ",$X[2]).")";}}foreach(foreign_keys($Q)as$Md=>$q){foreach($q["source"]as$x=>$e){if(!$if[$e])continue
2;$q["source"][$x]=idf_unescape($if[$e]);}if(!isset($Pc[" $Md"]))$Pc[]=" ".format_foreign_key($q);}queries("BEGIN");}foreach($p
as$x=>$o)$p[$x]="  ".implode($o);$p=array_merge($p,array_filter($Pc));if(!queries("CREATE TABLE ".table($Q!=""?"adminer_$C":$C)." (\n".implode(",\n",$p)."\n)"))return
false;if($Q!=""){if($if&&!queries("INSERT INTO ".table("adminer_$C")." (".implode(", ",$if).") SELECT ".implode(", ",array_map('idf_escape',array_keys($if)))." FROM ".table($Q)))return
false;$Bh=array();foreach(triggers($Q)as$_h=>$nh){$zh=trigger($_h);$Bh[]="CREATE TRIGGER ".idf_escape($_h)." ".implode(" ",$nh)." ON ".table($C)."\n$zh[Statement]";}if(!queries("DROP TABLE ".table($Q)))return
false;queries("ALTER TABLE ".table("adminer_$C")." RENAME TO ".table($C));if(!alter_indexes($C,$v))return
false;foreach($Bh
as$zh){if(!queries($zh))return
false;}queries("COMMIT");}return
true;}function
index_sql($Q,$U,$C,$f){return"CREATE $U ".($U!="INDEX"?"INDEX ":"").idf_escape($C!=""?$C:uniqid($Q."_"))." ON ".table($Q)." $f";}function
alter_indexes($Q,$c){foreach($c
as$Ff){if($Ff[0]=="PRIMARY")return
...
drop_views($bi){return
apply_queries("DROP VIEW",$bi);}function
drop_tables($S){return
apply_queries("DROP TABLE",$S);}function
move_tables($S,$bi,$eh){return
false;}function
trigger($C){global$h;if($C=="")return
array("Statement"=>"BEGIN\n\t;\nEND");$t='(?:[^`"\\s]+|`[^`]*`|"[^"]*")+';$Ah=trigger_options();preg_match("~^CREATE\\s+TRIGGER\\s*$t\\s*(".implode("|",$Ah["Timing"]).")\\s+([a-z]+)(?:\\s+OF\\s+($t))?\\s+ON\\s*$t\\s*(?:FOR\\s+EACH\\s+ROW\\s)?(.*)~is",$h->result("SELECT sql FROM sqlite_master WHERE type = 'trigger' AND name = ".q($C)),$B);$Je=$B[3];return
array("Timing"=>strtoupper($B[1]),"Event"=>strtoupper($B[2]).($Je?" OF":""),"Of"=>($Je[0]=='`'||$Je[0]=='"'?idf_unescape($Je):$Je),"Trigger"=>$C,"Statement"=>$B[4],);}function
triggers($Q){$J=array();$Ah=trigger_options();foreach(get_rows("SELECT * FROM sqlite_master WHERE type = 'trigger' AND tbl_name = ".q($Q))as$K){preg_match('~^CREATE\\s+TRIGGER\\s*(?:[^`"\\s]+|`[^`]*`|"[^"]*")+\\s*('.implode("|",$Ah["Timing"]).')\\s*(.*)\\s+ON\\b~iU',$K["sql"],$B);$J[$K["name"]]=array($B[1],$B[2]);}return$J;}function
...
true;}function
create_sql($Q,$Oa){global$h;$J=$h->result("SELECT sql FROM sqlite_master WHERE type IN ('table', 'view') AND name = ".q($Q));foreach(indexes($Q)as$C=>$u){if($C=='')continue;$J.=";\n\n".index_sql($Q,$u['type'],$C,"(".implode(", ",array_map('idf_escape',$u['columns'])).")");}return$J;}function
...
show_status(){$J=array();foreach(get_vals("PRAGMA compile_options")as$Xe){list($x,$X)=explode("=",$Xe,2);$J[$x]=$X;}return$J;}function
convert_field($o){}function
unconvert_field($o,$J){return$J;}function
support($Ic){return
preg_match('~^(columns|database|drop_col|dump|indexes|move_col|sql|status|table|trigger|variables|view|view_trigger)$~',$Ic);}$w="sqlite";$Eh=array("integer"=>0,"real"=>0,"numeric"=>0,"text"=>0,"blob"=>0);$Og=array_keys($Eh);$Lh=array();$Ve=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT IN","IS NOT NULL","SQL");$Xc=array("hex","length","lower","round","unixepoch","upper");$cd=array("avg","count","count distinct","group_concat","max","min","sum");$ic=array(array(),array("integer|real|numeric"=>"+/-","text"=>"||",));}$ac["pgsql"]="PostgreSQL";if(isset($_GET["pgsql"])){$Cf=array("PgSQL","PDO_PgSQL");define("DRIVER","pgsql");if(extension_loaded("pgsql")){class
Min_DB{var$extension="PgSQL",$_link,$_result,$_string,$_database=true,$server_info,$affected_rows,$error;function
_error($tc,$n){if(ini_bool("html_errors"))$n=html_entity_decode(strip_tags($n));$n=preg_replace('~^[^:]*: ~','',$n);$this->error=$n;}function
connect($N=null,$V=null,$G=null,$ja=null,$ka=null,$la=null){global$b;$m=$b->database();set_error_handler(array($this,'_error'));$this->_string="host='".str_replace(":","' port='",addcslashes($N,"'\\"))."' user='".addcslashes($V,"'\\")."' password='".addcslashes($G,"'\\")."'";$this->_link=@pg_connect("$this->_string dbname='".($m!=""?addcslashes($m,"'\\"):"postgres")."'",PGSQL_CONNECT_FORCE_NEW);if(!$this->_link&&$m!=""){$this->_database=false;$this->_link=@pg_connect("$this->_string dbname='postgres'",PGSQL_CONNECT_FORCE_NEW);}restore_error_handler();if($this->_link){$Zh=pg_version($this->_link);$this->server_info=$Zh["server"];pg_set_client_encoding($this->_link,"UTF8");}return(bool)$this->_link;}function
quote($P){return"'".pg_escape_string($this->_link,$P)."'";}function
select_db($Jb){global$b;if($Jb==$b->database())return$this->_database;$J=@pg_connect("$this->_string dbname='".addcslashes($Jb,"'\\")."'",PGSQL_CONNECT_FORCE_NEW);if($J)$this->_link=$J;return$J;}function
close(){$this->_link=@pg_connect("$this->_string dbname='postgres'");}function
query($H,$Fh=false){$I=@pg_query($this->_link,$H);$this->error="";if(!$I){$this->error=pg_last_error($this->_link);return
...
connect($N=null,$V=null,$G=null,$ja=null,$ka=null,$la=null){global$b;$m=$b->database();$P="pgsql:host='".str_replace(":","' port='",addcslashes($N,"'\\"))."' options='-c client_encoding=utf8'";$this->dsn("$P dbname='".($m!=""?addcslashes($m,"'\\"):"postgres")."'",$V,$G);return
true;}function
...
extends
Min_SQL{function
insertUpdate($Q,$L,$Ff){global$h;foreach($L
as$O){$Mh=array();$Z=array();foreach($O
as$x=>$X){$Mh[]="$x = $X";if(isset($Ff[idf_unescape($x)]))$Z[]="$x = $X";}if(!(($Z&&queries("UPDATE ".table($Q)." SET ".implode(", ",$Mh)." WHERE ".implode(" AND ",$Z))&&$h->affected_rows)||queries("INSERT INTO ".table($Q)." (".implode(", ",array_keys($O)).") VALUES (".implode(", ",$O).")")))return
false;}return
true;}}function
idf_escape($t){return'"'.str_replace('"','""',$t).'"';}function
table($t){return
...
fk_support($R){return
true;}function
fields($Q){$J=array();$Fa=array('timestamp without time zone'=>'timestamp','timestamp with time zone'=>'timestamptz',);foreach(get_rows("SELECT a.attname AS field, format_type(a.atttypid, a.atttypmod) AS full_type, d.adsrc AS default, a.attnotnull::int, col_description(c.oid, a.attnum) AS comment
FROM pg_class c
...
AND a.attnum > 0
ORDER BY a.attnum")as$K){preg_match('~([^([]+)(\((.*)\))?([a-z ]+)?((\[[0-9]*])*)$~',$K["full_type"],$B);list(,$U,$y,$K["length"],$za,$Ia)=$B;$K["length"].=$Ia;$fb=$U.$za;if(isset($Fa[$fb])){$K["type"]=$Fa[$fb];$K["full_type"]=$K["type"].$y.$Ia;}else{$K["type"]=$U;$K["full_type"]=$K["type"].$y.$za.$Ia;}$K["null"]=!$K["attnotnull"];$K["auto_increment"]=preg_match('~^nextval\\(~i',$K["default"]);$K["privileges"]=array("insert"=>1,"select"=>1,"update"=>1);if(preg_match('~(.+)::[^)]+(.*)~',$K["default"],$B))$K["default"]=($B[1][0]=="'"?idf_unescape($B[1]):$B[1]).$B[2];$J[$K["field"]]=$K;}return$J;}function
indexes($Q,$i=null){global$h;if(!is_object($i))$i=$h;$J=array();$Xg=$i->result("SELECT oid FROM pg_class WHERE relnamespace = (SELECT oid FROM pg_namespace WHERE nspname = current_schema()) AND relname = ".q($Q));$f=get_key_vals("SELECT attnum, attname FROM pg_attribute WHERE attrelid = $Xg AND attnum > 0",$i);foreach(get_rows("SELECT relname, indisunique::int, indisprimary::int, indkey, indoption FROM pg_index i, pg_class ci WHERE i.indrelid = $Xg AND ci.oid = i.indexrelid",$i)as$K){$cg=$K["relname"];$J[$cg]["type"]=($K["indisprimary"]?"PRIMARY":($K["indisunique"]?"UNIQUE":"INDEX"));$J[$cg]["columns"]=array();foreach(explode(" ",$K["indkey"])as$td)$J[$cg]["columns"][]=$f[$td];$J[$cg]["descs"]=array();foreach(explode(" ",$K["indoption"])as$ud)$J[$cg]["descs"][]=($ud&1?'1':null);$J[$cg]["lengths"]=array();}return$J;}function
foreign_keys($Q){global$Qe;$J=array();foreach(get_rows("SELECT conname, pg_get_constraintdef(oid) AS definition
FROM pg_constraint
...
alter_table($Q,$C,$p,$Pc,$ub,$qc,$d,$Oa,$rf){$c=array();$Pf=array();foreach($p
as$o){$e=idf_escape($o[0]);$X=$o[1];if(!$X)$c[]="DROP $e";else{$Vh=$X[5];unset($X[5]);if(isset($X[6])&&$o[0]=="")$X[1]=($X[1]=="bigint"?" big":" ")."serial";if($o[0]=="")$c[]=($Q!=""?"ADD ":"  ").implode($X);else{if($e!=$X[0])$Pf[]="ALTER TABLE ".table($Q)." RENAME $e TO $X[0]";$c[]="ALTER $e TYPE$X[1]";if(!$X[6]){$c[]="ALTER $e ".($X[3]?"SET$X[3]":"DROP DEFAULT");$c[]="ALTER $e ".($X[2]==" NULL"?"DROP NOT":"SET").$X[2];}}if($o[0]!=""||$Vh!="")$Pf[]="COMMENT ON COLUMN ".table($Q).".$X[0] IS ".($Vh!=""?substr($Vh,9):"''");}}$c=array_merge($c,$Pc);if($Q=="")array_unshift($Pf,"CREATE TABLE ".table($C)." (\n".implode(",\n",$c)."\n)");elseif($c)array_unshift($Pf,"ALTER TABLE ".table($Q)."\n".implode(",\n",$c));if($Q!=""&&$Q!=$C)$Pf[]="ALTER TABLE ".table($Q)." RENAME TO ".table($C);if($Q!=""||$ub!="")$Pf[]="COMMENT ON TABLE ".table($C)." IS ".q($ub);if($Oa!=""){}foreach($Pf
as$H){if(!queries($H))return
false;}return
true;}function
alter_indexes($Q,$c){$j=array();$bc=array();$Pf=array();foreach($c
as$X){if($X[0]!="INDEX")$j[]=($X[2]=="DROP"?"\nDROP CONSTRAINT ".idf_escape($X[1]):"\nADD".($X[1]!=""?" CONSTRAINT ".idf_escape($X[1]):"")." $X[0] ".($X[0]=="PRIMARY"?"KEY ":"")."(".implode(", ",$X[2]).")");elseif($X[2]=="DROP")$bc[]=idf_escape($X[1]);else$Pf[]="CREATE INDEX ".idf_escape($X[1]!=""?$X[1]:uniqid($Q."_"))." ON ".table($Q)." (".implode(", ",$X[2]).")";}if($j)array_unshift($Pf,"ALTER TABLE ".table($Q).implode(",",$j));if($bc)array_unshift($Pf,"DROP INDEX ".implode(", ",$bc));foreach($Pf
as$H){if(!queries($H))return
false;}return
true;}function
truncate_tables($S){return
...
true;}function
drop_views($bi){return
drop_tables($bi);}function
drop_tables($S){foreach($S
as$Q){$Lg=table_status($Q);if(!queries("DROP ".strtoupper($Lg["Engine"])." ".table($Q)))return
false;}return
true;}function
move_tables($S,$bi,$eh){foreach(array_merge($S,$bi)as$Q){$Lg=table_status($Q);if(!queries("ALTER ".strtoupper($Lg["Engine"])." ".table($Q)." SET SCHEMA ".idf_escape($eh)))return
false;}return
true;}function
...
found_rows($R,$Z){global$h;if(preg_match("~ rows=([0-9]+)~",$h->result("EXPLAIN SELECT * FROM ".idf_escape($R["Name"]).($Z?" WHERE ".implode(" AND ",$Z):"")),$bg))return$bg[1];return
false;}function
types(){return
...
set_schema($rg){global$h,$Eh,$Og;$J=$h->query("SET search_path TO ".idf_escape($rg));foreach(types()as$U){if(!isset($Eh[$U])){$Eh[$U]=0;$Og[lang(23)][]=$U;}}return$J;}function
...
convert_field($o){}function
unconvert_field($o,$J){return$J;}function
support($Ic){global$h;return
preg_match('~^(database|table|columns|sql|indexes|comment|view|'.($h->server_info>=9.3?'materializedview|':'').'scheme|processlist|sequence|trigger|type|variables|drop_col)$~',$Ic);}$w="pgsql";$Eh=array();$Og=array();foreach(array(lang(24)=>array("smallint"=>5,"integer"=>10,"bigint"=>19,"boolean"=>1,"numeric"=>0,"real"=>7,"double precision"=>16,"money"=>20),lang(25)=>array("date"=>13,"time"=>17,"timestamp"=>20,"timestamptz"=>21,"interval"=>0),lang(26)=>array("character"=>0,"character varying"=>0,"text"=>0,"tsquery"=>0,"tsvector"=>0,"uuid"=>0,"xml"=>0),lang(27)=>array("bit"=>0,"bit varying"=>0,"bytea"=>0),lang(28)=>array("cidr"=>43,"inet"=>43,"macaddr"=>17,"txid_snapshot"=>0),lang(29)=>array("box"=>0,"circle"=>0,"line"=>0,"lseg"=>0,"path"=>0,"point"=>0,"polygon"=>0),)as$x=>$X){$Eh+=$X;$Og[$x]=array_keys($X);}$Lh=array();$Ve=array("=","<",">","<=",">=","!=","~","!~","LIKE","LIKE %%","ILIKE","ILIKE %%","IN","IS NULL","NOT LIKE","NOT IN","IS NOT NULL");$Xc=array("char_length","lower","round","to_hex","to_timestamp","upper");$cd=array("avg","count","count distinct","max","min","sum");$ic=array(array("char"=>"md5","date|time"=>"now",),array("int|numeric|real|money"=>"+/-","date|time"=>"+ interval/- interval","char|text"=>"||",));}$ac["oracle"]="Oracle";if(isset($_GET["oracle"])){$Cf=array("OCI8","PDO_OCI");define("DRIVER","oracle");if(extension_loaded("oci8")){class
Min_DB{var$extension="oci8",$_link,$_result,$server_info,$affected_rows,$errno,$error;function
_error($tc,$n){if(ini_bool("html_errors"))$n=html_entity_decode(strip_tags($n));$n=preg_replace('~^[^:]*: ~','',$n);$this->error=$n;}function
connect($N=null,$V=null,$G=null,$ja=null,$ka=null,$la=null){$this->_link=@oci_new_connect($V,$G,$N,"AL32UTF8");if($this->_link){$this->server_info=oci_server_version($this->_link);return
true;}$n=oci_error();$this->error=$n["message"];return
false;}function
quote($P){return"'".str_replace("'","''",$P)."'";}function
select_db($Jb){return
true;}function
query($H,$Fh=false){$I=oci_parse($this->_link,$H);$this->error="";if(!$I){$n=oci_error($this->_link);$this->errno=$n["code"];$this->error=$n["message"];return
...
connect($N=null,$V=null,$G=null,$ja=null,$ka=null,$la=null){$this->dsn("oci:dbname=//$N;charset=AL32UTF8",$V,$G);return
true;}function
select_db($Jb){return
true;}}}class
Min_Driver
...
limit($H,$Z,$z,$D=0,$yg=" "){return($D?" * FROM (SELECT t.*, rownum AS rnum FROM (SELECT $H$Z) t WHERE rownum <= ".($z+$D).") WHERE rnum > $D":($z!==null?" * FROM (SELECT $H$Z) WHERE rownum <= ".($z+$D):" $H$Z"));}function
limit1($H,$Z){return" $H$Z";}function
...
count_tables($l){return
array();}function
table_status($C=""){$J=array();$tg=q($C);foreach(get_rows('SELECT table_name "Name", \'table\' "Engine", avg_row_len * num_rows "Data_length", num_rows "Rows" FROM all_tables WHERE tablespace_name = '.q(DB).($C!=""?" AND table_name = $tg":"")."
...
ORDER BY uc.constraint_type, uic.column_position",$i)as$K){$rd=$K["INDEX_NAME"];$J[$rd]["type"]=($K["CONSTRAINT_TYPE"]=="P"?"PRIMARY":($K["CONSTRAINT_TYPE"]=="U"?"UNIQUE":"INDEX"));$J[$rd]["columns"][]=$K["COLUMN_NAME"];$J[$rd]["lengths"][]=($K["CHAR_LENGTH"]&&$K["CHAR_LENGTH"]!=$K["COLUMN_LENGTH"]?$K["CHAR_LENGTH"]:null);$J[$rd]["descs"][]=($K["DESCEND"]?'1':null);}return$J;}function
...
found_rows($R,$Z){}function
alter_table($Q,$C,$p,$Pc,$ub,$qc,$d,$Oa,$rf){$c=$bc=array();foreach($p
as$o){$X=$o[1];if($X&&$o[0]!=""&&idf_escape($o[0])!=$X[0])queries("ALTER TABLE ".table($Q)." RENAME COLUMN ".idf_escape($o[0])." TO $X[0]");if($X)$c[]=($Q!=""?($o[0]!=""?"MODIFY (":"ADD ("):"  ").implode($X).($Q!=""?")":"");else$bc[]=idf_escape($o[0]);}if($Q=="")return
queries("CREATE TABLE ".table($C)." (\n".implode(",\n",$c)."\n)");return(!$c||queries("ALTER TABLE ".table($Q)."\n".implode("\n",$c)))&&(!$bc||queries("ALTER TABLE ".table($Q)." DROP (".implode(", ",$bc).")"))&&($Q==$C||queries("ALTER TABLE ".table($Q)." RENAME TO ".table($C)));}function
...
set_schema($sg){global$h;return$h->query("ALTER SESSION SET CURRENT_SCHEMA = ".idf_escape($sg));}function
show_variables(){return
...
unconvert_field($o,$J){return$J;}function
support($Ic){return
preg_match('~^(columns|database|drop_col|indexes|processlist|scheme|sql|status|table|variables|view|view_trigger)$~',$Ic);}$w="oracle";$Eh=array();$Og=array();foreach(array(lang(24)=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),lang(25)=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),lang(26)=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),lang(27)=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$x=>$X){$Eh+=$X;$Og[$x]=array_keys($X);}$Lh=array();$Ve=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","SQL");$Xc=array("length","lower","round","upper");$cd=array("avg","count","count distinct","max","min","sum");$ic=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$ac["mssql"]="MS SQL";if(isset($_GET["mssql"])){$Cf=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
Min_DB{var$extension="sqlsrv",$_link,$_result,$server_info,$affected_rows,$errno,$error;function
_get_error(){$this->error="";foreach(sqlsrv_errors()as$n){$this->errno=$n["code"];$this->error.="$n[message]\n";}$this->error=rtrim($this->error);}function
connect($N=null,$V=null,$G=null,$ja=null,$ka=null,$la=null){$this->_link=@sqlsrv_connect($N,array("UID"=>$V,"PWD"=>$G,"CharacterSet"=>"UTF-8"));if($this->_link){$vd=sqlsrv_server_info($this->_link);$this->server_info=$vd['SQLServerVersion'];}else$this->_get_error();return(bool)$this->_link;}function
quote($P){return"'".str_replace("'","''",$P)."'";}function
select_db($Jb){return$this->query("USE ".idf_escape($Jb));}function
query($H,$Fh=false){$I=sqlsrv_query($this->_link,$H);$this->error="";if(!$I){$this->_get_error();return
...
connect($N=null,$V=null,$G=null,$ja=null,$ka=null,$la=null){$this->_link=@mssql_connect($N,$V,$G);if($this->_link){$I=$this->query("SELECT SERVERPROPERTY('ProductLevel'), SERVERPROPERTY('Edition')");$K=$I->fetch_row();$this->server_info=$this->result("sp_server_info 2",2)." [$K[0]] $K[1]";}else$this->error=mssql_get_last_message();return(bool)$this->_link;}function
quote($P){return"'".str_replace("'","''",$P)."'";}function
select_db($Jb){return
mssql_select_db($Jb);}function
query($H,$Fh=false){$I=mssql_query($H,$this->_link);$this->error="";if(!$I){$this->error=mssql_get_last_message();return
...
extends
Min_SQL{function
insertUpdate($Q,$L,$Ff){foreach($L
as$O){$Mh=array();$Z=array();foreach($O
as$x=>$X){$Mh[]="$x = $X";if(isset($Ff[idf_unescape($x)]))$Z[]="$x = $X";}if(!queries("MERGE ".table($Q)." USING (VALUES(".implode(", ",$O).")) AS source (c".implode(", c",range(1,count($O))).") ON ".implode(" AND ",$Z)." WHEN MATCHED THEN UPDATE SET ".implode(", ",$Mh)." WHEN NOT MATCHED THEN INSERT (".implode(", ",array_keys($O)).") VALUES (".implode(", ",$O).");"))return
false;}return
true;}function
begin(){return
...
limit($H,$Z,$z,$D=0,$yg=" "){return($z!==null?" TOP (".($z+$D).")":"")." $H$Z";}function
limit1($H,$Z){return
...
alter_table($Q,$C,$p,$Pc,$ub,$qc,$d,$Oa,$rf){$c=array();foreach($p
as$o){$e=idf_escape($o[0]);$X=$o[1];if(!$X)$c["DROP"][]=" COLUMN $e";else{$X[1]=preg_replace("~( COLLATE )'(\\w+)'~","\\1\\2",$X[1]);if($o[0]=="")$c["ADD"][]="\n  ".implode("",$X).($Q==""?substr($Pc[$X[0]],16+strlen($X[0])):"");else{unset($X[6]);if($e!=$X[0])queries("EXEC sp_rename ".q(table($Q).".$e").", ".q(idf_unescape($X[0])).", 'COLUMN'");$c["ALTER COLUMN ".implode("",$X)][]="";}}}if($Q=="")return
queries("CREATE TABLE ".table($C)." (".implode(",",(array)$c["ADD"])."\n)");if($Q!=$C)queries("EXEC sp_rename ".q(table($Q)).", ".q($C));if($Pc)$c[""]=$Pc;foreach($c
as$x=>$X){if(!queries("ALTER TABLE ".idf_escape($C)." $x".implode(",",$X)))return
false;}return
true;}function
alter_indexes($Q,$c){$u=array();$bc=array();foreach($c
as$X){if($X[2]=="DROP"){if($X[0]=="PRIMARY")$bc[]=idf_escape($X[1]);else$u[]=idf_escape($X[1])." ON ".table($Q);}elseif(!queries(($X[0]!="PRIMARY"?"CREATE $X[0] ".($X[0]!="INDEX"?"INDEX ":"").idf_escape($X[1]!=""?$X[1]:uniqid($Q."_"))." ON ".table($Q):"ALTER TABLE ".table($Q)." ADD PRIMARY KEY")." (".implode(", ",$X[2]).")"))return
false;}return(!$u||queries("DROP INDEX ".implode(", ",$u)))&&(!$bc||queries("ALTER TABLE ".table($Q)." DROP ".implode(", ",$bc)));}function
...
apply_queries("TRUNCATE TABLE",$S);}function
drop_views($bi){return
queries("DROP VIEW ".implode(", ",array_map('table',$bi)));}function
drop_tables($S){return
queries("DROP TABLE ".implode(", ",array_map('table',$S)));}function
move_tables($S,$bi,$eh){return
apply_queries("ALTER SCHEMA ".idf_escape($eh)." TRANSFER",array_merge($S,$bi));}function
trigger($C){if($C=="")return
...
unconvert_field($o,$J){return$J;}function
support($Ic){return
preg_match('~^(columns|database|drop_col|indexes|scheme|sql|table|trigger|view|view_trigger)$~',$Ic);}$w="mssql";$Eh=array();$Og=array();foreach(array(lang(24)=>array("tinyint"=>3,"smallint"=>5,"int"=>10,"bigint"=>20,"bit"=>1,"decimal"=>0,"real"=>12,"float"=>53,"smallmoney"=>10,"money"=>20),lang(25)=>array("date"=>10,"smalldatetime"=>19,"datetime"=>19,"datetime2"=>19,"time"=>8,"datetimeoffset"=>10),lang(26)=>array("char"=>8000,"varchar"=>8000,"text"=>2147483647,"nchar"=>4000,"nvarchar"=>4000,"ntext"=>1073741823),lang(27)=>array("binary"=>8000,"varbinary"=>8000,"image"=>2147483647),)as$x=>$X){$Eh+=$X;$Og[$x]=array_keys($X);}$Lh=array();$Ve=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT IN","IS NOT NULL");$Xc=array("len","lower","round","upper");$cd=array("avg","count","count distinct","max","min","sum");$ic=array(array("date|time"=>"getdate",),array("int|decimal|real|float|money|datetime"=>"+/-","char|text"=>"+",));}$ac['firebird']='Firebird (alpha)';if(isset($_GET["firebird"])){$Cf=array("interbase");define("DRIVER","firebird");if(extension_loaded("interbase")){class
Min_DB{var$extension="Firebird",$server_info,$affected_rows,$errno,$error,$_link,$_result;function
connect($N=null,$V=null,$G=null,$ja=null,$ka=null,$la=null){$this->_link=ibase_connect($N,$V,$G);if($this->_link){$Oh=explode(':',$N);$this->service_link=ibase_service_attach($Oh[0],$V,$G);$this->server_info=ibase_server_info($this->service_link,IBASE_SVC_SERVER_VERSION);}else{$this->errno=ibase_errcode();$this->error=ibase_errmsg();}return(bool)$this->_link;}function
quote($P){return"'".str_replace("'","''",$P)."'";}function
select_db($Jb){return($Jb=="domain");}function
query($H,$Fh=false){$I=ibase_query($H,$this->_link);if(!$I){$this->errno=ibase_errcode();$this->error=ibase_errmsg();return
...
limit($H,$Z,$z,$D=0,$yg=" "){$J='';$J.=($z!==null?$yg."FIRST $z".($D?" SKIP $D":""):"");$J.=" $H$Z";return$J;}function
limit1($H,$Z){return
limit($H,$Z,1);}function
...
count_tables($l){return
array();}function
table_status($C="",$Hc=false){global$h;$J=array();$Hb=tables_list();foreach($Hb
...
true;}function
support($Ic){return
preg_match("~^(columns|sql|status|table)$~",$Ic);}$w="firebird";$Ve=array("=");$Xc=array();$cd=array();$ic=array();}$ac["simpledb"]="SimpleDB";if(isset($_GET["simpledb"])){$Cf=array("SimpleXML");define("DRIVER","simpledb");if(class_exists('SimpleXMLElement')){class
Min_DB{var$extension="SimpleXML",$server_info='2009-04-15',$error,$timeout,$next,$affected_rows,$_result;function
select_db($Jb){return($Jb=="domain");}function
query($H,$Fh=false){$F=array('SelectExpression'=>$H,'ConsistentRead'=>'true');if($this->next)$F['NextToken']=$this->next;$I=sdb_request_all('Select','Item',$F,$this->timeout);if($I===false)return$I;if(preg_match('~^\s*SELECT\s+COUNT\(~i',$H)){$Sg=0;foreach($I
as$Hd)$Sg+=$Hd->Attribute->Value;$I=array((object)array('Attribute'=>array((object)array('Name'=>'Count','Value'=>$Sg,))));}return
new
Min_Result($I);}function
multi_query($H){return$this->_result=$this->query($H);}function
store_result(){return$this->_result;}function
...
as$La){$C=$this->_processValue($La->Name);$Y=$this->_processValue($La->Value);if(isset($K[$C])){$K[$C]=(array)$K[$C];$K[$C][]=$Y;}else$K[$C]=$Y;}$this->_rows[]=$K;foreach($K
as$x=>$X){if(!isset($this->_rows[0][$x]))$this->_rows[0][$x]=null;}}$this->num_rows=count($this->_rows);}function
_processValue($lc){return(is_object($lc)&&$lc['encoding']=='base64'?base64_decode($lc):(string)$lc);}function
...
fetch_field(){$Nd=array_keys($this->_rows[0]);return(object)array('name'=>$Nd[$this->_offset++]);}}}class
Min_Driver
extends
Min_SQL{public$Ff="itemName()";function
_chunkRequest($od,$ya,$F,$Ac=array()){global$h;foreach(array_chunk($od,25)as$jb){$nf=$F;foreach($jb
as$s=>$md){$nf["Item.$s.ItemName"]=$md;foreach($Ac
as$x=>$X)$nf["Item.$s.$x"]=$X;}if(!sdb_request($ya,$nf))return
false;}$h->affected_rows=count($od);return
true;}function
_extractIds($Q,$Qf,$z){$J=array();if(preg_match_all("~itemName\(\) = (('[^']*+')+)~",$Qf,$he))$J=array_map('idf_unescape',$he[1]);else{foreach(sdb_request_all('Select','Item',array('SelectExpression'=>'SELECT itemName() FROM '.table($Q).$Qf.($z?" LIMIT 1":"")))as$Hd)$J[]=$Hd->Name;}return$J;}function
select($Q,$M,$Z,$ad,$af=array(),$z=1,$E=0,$Hf=false){global$h;$h->next=$_GET["next"];$J=parent::select($Q,$M,$Z,$ad,$af,$z,$E,$Hf);$h->next=0;return$J;}function
delete($Q,$Qf,$z=0){return$this->_chunkRequest($this->_extractIds($Q,$Qf,$z),'BatchDeleteAttributes',array('DomainName'=>$Q));}function
update($Q,$O,$Qf,$z=0,$yg="\n"){$Pb=array();$zd=array();$s=0;$od=$this->_extractIds($Q,$Qf,$z);$md=idf_unescape($O["`itemName()`"]);unset($O["`itemName()`"]);foreach($O
as$x=>$X){$x=idf_unescape($x);if($X=="NULL"||($md!=""&&array($md)!=$od))$Pb["Attribute.".count($Pb).".Name"]=$x;if($X!="NULL"){foreach((array)$X
as$Jd=>$W){$zd["Attribute.$s.Name"]=$x;$zd["Attribute.$s.Value"]=(is_array($X)?$W:idf_unescape($W));if(!$Jd)$zd["Attribute.$s.Replace"]="true";$s++;}}}$F=array('DomainName'=>$Q);return(!$zd||$this->_chunkRequest(($md!=""?array($md):$od),'BatchPutAttributes',$F,$zd))&&(!$Pb||$this->_chunkRequest($od,'BatchDeleteAttributes',$F,$Pb));}function
...
table_status($C="",$Hc=false){$J=array();foreach(($C!=""?array($C=>true):tables_list())as$Q=>$U){$K=array("Name"=>$Q,"Auto_increment"=>"");if(!$Hc){$ue=sdb_request('DomainMetadata',array('DomainName'=>$Q));if($ue){foreach(array("Rows"=>"ItemCount","Data_length"=>"ItemNamesSizeBytes","Index_length"=>"AttributeValuesSizeBytes","Data_free"=>"AttributeNamesSizeBytes",)as$x=>$X)$K[$x]=(string)$ue->$X;}}if($C!="")return$K;$J[$Q]=$K;}return$J;}function
...
idf_escape($t);}function
idf_escape($t){return"`".str_replace("`","``",$t)."`";}function
limit($H,$Z,$z,$D=0,$yg=" "){return" $H$Z".($z!==null?$yg."LIMIT $z":"");}function
unconvert_field($o,$J){return$J;}function
fk_support($R){}function
engines(){return
array();}function
alter_table($Q,$C,$p,$Pc,$ub,$qc,$d,$Oa,$rf){return($Q==""&&sdb_request('CreateDomain',array('DomainName'=>$C)));}function
drop_tables($S){foreach($S
...
last_id(){}function
hmac($Ea,$Hb,$x,$Uf=false){$Xa=64;if(strlen($x)>$Xa)$x=pack("H*",$Ea($x));$x=str_pad($x,$Xa,"\0");$Kd=$x^str_repeat("\x36",$Xa);$Ld=$x^str_repeat("\x5C",$Xa);$J=$Ea($Ld.pack("H*",$Ea($Kd.$Hb)));if($Uf)$J=pack("H*",$J);return$J;}function
sdb_request($ya,$F=array()){global$b,$h;list($kd,$F['AWSAccessKeyId'],$ug)=$b->credentials();$F['Action']=$ya;$F['Timestamp']=gmdate('Y-m-d\TH:i:s+00:00');$F['Version']='2009-04-15';$F['SignatureVersion']=2;$F['SignatureMethod']='HmacSHA1';ksort($F);$H='';foreach($F
as$x=>$X)$H.='&'.rawurlencode($x).'='.rawurlencode($X);$H=str_replace('%7E','~',substr($H,1));$H.="&Signature=".urlencode(base64_encode(hmac('sha1',"POST\n".preg_replace('~^https?://~','',$kd)."\n/\n$H",$ug,true)));@ini_set('track_errors',1);$Kc=@file_get_contents((preg_match('~^https?://~',$kd)?$kd:"http://$kd"),false,stream_context_create(array('http'=>array('method'=>'POST','content'=>$H,'ignore_errors'=>1,))));if(!$Kc){$h->error=$php_errormsg;return
false;}libxml_use_internal_errors(true);$hi=simplexml_load_string($Kc);if(!$hi){$n=libxml_get_last_error();$h->error=$n->message;return
false;}if($hi->Errors){$n=$hi->Errors->Error;$h->error="$n->Message ($n->Code)";return
false;}$h->error='';$dh=$ya."Result";return($hi->$dh?$hi->$dh:true);}function
sdb_request_all($ya,$dh,$F=array(),$mh=0){$J=array();$Kg=($mh?microtime(true):0);$z=(preg_match('~LIMIT\s+(\d+)\s*$~i',$F['SelectExpression'],$B)?$B[1]:0);do{$hi=sdb_request($ya,$F);if(!$hi)break;foreach($hi->$dh
as$lc)$J[]=$lc;if($z&&count($J)>=$z){$_GET["next"]=$hi->NextToken;break;}if($mh&&microtime(true)-$Kg>$mh)return
false;$F['NextToken']=$hi->NextToken;if($z)$F['SelectExpression']=preg_replace('~\d+\s*$~',$z-count($J),$F['SelectExpression']);}while($hi->NextToken);return$J;}$w="simpledb";$Ve=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","IS NOT NULL");$Xc=array();$cd=array("count");$ic=array(array("json"));}$ac["mongo"]="MongoDB (beta)";if(isset($_GET["mongo"])){$Cf=array("mongo");define("DRIVER","mongo");if(class_exists('MongoDB')){class
Min_DB{var$extension="Mongo",$error,$last_id,$_link,$_db;function
connect($N=null,$V=null,$G=null,$ja=null,$ka=null,$la=null){global$b;$m=$b->database();$Ye=array();if($V!=""){$Ye["username"]=$V;$Ye["password"]=$G;}if($m!="")$Ye["db"]=$m;try{$this->_link=@new
MongoClient("mongodb://$N",$Ye);return
true;}catch(Exception$xc){$this->error=$xc->getMessage();return
false;}}function
query($H){return
false;}function
select_db($Jb){try{$this->_db=$this->_link->selectDB($Jb);return
true;}catch(Exception$xc){$this->error=$xc->getMessage();return
false;}}function
...
fetch_row(){$J=$this->fetch_assoc();if(!$J)return$J;return
array_values($J);}function
fetch_field(){$Nd=array_keys($this->_rows[0]);$C=$Nd[$this->_offset++];return(object)array('name'=>$C,'charsetnr'=>$this->_charset[$C],);}}}class
Min_Driver
extends
Min_SQL{public$Ff="_id";function
select($Q,$M,$Z,$ad,$af=array(),$z=1,$E=0,$Hf=false){$M=($M==array("*")?array():array_fill_keys($M,true));$Fg=array();foreach($af
as$X){$X=preg_replace('~ DESC$~','',$X,1,$Db);$Fg[$X]=($Db?-1:1);}return
new
Min_Result($this->_conn->_db->selectCollection($Q)->find(array(),$M)->sort($Fg)->limit(+$z)->skip($E*$z));}function
insert($Q,$O){try{$J=$this->_conn->_db->selectCollection($Q)->insert($O);$this->_conn->errno=$J['code'];$this->_conn->error=$J['err'];$this->_conn->last_id=$O['_id'];return!$J['err'];}catch(Exception$xc){$this->_conn->error=$xc->getMessage();return
false;}}}function
connect(){global$b;$h=new
Min_DB;$k=$b->credentials();if($h->connect($k[0],$k[1],$k[2]))return$h;return$h->error;}function
error(){global$h;return
h($h->error);}function
logged_user(){global$b;$k=$b->credentials();return$k[1];}function
get_databases($Oc){global$h;$J=array();$Mb=$h->_link->listDBs();foreach($Mb['databases']as$m)$J[]=$m['name'];return$J;}function
collations(){return
array();}function
db_collation($m,$qb){}function
count_tables($l){global$h;$J=array();foreach($l
...
table_status($C="",$Hc=false){$J=array();foreach(tables_list()as$Q=>$U){$J[$Q]=array("Name"=>$Q);if($C==$Q)return$J[$Q];}return$J;}function
information_schema(){}function
is_view($R){}function
drop_databases($l){global$h;foreach($l
as$m){$fg=$h->_link->selectDB($m)->drop();if(!$fg['ok'])return
false;}return
true;}function
indexes($Q,$i=null){global$h;$J=array();foreach($h->_db->selectCollection($Q)->getIndexInfo()as$u){$Sb=array();foreach($u["key"]as$e=>$U)$Sb[]=($U==-1?'1':null);$J[$u["name"]]=array("type"=>($u["name"]=="_id_"?"PRIMARY":($u["unique"]?"UNIQUE":"INDEX")),"columns"=>array_keys($u["key"]),"lengths"=>array(),"descs"=>$Sb,);}return$J;}function
fields($Q){return
fields_from_edit();}function
convert_field($o){}function
unconvert_field($o,$J){return$J;}function
...
alter_table($Q,$C,$p,$Pc,$ub,$qc,$d,$Oa,$rf){global$h;if($Q==""){$h->_db->createCollection($C);return
true;}}function
drop_tables($S){global$h;foreach($S
as$Q){$fg=$h->_db->selectCollection($Q)->drop();if(!$fg['ok'])return
false;}return
true;}function
truncate_tables($S){global$h;foreach($S
as$Q){$fg=$h->_db->selectCollection($Q)->remove();if(!$fg['ok'])return
false;}return
true;}function
...
as$e){$e=preg_replace('~ DESC$~','',$e,1,$Db);$f[$e]=($Db?-1:1);}$J=$h->_db->selectCollection($Q)->ensureIndex($f,array("unique"=>($U=="UNIQUE"),"name"=>$C,));}if($J['errmsg']){$h->error=$J['errmsg'];return
false;}}return
true;}function
...
support($Ic){return
preg_match("~database|indexes~",$Ic);}$w="mongo";$Ve=array("=");$Xc=array();$cd=array();$ic=array(array("json"));}$ac["elastic"]="Elasticsearch (beta)";if(isset($_GET["elastic"])){$Cf=array("json");define("DRIVER","elastic");if(function_exists('json_decode')){class
Min_DB{var$extension="JSON",$server_info,$errno,$error,$_url;function
rootQuery($uf,$zb=array(),$ve='GET'){@ini_set('track_errors',1);$Kc=@file_get_contents($this->_url.'/'.ltrim($uf,'/'),false,stream_context_create(array('http'=>array('method'=>$ve,'content'=>json_encode($zb),'ignore_errors'=>1,))));if(!$Kc){$this->error=$php_errormsg;return$Kc;}if(!preg_match('~^HTTP/[0-9.]+ 2~i',$http_response_header[0])){$this->error=$Kc;return
false;}$J=json_decode($Kc,true);if($J===null){$this->errno=json_last_error();if(function_exists('json_last_error_msg'))$this->error=json_last_error_msg();else{$yb=get_defined_constants(true);foreach($yb['json']as$C=>$Y){if($Y==$this->errno&&preg_match('~^JSON_ERROR_~',$C)){$this->error=$C;break;}}}}return$J;}function
query($uf,$zb=array(),$ve='GET'){return$this->rootQuery(($this->_db!=""?"$this->_db/":"/").ltrim($uf,'/'),$zb,$ve);}function
connect($N=null,$V=null,$G=null,$ja=null,$ka=null,$la=null){$this->_url="http://$V:$G@$N/";$J=$this->query('');if($J)$this->server_info=$J['version']['number'];return(bool)$J;}function
select_db($Jb){$this->_db=$Jb;return
true;}function
quote($P){return$P;}}class
...
extends
Min_SQL{function
select($Q,$M,$Z,$ad,$af=array(),$z=1,$E=0,$Hf=false){global$b;$Hb=array();$H="$Q/_search";if($M!=array("*"))$Hb["fields"]=$M;if($af){$Fg=array();foreach($af
as$ob){$ob=preg_replace('~ DESC$~','',$ob,1,$Db);$Fg[]=($Db?array($ob=>"desc"):$ob);}$Hb["sort"]=$Fg;}if($z){$Hb["size"]=+$z;if($E)$Hb["from"]=($E*$z);}foreach($Z
as$X){list($ob,$Te,$X)=explode(" ",$X,3);if($ob=="_id")$Hb["query"]["ids"]["values"][]=$X;elseif($ob.$X!=""){$hh=array("term"=>array(($ob!=""?$ob:"_all")=>$X));if($Te=="=")$Hb["query"]["filtered"]["filter"]["and"][]=$hh;else$Hb["query"]["filtered"]["query"]["bool"]["must"][]=$hh;}}if($Hb["query"]&&!$Hb["query"]["filtered"]["query"]&&!$Hb["query"]["ids"])$Hb["query"]["filtered"]["query"]=array("match_all"=>array());$Kg=microtime(true);$tg=$this->_conn->query($H,$Hb);if($Hf)echo$b->selectQuery("$H: ".print_r($Hb,true),format_time($Kg));if(!$tg)return
false;$J=array();foreach($tg['hits']['hits']as$jd){$K=array();if($M==array("*"))$K["_id"]=$jd["_id"];$p=$jd['_source'];if($M!=array("*")){$p=array();foreach($M
as$x)$p[$x]=$jd['fields'][$x];}foreach($p
as$x=>$X){if($Hb["fields"])$X=$X[0];$K[$x]=(is_array($X)?json_encode($X):$X);}$J[]=$K;}return
new
...
table_status($C="",$Hc=false){global$h;$tg=$h->query("_search?search_type=count",array("facets"=>array("count_by_type"=>array("terms"=>array("field"=>"_type",)))),"POST");$J=array();if($tg){foreach($tg["facets"]["count_by_type"]["terms"]as$Q)$J[$Q["term"]]=array("Name"=>$Q["term"],"Engine"=>"table","Rows"=>$Q["count"],);if($C!=""&&$C==$Q["term"])return$J[$C];}return$J;}function
error(){global$h;return
h($h->error);}function
information_schema(){}function
is_view($R){}function
...
fields($Q){global$h;$I=$h->query("$Q/_mapping");$J=array();if($I){$fe=$I[$Q]['properties'];if(!$fe)$fe=$I[$h->_db]['mappings'][$Q]['properties'];if($fe){foreach($fe
...
as$Q)$J=$J&&$h->query(urlencode($Q),array(),'DELETE');return$J;}$w="elastic";$Ve=array("=","query");$Xc=array();$cd=array();$ic=array(array("json"));}$ac=array("server"=>"MySQL")+$ac;if(!defined("DRIVER")){$Cf=array("MySQLi","MySQL","PDO_MySQL");define("DRIVER","server");if(extension_loaded("mysqli")){class
Min_DB
extends
MySQLi{var$extension="MySQLi";function
__construct(){parent::init();}function
connect($N=null,$V=null,$G=null,$ja=null,$ka=null,$la=null){mysqli_report(MYSQLI_REPORT_OFF);list($kd,$zf)=explode(":",$N,2);$J=@$this->real_connect(($N!=""?$kd:ini_get("mysqli.default_host")),($N.$V!=""?$V:ini_get("mysqli.default_user")),($N.$V.$G!=""?$G:ini_get("mysqli.default_pw")),null,(is_numeric($zf)?$zf:ini_get("mysqli.default_port")),(!is_numeric($zf)?$zf:null));return$J;}function
...
connect($N=null,$V=null,$G=null,$ja=null,$ka=null,$la=null){$this->_link=@mysql_connect(($N!=""?$N:ini_get("mysql.default_host")),("$N$V"!=""?$V:ini_get("mysql.default_user")),("$N$V$G"!=""?$G:ini_get("mysql.default_password")),true,131072);if($this->_link)$this->server_info=mysql_get_server_info($this->_link);else$this->error=mysql_error();return(bool)$this->_link;}function
...
query($H,$Fh=false){$I=@($Fh?mysql_unbuffered_query($H,$this->_link):mysql_query($H,$this->_link));$this->error="";if(!$I){$this->errno=mysql_errno($this->_link);$this->error=mysql_error($this->_link);return
...
connect($N=null,$V=null,$G=null,$ja=null,$ka=null,$la=null){$this->dsn("mysql:charset=utf8;host=".str_replace(":",";unix_socket=",preg_replace('~:(\\d)~',';port=\\1',$N)),$V,$G);return
true;}function
set_charset($db){$this->query("SET NAMES $db");}function
select_db($Jb){return$this->query("USE ".idf_escape($Jb));}function
query($H,$Fh=false){$this->setAttribute(1000,!$Fh);return
parent::query($H,$Fh);}}}class
Min_Driver
extends
Min_SQL{function
insert($Q,$O){return($O?parent::insert($Q,$O):queries("INSERT INTO ".table($Q)." ()\nVALUES ()"));}function
insertUpdate($Q,$L,$Ff){$f=array_keys(reset($L));$Df="INSERT INTO ".table($Q)." (".implode(", ",$f).") VALUES\n";$Wh=array();foreach($f
as$x)$Wh[$x]="$x = VALUES($x)";$Rg="\nON DUPLICATE KEY UPDATE ".implode(", ",$Wh);$Wh=array();$y=0;foreach($L
as$O){$Y="(".implode(", ",$O).")";if($Wh&&(strlen($Df)+$y+strlen($Y)+strlen($Rg)>1e6)){if(!queries($Df.implode(",\n",$Wh).$Rg))return
false;$Wh=array();$y=0;}$Wh[]=$Y;$y+=strlen($Y)+2;}return
queries($Df.implode(",\n",$Wh).$Rg);}}function
idf_escape($t){return"`".str_replace("`","``",$t)."`";}function
table($t){return
idf_escape($t);}function
connect(){global$b;$h=new
Min_DB;$k=$b->credentials();if($h->connect($k[0],$k[1],$k[2])){$h->set_charset(charset($h));$h->query("SET sql_quote_show_create = 1, autocommit = 1");return$h;}$J=$h->error;if(function_exists('iconv')&&!is_utf8($J)&&strlen($pg=iconv("windows-1250","utf-8",$J))>strlen($J))$J=$pg;return$J;}function
get_databases($Oc){global$h;$J=get_session("dbs");if($J===null){$H=($h->server_info>=5?"SELECT SCHEMA_NAME FROM information_schema.SCHEMATA":"SHOW DATABASES");$J=($Oc?slow_query($H):get_vals($H));restart_session();set_session("dbs",$J);stop_session();}return$J;}function
limit($H,$Z,$z,$D=0,$yg=" "){return" $H$Z".($z!==null?$yg."LIMIT $z".($D?" OFFSET $D":""):"");}function
limit1($H,$Z){return
limit($H,$Z,1);}function
db_collation($m,$qb){global$h;$J=null;$j=$h->result("SHOW CREATE DATABASE ".idf_escape($m),1);if(preg_match('~ COLLATE ([^ ]+)~',$j,$B))$J=$B[1];elseif(preg_match('~ CHARACTER SET ([^ ]+)~',$j,$B))$J=$qb[$B[1]][-1];return$J;}function
...
table_status($C="",$Hc=false){global$h;$J=array();foreach(get_rows($Hc&&$h->server_info>=5?"SELECT TABLE_NAME AS Name, Engine, TABLE_COMMENT AS Comment FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() ".($C!=""?"AND TABLE_NAME = ".q($C):"ORDER BY Name"):"SHOW TABLE STATUS".($C!=""?" LIKE ".q(addcslashes($C,"%_\\")):""))as$K){if($K["Engine"]=="InnoDB")$K["Comment"]=preg_replace('~(?:(.+); )?InnoDB free: .*~','\\1',$K["Comment"]);if(!isset($K["Engine"]))$K["Comment"]="";if($C!="")return$K;$J[$K["Name"]]=$K;}return$J;}function
...
foreign_keys($Q){global$h,$Qe;static$wf='`(?:[^`]|``)+`';$J=array();$Eb=$h->result("SHOW CREATE TABLE ".table($Q),1);if($Eb){preg_match_all("~CONSTRAINT ($wf) FOREIGN KEY ?\\(((?:$wf,? ?)+)\\) REFERENCES ($wf)(?:\\.($wf))? \\(((?:$wf,? ?)+)\\)(?: ON DELETE ($Qe))?(?: ON UPDATE ($Qe))?~",$Eb,$he,PREG_SET_ORDER);foreach($he
as$B){preg_match_all("~$wf~",$B[2],$Gg);preg_match_all("~$wf~",$B[5],$eh);$J[idf_unescape($B[1])]=array("db"=>idf_unescape($B[4]!=""?$B[3]:$B[4]),"table"=>idf_unescape($B[4]!=""?$B[4]:$B[3]),"source"=>array_map('idf_unescape',$Gg[0]),"target"=>array_map('idf_unescape',$eh[0]),"on_delete"=>($B[6]?$B[6]:"RESTRICT"),"on_update"=>($B[7]?$B[7]:"RESTRICT"),);}}return$J;}function
view($C){global$h;return
...
error_line(){global$h;if(preg_match('~ at line ([0-9]+)$~',$h->error,$bg))return$bg[1]-1;}function
create_database($m,$d){return
queries("CREATE DATABASE ".idf_escape($m).($d?" COLLATE ".q($d):""));}function
drop_databases($l){$J=apply_queries("DROP DATABASE",$l,'idf_escape');restart_session();set_session("dbs",null);return$J;}function
rename_database($C,$d){$J=false;if(create_database($C,$d)){$dg=array();foreach(tables_list()as$Q=>$U)$dg[]=table($Q)." TO ".idf_escape($C).".".table($Q);$J=(!$dg||queries("RENAME TABLE ".implode(", ",$dg)));if($J)queries("DROP DATABASE ".idf_escape(DB));restart_session();set_session("dbs",null);}return$J;}function
auto_increment(){$Pa=" PRIMARY KEY";if($_GET["create"]!=""&&$_POST["auto_increment_col"]){foreach(indexes($_GET["create"])as$u){if(in_array($_POST["fields"][$_POST["auto_increment_col"]]["orig"],$u["columns"],true)){$Pa="";break;}if($u["type"]=="PRIMARY")$Pa=" UNIQUE";}}return" AUTO_INCREMENT$Pa";}function
alter_table($Q,$C,$p,$Pc,$ub,$qc,$d,$Oa,$rf){$c=array();foreach($p
as$o)$c[]=($o[1]?($Q!=""?($o[0]!=""?"CHANGE ".idf_escape($o[0]):"ADD"):" ")." ".implode($o[1]).($Q!=""?$o[2]:""):"DROP ".idf_escape($o[0]));$c=array_merge($c,$Pc);$Lg=($ub!==null?" COMMENT=".q($ub):"").($qc?" ENGINE=".q($qc):"").($d?" COLLATE ".q($d):"").($Oa!=""?" AUTO_INCREMENT=$Oa":"");if($Q=="")return
queries("CREATE TABLE ".table($C)." (\n".implode(",\n",$c)."\n)$Lg$rf");if($Q!=$C)$c[]="RENAME TO ".table($C);if($Lg)$c[]=ltrim($Lg);return($c||$rf?queries("ALTER TABLE ".table($Q)."\n".implode(",\n",$c).$rf):true);}function
alter_indexes($Q,$c){foreach($c
...
drop_tables($S){return
queries("DROP TABLE ".implode(", ",array_map('table',$S)));}function
move_tables($S,$bi,$eh){$dg=array();foreach(array_merge($S,$bi)as$Q)$dg[]=table($Q)." TO ".idf_escape($eh).".".table($Q);return
queries("RENAME TABLE ".implode(", ",$dg));}function
copy_tables($S,$bi,$eh){queries("SET sql_mode = 'NO_AUTO_VALUE_ON_ZERO'");foreach($S
as$Q){$C=($eh==DB?table("copy_$Q"):idf_escape($eh).".".table($Q));if(!queries("\nDROP TABLE IF EXISTS $C")||!queries("CREATE TABLE $C LIKE ".table($Q))||!queries("INSERT INTO $C SELECT * FROM ".table($Q)))return
false;}foreach($bi
as$Q){$C=($eh==DB?table("copy_$Q"):idf_escape($eh).".".table($Q));$ai=view($Q);if(!queries("DROP VIEW IF EXISTS $C")||!queries("CREATE VIEW $C AS $ai[select]"))return
false;}return
true;}function
trigger($C){if($C=="")return
...
routine($C,$U){global$h,$sc,$xd,$Eh;$Fa=array("bool","boolean","integer","double precision","real","dec","numeric","fixed","national char","national varchar");$Dh="((".implode("|",array_merge(array_keys($Eh),$Fa)).")\\b(?:\\s*\\(((?:[^'\")]|$sc)++)\\))?\\s*(zerofill\\s*)?(unsigned(?:\\s+zerofill)?)?)(?:\\s*(?:CHARSET|CHARACTER\\s+SET)\\s*['\"]?([^'\"\\s,]+)['\"]?)?";$wf="\\s*(".($U=="FUNCTION"?"":$xd).")?\\s*(?:`((?:[^`]|``)*)`\\s*|\\b(\\S+)\\s+)$Dh";$j=$h->result("SHOW CREATE $U ".idf_escape($C),2);preg_match("~\\(((?:$wf\\s*,?)*)\\)\\s*".($U=="FUNCTION"?"RETURNS\\s+$Dh\\s+":"")."(.*)~is",$j,$B);$p=array();preg_match_all("~$wf\\s*,?~is",$B[1],$he,PREG_SET_ORDER);foreach($he
as$mf){$C=str_replace("``","`",$mf[2]).$mf[3];$p[]=array("field"=>$C,"type"=>strtolower($mf[5]),"length"=>preg_replace_callback("~$sc~s",'normalize_enum',$mf[6]),"unsigned"=>strtolower(preg_replace('~\\s+~',' ',trim("$mf[8] $mf[7]"))),"null"=>1,"full_type"=>$mf[4],"inout"=>strtoupper($mf[1]),"collation"=>strtolower($mf[9]),);}if($U!="FUNCTION")return
...
set_schema($rg){return
true;}function
create_sql($Q,$Oa){global$h;$J=$h->result("SHOW CREATE TABLE ".table($Q),1);if(!$Oa)$J=preg_replace('~ AUTO_INCREMENT=\\d+~','',$J);return$J;}function
...
support($Ic){global$h;return!preg_match("~scheme|sequence|type|view_trigger".($h->server_info<5.1?"|event|partitioning".($h->server_info<5?"|routine|trigger|view":""):"")."~",$Ic);}$w="sql";$Eh=array();$Og=array();foreach(array(lang(24)=>array("tinyint"=>3,"smallint"=>5,"mediumint"=>8,"int"=>10,"bigint"=>20,"decimal"=>66,"float"=>12,"double"=>21),lang(25)=>array("date"=>10,"datetime"=>19,"timestamp"=>19,"time"=>10,"year"=>4),lang(26)=>array("char"=>255,"varchar"=>65535,"tinytext"=>255,"text"=>65535,"mediumtext"=>16777215,"longtext"=>4294967295),lang(30)=>array("enum"=>65535,"set"=>64),lang(27)=>array("bit"=>20,"binary"=>255,"varbinary"=>65535,"tinyblob"=>255,"blob"=>65535,"mediumblob"=>16777215,"longblob"=>4294967295),lang(29)=>array("geometry"=>0,"point"=>0,"linestring"=>0,"polygon"=>0,"multipoint"=>0,"multilinestring"=>0,"multipolygon"=>0,"geometrycollection"=>0),)as$x=>$X){$Eh+=$X;$Og[$x]=array_keys($X);}$Lh=array("unsigned","zerofill","unsigned zerofill");$Ve=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","REGEXP","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","SQL");$Xc=array("char_length","date","from_unixtime","lower","round","sec_to_time","time_to_sec","upper");$cd=array("avg","count","count distinct","group_concat","max","min","sum");$ic=array(array("char"=>"md5/sha1/password/encrypt/uuid","binary"=>"md5/sha1","date|time"=>"now",),array("(^|[^o])int|float|double|decimal"=>"+/-","date"=>"+ interval/- interval","time"=>"addtime/subtime","char|text"=>"concat",));}define("SERVER",$_GET[DRIVER]);define("DB",$_GET["db"]);define("ME",preg_replace('~^[^?]*/([^?]*).*~','\\1',$_SERVER["REQUEST_URI"]).'?'.(sid()?SID.'&':'').(SERVER!==null?DRIVER."=".urlencode(SERVER).'&':'').(isset($_GET["username"])?"username=".urlencode($_GET["username"]).'&':'').(DB!=""?'db='.urlencode(DB).'&'.(isset($_GET["ns"])?"ns=".urlencode($_GET["ns"])."&":""):''));$ia="4.2.3";class
...
head(){return
true;}function
loginForm(){global$ac;echo'<table cellspacing="0">
<tr><th>',lang(31),'<td>',html_select("auth[driver]",$ac,DRIVER,"loginDriver(this);"),'<tr><th>',lang(32),'<td><input name="auth[server]" value="',h(SERVER),'" title="hostname[:port]" placeholder="localhost" autocapitalize="off">
...
selectLinks($Vg,$O=""){echo'<p class="links">';$ce=array("select"=>lang(38));if(support("table")||support("indexes"))$ce["table"]=lang(39);if(support("table")){if(is_view($Vg))$ce["view"]=lang(40);else$ce["create"]=lang(41);}if($O!==null)$ce["edit"]=lang(42);foreach($ce
as$x=>$X)echo" <a href='".h(ME)."$x=".urlencode($Vg["Name"]).($x=="edit"?$O:"")."'".bold(isset($_GET[$x])).">$X</a>";echo"\n";}function
foreignKeys($Q){return
foreign_keys($Q);}function
backwardKeys($Q,$Ug){return
array();}function
backwardKeysPrint($Ra,$K){}function
selectQuery($H,$lh){global$w;return"<p><code class='jush-$w'>".h(str_replace("\n"," ",$H))."</code> <span class='time'>($lh)</span>".(support("sql")?" <a href='".h(ME)."sql=".urlencode($H)."'>".lang(10)."</a>":"")."</p>";}function
rowDescription($Q){return"";}function
rowDescriptions($L,$Qc){return$L;}function
selectLink($X,$o){}function
selectVal($X,$_,$o,$hf){$J=($X===null?"<i>NULL</i>":(preg_match("~char|binary~",$o["type"])&&!preg_match("~var~",$o["type"])?"<code>$X</code>":$X));if(preg_match('~blob|bytea|raw|file~',$o["type"])&&!is_utf8($X))$J=lang(43,strlen($hf));return($_?"<a href='".h($_)."'".(is_url($_)?" rel='noreferrer'":"").">$J</a>":$J);}function
editVal($X,$o){return$X;}function
selectColumnsPrint($M,$f){global$Xc,$cd;print_fieldset("select",lang(44),$M);$s=0;$M[""]=array();foreach($M
as$x=>$X){$X=$_GET["columns"][$x];$e=select_input(" name='columns[$s][col]' onchange='".($x!==""?"selectFieldChange(this.form)":"selectAddRow(this)").";'",$f,$X["col"]);echo"<div>".($Xc||$cd?"<select name='columns[$s][fun]' onchange='helpClose();".($x!==""?"":" this.nextSibling.nextSibling.onchange();")."'".on_help("getTarget(event).value && getTarget(event).value.replace(/ |\$/, '(') + ')'",1).">".optionlist(array(-1=>"")+array_filter(array(lang(45)=>$Xc,lang(46)=>$cd)),$X["fun"])."</select>"."($e)":$e)."</div>\n";$s++;}echo"</div></fieldset>\n";}function
selectSearchPrint($Z,$f,$v){print_fieldset("search",lang(47),$Z);foreach($v
as$s=>$u){if($u["type"]=="FULLTEXT"){echo"(<i>".implode("</i>, <i>",array_map('h',$u["columns"]))."</i>) AGAINST"," <input type='search' name='fulltext[$s]' value='".h($_GET["fulltext"][$s])."' onchange='selectFieldChange(this.form);'>",checkbox("boolean[$s]",1,isset($_GET["boolean"][$s]),"BOOL"),"<br>\n";}}$_GET["where"]=(array)$_GET["where"];reset($_GET["where"]);$cb="this.nextSibling.onchange();";for($s=0;$s<=count($_GET["where"]);$s++){list(,$X)=each($_GET["where"]);if(!$X||("$X[col]$X[val]"!=""&&in_array($X["op"],$this->operators))){echo"<div>".select_input(" name='where[$s][col]' onchange='$cb'",$f,$X["col"],"(".lang(48).")"),html_select("where[$s][op]",$this->operators,$X["op"],$cb),"<input type='search' name='where[$s][val]' value='".h($X["val"])."' onchange='".($X?"selectFieldChange(this.form)":"selectAddRow(this)").";' onkeydown='selectSearchKeydown(this, event);' onsearch='selectSearchSearch(this);'></div>\n";}}echo"</div></fieldset>\n";}function
selectOrderPrint($af,$f,$v){print_fieldset("sort",lang(49),$af);$s=0;foreach((array)$_GET["order"]as$x=>$X){if($X!=""){echo"<div>".select_input(" name='order[$s]' onchange='selectFieldChange(this.form);'",$f,$X),checkbox("desc[$s]",1,isset($_GET["desc"][$x]),lang(50))."</div>\n";$s++;}}echo"<div>".select_input(" name='order[$s]' onchange='selectAddRow(this);'",$f),checkbox("desc[$s]",1,false,lang(50))."</div>\n","</div></fieldset>\n";}function
...
selectLengthPrint($kh){if($kh!==null){echo"<fieldset><legend>".lang(52)."</legend><div>","<input type='number' name='text_length' class='size' value='".h($kh)."'>","</div></fieldset>\n";}}function
...
selectEmailPrint($nc,$f){}function
selectColumnsProcess($f,$v){global$Xc,$cd;$M=array();$ad=array();foreach((array)$_GET["columns"]as$x=>$X){if($X["fun"]=="count"||($X["col"]!=""&&(!$X["fun"]||in_array($X["fun"],$Xc)||in_array($X["fun"],$cd)))){$M[$x]=apply_sql_function($X["fun"],($X["col"]!=""?idf_escape($X["col"]):"*"));if(!in_array($X["fun"],$cd))$ad[]=$M[$x];}}return
array($M,$ad);}function
selectSearchProcess($p,$v){global$h,$w;$J=array();foreach($v
as$s=>$u){if($u["type"]=="FULLTEXT"&&$_GET["fulltext"][$s]!="")$J[]="MATCH (".implode(", ",array_map('idf_escape',$u["columns"])).") AGAINST (".q($_GET["fulltext"][$s]).(isset($_GET["boolean"][$s])?" IN BOOLEAN MODE":"").")";}foreach((array)$_GET["where"]as$X){if("$X[col]$X[val]"!=""&&in_array($X["op"],$this->operators)){$wb=" $X[op]";if(preg_match('~IN$~',$X["op"])){$qd=process_length($X["val"]);$wb.=" ".($qd!=""?$qd:"(NULL)");}elseif($X["op"]=="SQL")$wb=" $X[val]";elseif($X["op"]=="LIKE %%")$wb=" LIKE ".$this->processInput($p[$X["col"]],"%$X[val]%");elseif($X["op"]=="ILIKE %%")$wb=" ILIKE ".$this->processInput($p[$X["col"]],"%$X[val]%");elseif(!preg_match('~NULL$~',$X["op"]))$wb.=" ".$this->processInput($p[$X["col"]],$X["val"]);if($X["col"]!="")$J[]=idf_escape($X["col"]).$wb;else{$rb=array();foreach($p
as$C=>$o){$Fd=preg_match('~char|text|enum|set~',$o["type"]);if((is_numeric($X["val"])||!preg_match('~(^|[^o])int|float|double|decimal|bit~',$o["type"]))&&(!preg_match("~[\x80-\xFF]~",$X["val"])||$Fd)){$C=idf_escape($C);$rb[]=($w=="sql"&&$Fd&&!preg_match("~^utf8_~",$o["collation"])?"CONVERT($C USING ".charset($h).")":$C);}}$J[]=($rb?"(".implode("$wb OR ",$rb)."$wb)":"0");}}}return$J;}function
...
messageQuery($H,$lh){global$w;restart_session();$hd=&get_session("queries");$md="sql-".count($hd[$_GET["db"]]);if(strlen($H)>1e6)$H=preg_replace('~[\x80-\xFF]+$~','',substr($H,0,1e6))."\n...";$hd[$_GET["db"]][]=array($H,time(),$lh);return" <span class='time'>".@date("H:i:s")."</span> <a href='#$md' onclick=\"return !toggle('$md');\">".lang(55)."</a>"."<div id='$md' class='hidden'><pre><code class='jush-$w'>".shorten_utf8($H,1000).'</code></pre>'.($lh?" <span class='time'>($lh)</span>":'').(support("sql")?'<p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($hd[$_GET["db"]])-1)).'">'.lang(10).'</a>':'').'</div>';}function
editFunctions($o){global$ic;$J=($o["null"]?"NULL/":"");foreach($ic
as$x=>$Xc){if(!$x||(!isset($_GET["call"])&&(isset($_GET["select"])||where($_GET)))){foreach($Xc
as$wf=>$X){if(!$wf||preg_match("~$wf~",$o["type"]))$J.="/$X";}if($x&&!preg_match('~set|blob|bytea|raw|file~',$o["type"]))$J.="/SQL";}}if($o["auto_increment"]&&!isset($_GET["select"])&&!where($_GET))$J=lang(56);return
explode("/",$J);}function
editInput($Q,$o,$Ma,$Y){if($o["type"]=="enum")return(isset($_GET["select"])?"<label><input type='radio'$Ma value='-1' checked><i>".lang(8)."</i></label> ":"").($o["null"]?"<label><input type='radio'$Ma value=''".($Y!==null||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"").enum_input("radio",$Ma,$o,$Y,0);return"";}function
...
dumpTable($Q,$Pg,$Gd=0){if($_POST["format"]!="sql"){echo"\xef\xbb\xbf";if($Pg)dump_csv(array_keys(fields($Q)));}else{if($Gd==2){$p=array();foreach(fields($Q)as$C=>$o)$p[]=idf_escape($C)." $o[full_type]";$j="CREATE TABLE ".table($Q)." (".implode(", ",$p).")";}else$j=create_sql($Q,$_POST["auto_increment"]);set_utf8mb4($j);if($Pg&&$j){if($Pg=="DROP+CREATE"||$Gd==1)echo"DROP ".($Gd==2?"VIEW":"TABLE")." IF EXISTS ".table($Q).";\n";if($Gd==1)$j=remove_definer($j);echo"$j;\n\n";}}}function
dumpData($Q,$Pg,$H){global$h,$w;$je=($w=="sqlite"?0:1048576);if($Pg){if($_POST["format"]=="sql"){if($Pg=="TRUNCATE+INSERT")echo
truncate_sql($Q).";\n";$p=fields($Q);}$I=$h->query($H,1);if($I){$zd="";$ab="";$Nd=array();$Rg="";$Jc=($Q!=''?'fetch_assoc':'fetch_row');while($K=$I->$Jc()){if(!$Nd){$Wh=array();foreach($K
as$X){$o=$I->fetch_field();$Nd[]=$o->name;$x=idf_escape($o->name);$Wh[]="$x = VALUES($x)";}$Rg=($Pg=="INSERT+UPDATE"?"\nON DUPLICATE KEY UPDATE ".implode(", ",$Wh):"").";\n";}if($_POST["format"]!="sql"){if($Pg=="table"){dump_csv($Nd);$Pg="INSERT";}dump_csv($K);}else{if(!$zd)$zd="INSERT INTO ".table($Q)." (".implode(", ",array_map('idf_escape',$Nd)).") VALUES";foreach($K
as$x=>$X){$o=$p[$x];$K[$x]=($X!==null?unconvert_field($o,preg_match('~(^|[^o])int|float|double|decimal~',$o["type"])&&$X!=''?$X:q($X)):"NULL");}$pg=($je?"\n":" ")."(".implode(",\t",$K).")";if(!$ab)$ab=$zd.$pg;elseif(strlen($ab)+4+strlen($pg)+strlen($Rg)<$je)$ab.=",$pg";else{echo$ab.$Rg;$ab=$zd.$pg;}}}if($ab)echo$ab.$Rg;}elseif($_POST["format"]=="sql")echo"-- ".str_replace("\n"," ",$h->error)."\n";}}function
dumpFilename($nd){return
friendly_url($nd!=""?$nd:(SERVER!=""?SERVER:"localhost"));}function
dumpHeaders($nd,$ye=false){$kf=$_POST["output"];$Dc=(preg_match('~sql~',$_POST["format"])?"sql":($ye?"tar":"csv"));header("Content-Type: ".($kf=="gz"?"application/x-gzip":($Dc=="tar"?"application/x-tar":($Dc=="sql"||$kf!="file"?"text/plain":"text/csv")."; charset=utf-8")));if($kf=="gz")ob_start('ob_gzencode',1e6);return$Dc;}function
...
true;}function
navigation($xe){global$ia,$w,$ac,$h;echo'<h1>
...
';if($xe=="auth"){$Nc=true;foreach((array)$_SESSION["pwds"]as$Yh=>$Bg){foreach($Bg
as$N=>$Th){foreach($Th
as$V=>$G){if($G!==null){if($Nc){echo"<p id='logins' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";$Nc=false;}$Mb=$_SESSION["db"][$Yh][$N][$V];foreach(($Mb?array_keys($Mb):array(""))as$m)echo"<a href='".h(auth_url($Yh,$N,$V,$m))."'>($ac[$Yh]) ".h($V.($N!=""?"@$N":"").($m!=""?" - $m":""))."</a><br>\n";}}}}}else{if($_GET["ns"]!==""&&!$xe&&DB!=""){$h->select_db(DB);$S=table_status('',true);}if(support("sql")){echo'<script type="text/javascript" src="',h(preg_replace("~\\?.*~","",ME))."?file=jush.js&amp;version=4.2.3",'"></script>
<script type="text/javascript">
';if($S){$ce=array();foreach($S
as$Q=>$U)$ce[]=preg_quote($Q,'/');echo"var jushLinks = { $w: [ '".js_escape(ME).(support("table")?"table=":"select=")."\$&', /\\b(".implode("|",$ce).")\\b/g ] };\n";foreach(array("bac","bra","sqlite_quo","mssql_bra")as$X)echo"jushLinks.$X = jushLinks.$w;\n";}echo'bodyLoad(\'',(is_object($h)?substr($h->server_info,0,3):""),'\');
</script>
';}$this->databasesPrint($xe);if(DB==""||!$xe){echo"<p class='links'>".(support("sql")?"<a href='".h(ME)."sql='".bold(isset($_GET["sql"])&&!isset($_GET["import"])).">".lang(55)."</a>\n<a href='".h(ME)."import='".bold(isset($_GET["import"])).">".lang(64)."</a>\n":"")."";if(support("dump"))echo"<a href='".h(ME)."dump=".urlencode(isset($_GET["table"])?$_GET["table"]:$_GET["select"])."' id='dump'".bold(isset($_GET["dump"])).">".lang(65)."</a>\n";}if($_GET["ns"]!==""&&!$xe&&DB!=""){echo'<a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".lang(66)."</a>\n";if(!$S)echo"<p class='message'>".lang(9)."\n";else$this->tablesPrint($S);}}}function
databasesPrint($xe){global$b,$h;$l=$this->databases();echo'<form action="">
<p id="dbs">
';hidden_fields_get();$Kb=" onmousedown='dbMouseDown(event, this);' onchange='dbChange(this);'";echo"<span title='".lang(67)."'>DB</span>: ".($l?"<select name='db'$Kb>".optionlist(array(""=>"")+$l,DB)."</select>":'<input name="db" value="'.h(DB).'" autocapitalize="off">'),"<input type='submit' value='".lang(20)."'".($l?" class='hidden'":"").">\n";if($xe!="db"&&DB!=""&&$h->select_db(DB)){if(support("scheme")){echo"<br>".lang(68).": <select name='ns'$Kb>".optionlist(array(""=>"")+$b->schemas(),$_GET["ns"])."</select>";if($_GET["ns"]!="")set_schema($_GET["ns"]);}}echo(isset($_GET["sql"])?'<input type="hidden" name="sql" value="">':(isset($_GET["schema"])?'<input type="hidden" name="schema" value="">':(isset($_GET["dump"])?'<input type="hidden" name="dump" value="">':(isset($_GET["privileges"])?'<input type="hidden" name="privileges" value="">':"")))),"</p></form>\n";}function
tablesPrint($S){echo"<p id='tables' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";foreach($S
as$Q=>$Lg){echo'<a href="'.h(ME).'select='.urlencode($Q).'"'.bold($_GET["select"]==$Q||$_GET["edit"]==$Q,"select").">".lang(69)."</a> ";$C=$this->tableName($Lg);echo(support("table")||support("indexes")?'<a href="'.h(ME).'table='.urlencode($Q).'"'.bold(in_array($Q,array($_GET["table"],$_GET["create"],$_GET["indexes"],$_GET["foreign"],$_GET["trigger"])),(is_view($Lg)?"view":""),"structure")." title='".lang(39)."'>$C</a>":"<span>$C</span>")."<br>\n";}}}$b=(function_exists('adminer_object')?adminer_object():new
Adminer);if($b->operators===null)$b->operators=$Ve;function
page_header($oh,$n="",$Za=array(),$ph=""){global$ca,$ia,$b,$ac,$w;page_headers();if(is_ajax()&&$n){page_messages($n);exit;}$qh=$oh.($ph!=""?": $ph":"");$rh=strip_tags($qh.(SERVER!=""&&SERVER!="localhost"?h(" - ".SERVER):"")." - ".$b->name());echo'<!DOCTYPE html>
...
<div id="content">
';if($Za!==null){$_=substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1);echo'<p id="breadcrumb"><a href="'.h($_?$_:".").'">'.$ac[DRIVER].'</a> &raquo; ';$_=substr(preg_replace('~\b(db|ns)=[^&]*&~','',ME),0,-1);$N=(SERVER!=""?h(SERVER):lang(32));if($Za===false)echo"$N\n";else{echo"<a href='".($_?h($_):".")."' accesskey='1' title='Alt+Shift+1'>$N</a> &raquo; ";if($_GET["ns"]!=""||(DB!=""&&is_array($Za)))echo'<a href="'.h($_."&db=".urlencode(DB).(support("scheme")?"&ns=":"")).'">'.h(DB).'</a> &raquo; ';if(is_array($Za)){if($_GET["ns"]!="")echo'<a href="'.h(substr(ME,0,-1)).'">'.h($_GET["ns"]).'</a> &raquo; ';foreach($Za
as$x=>$X){$Rb=(is_array($X)?$X[1]:h($X));if($Rb!="")echo"<a href='".h(ME."$x=").urlencode(is_array($X)?$X[0]:$X)."'>$Rb</a> &raquo; ";}}echo"$oh\n";}}echo"<h2>$qh</h2>\n","<div id='ajaxstatus' class='jsonly hidden'></div>\n";restart_session();page_messages($n);$l=&get_session("dbs");if(DB!=""&&$l&&!in_array(DB,$l,true))$l=null;stop_session();define("PAGE_HEADER",1);}function
page_headers(){global$b;header("Content-Type: text/html; charset=utf-8");header("Cache-Control: no-cache");if($b->headers()){header("X-Frame-Options: deny");header("X-XSS-Protection: 0");}}function
page_messages($n){$Nh=preg_replace('~^[^?]*~','',$_SERVER["REQUEST_URI"]);$te=$_SESSION["messages"][$Nh];if($te){echo"<div class='message'>".implode("</div>\n<div class='message'>",$te)."</div>\n";unset($_SESSION["messages"][$Nh]);}if($n)echo"<div class='error'>$n</div>\n";}function
...
long2str($W,$di){$pg='';foreach($W
as$X)$pg.=pack('V',$X);if($di)return
substr($pg,0,end($W));return$pg;}function
str2long($pg,$di){$W=array_values(unpack('V*',str_pad($pg,4*ceil(strlen($pg)/4),"\0")));if($di)$W[]=strlen($pg);return$W;}function
xxtea_mx($ji,$ii,$Sg,$Jd){return
int32((($ji>>5&0x7FFFFFF)^$ii<<2)+(($ii>>3&0x1FFFFFFF)^$ji<<4))^int32(($Sg^$ii)+($Jd^$ji));}function
encrypt_string($Ng,$x){if($Ng=="")return"";$x=array_values(unpack("V*",pack("H*",md5($x))));$W=str2long($Ng,true);$_e=count($W)-1;$ji=$W[$_e];$ii=$W[0];$Of=floor(6+52/($_e+1));$Sg=0;while($Of-->0){$Sg=int32($Sg+0x9E3779B9);$hc=$Sg>>2&3;for($lf=0;$lf<$_e;$lf++){$ii=$W[$lf+1];$ze=xxtea_mx($ji,$ii,$Sg,$x[$lf&3^$hc]);$ji=int32($W[$lf]+$ze);$W[$lf]=$ji;}$ii=$W[0];$ze=xxtea_mx($ji,$ii,$Sg,$x[$lf&3^$hc]);$ji=int32($W[$_e]+$ze);$W[$_e]=$ji;}return
long2str($W,false);}function
decrypt_string($Ng,$x){if($Ng=="")return"";if(!$x)return
false;$x=array_values(unpack("V*",pack("H*",md5($x))));$W=str2long($Ng,false);$_e=count($W)-1;$ji=$W[$_e];$ii=$W[0];$Of=floor(6+52/($_e+1));$Sg=int32($Of*0x9E3779B9);while($Sg){$hc=$Sg>>2&3;for($lf=$_e;$lf>0;$lf--){$ji=$W[$lf-1];$ze=xxtea_mx($ji,$ii,$Sg,$x[$lf&3^$hc]);$ii=int32($W[$lf]-$ze);$W[$lf]=$ii;}$ji=$W[$_e];$ze=xxtea_mx($ji,$ii,$Sg,$x[$lf&3^$hc]);$ii=int32($W[0]-$ze);$W[0]=$ii;$Sg=int32($Sg-0x9E3779B9);}return
long2str($W,true);}$h='';$gd=$_SESSION["token"];if(!$gd)$_SESSION["token"]=rand(1,1e6);$T=get_token();$xf=array();if($_COOKIE["adminer_permanent"]){foreach(explode(" ",$_COOKIE["adminer_permanent"])as$X){list($x)=explode(":",$X);$xf[$x]=$X;}}function
add_invalid_login(){global$b;$Lc=get_temp_dir()."/adminer.invalid";$Vc=@fopen($Lc,"r+");if(!$Vc){$Vc=@fopen($Lc,"w");if(!$Vc)return;}flock($Vc,LOCK_EX);$Bd=unserialize(stream_get_contents($Vc));$lh=time();if($Bd){foreach($Bd
as$Cd=>$X){if($X[0]<$lh)unset($Bd[$Cd]);}}$Ad=&$Bd[$b->bruteForceKey()];if(!$Ad)$Ad=array($lh+30*60,0);$Ad[1]++;$_g=serialize($Bd);rewind($Vc);fwrite($Vc,$_g);ftruncate($Vc,strlen($_g));flock($Vc,LOCK_UN);fclose($Vc);}$Na=$_POST["auth"];if($Na){$Bd=unserialize(@file_get_contents(get_temp_dir()."/adminer.invalid"));$Ad=$Bd[$b->bruteForceKey()];$Fe=($Ad[1]>30?$Ad[0]-time():0);if($Fe>0)auth_error(lang(73,ceil($Fe/60)));session_regenerate_id();$Yh=$Na["driver"];$N=$Na["server"];$V=$Na["username"];$G=(string)$Na["password"];$m=$Na["db"];set_password($Yh,$N,$V,$G);$_SESSION["db"][$Yh][$N][$V][$m]=true;if($Na["permanent"]){$x=base64_encode($Yh)."-".base64_encode($N)."-".base64_encode($V)."-".base64_encode($m);$If=$b->permanentLogin(true);$xf[$x]="$x:".base64_encode($If?encrypt_string($G,$If):"");cookie("adminer_permanent",implode(" ",$xf));}if(count($_POST)==1||DRIVER!=$Yh||SERVER!=$N||$_GET["username"]!==$V||DB!=$m)redirect(auth_url($Yh,$N,$V,$m));}elseif($_POST["logout"]){if($gd&&!verify_token()){page_header(lang(72),lang(74));page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$x)set_session($x,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),lang(75));}}elseif($xf&&!$_SESSION["pwds"]){session_regenerate_id();$If=$b->permanentLogin();foreach($xf
as$x=>$X){list(,$kb)=explode(":",$X);list($Yh,$N,$V,$m)=array_map('base64_decode',explode("-",$x));set_password($Yh,$N,$V,decrypt_string(base64_decode($kb),$If));$_SESSION["db"][$Yh][$N][$V][$m]=true;}}function
unset_permanent(){global$xf;foreach($xf
as$x=>$X){list($Yh,$N,$V,$m)=array_map('base64_decode',explode("-",$x));if($Yh==DRIVER&&$N==SERVER&&$V==$_GET["username"]&&$m==DB)unset($xf[$x]);}cookie("adminer_permanent",implode(" ",$xf));}function
auth_error($n){global$b,$gd;$n=h($n);$Cg=session_name();if(isset($_GET["username"])){header("HTTP/1.1 403 Forbidden");if(($_COOKIE[$Cg]||$_GET[$Cg])&&!$gd)$n=lang(76);else{add_invalid_login();$G=get_password();if($G!==null){if($G===false)$n.='<br>'.lang(77,'<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}if(!$_COOKIE[$Cg]&&$_GET[$Cg]&&ini_bool("session.use_only_cookies"))$n=lang(78);$F=session_get_cookie_params();cookie("adminer_key",($_COOKIE["adminer_key"]?$_COOKIE["adminer_key"]:rand_string()),$F["lifetime"]);page_header(lang(36),$n,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");exit;}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header(lang(79),lang(80,implode(", ",$Cf)),false);page_footer("auth");exit;}$h=connect();}$Zb=new
Min_Driver($h);if(!is_object($h)||!$b->login($_GET["username"],get_password()))auth_error((is_string($h)?$h:lang(81)));if($Na&&$_POST["token"])$_POST["token"]=$T;$n='';if($_POST){if(!verify_token()){$wd="max_input_vars";$ne=ini_get($wd);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$x){$X=ini_get($x);if($X&&(!$ne||$X<$ne)){$wd=$x;$ne=$X;}}}$n=(!$_POST["token"]&&$ne?lang(82,"'$wd'"):lang(74).' '.lang(83));}}elseif($_SERVER["REQUEST_METHOD"]=="POST"){$n=lang(84,"'post_max_size'");if(isset($_GET["sql"]))$n.=' '.lang(85);}if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false)session_write_close();function
select($I,$i=null,$df=array(),$z=0){global$w;$ce=array();$v=array();$f=array();$Wa=array();$Eh=array();$J=array();odd('');for($s=0;(!$z||$s<$z)&&($K=$I->fetch_row());$s++){if(!$s){echo"<table cellspacing='0' class='nowrap'>\n","<thead><tr>";for($Id=0;$Id<count($K);$Id++){$o=$I->fetch_field();$C=$o->name;$cf=$o->orgtable;$bf=$o->orgname;$J[$o->table]=$cf;if($df&&$w=="sql")$ce[$Id]=($C=="table"?"table=":($C=="possible_keys"?"indexes=":null));elseif($cf!=""){if(!isset($v[$cf])){$v[$cf]=array();foreach(indexes($cf,$i)as$u){if($u["type"]=="PRIMARY"){$v[$cf]=array_flip($u["columns"]);break;}}$f[$cf]=$v[$cf];}if(isset($f[$cf][$bf])){unset($f[$cf][$bf]);$v[$cf][$bf]=$Id;$ce[$Id]=$cf;}}if($o->charsetnr==63)$Wa[$Id]=true;$Eh[$Id]=$o->type;echo"<th".($cf!=""||$o->name!=$bf?" title='".h(($cf!=""?"$cf.":"").$bf)."'":"").">".h($C).($df?doc_link(array('sql'=>"explain-output.html#explain_".strtolower($C))):"");}echo"</thead>\n";}echo"<tr".odd().">";foreach($K
as$x=>$X){if($X===null)$X="<i>NULL</i>";elseif($Wa[$x]&&!is_utf8($X))$X="<i>".lang(43,strlen($X))."</i>";elseif(!strlen($X))$X="&nbsp;";else{$X=h($X);if($Eh[$x]==254)$X="<code>$X</code>";}if(isset($ce[$x])&&!$f[$ce[$x]]){if($df&&$w=="sql"){$Q=$K[array_search("table=",$ce)];$_=$ce[$x].urlencode($df[$Q]!=""?$df[$Q]:$Q);}else{$_="edit=".urlencode($ce[$x]);foreach($v[$ce[$x]]as$ob=>$Id)$_.="&where".urlencode("[".bracket_escape($ob)."]")."=".urlencode($K[$Id]);}$X="<a href='".h(ME.$_)."'>$X</a>";}echo"<td>$X";}}echo($s?"</table>":"<p class='message'>".lang(12))."\n";return$J;}function
referencable_primary($xg){$J=array();foreach(table_status('',true)as$Wg=>$Q){if($Wg!=$xg&&fk_support($Q)){foreach(fields($Wg)as$o){if($o["primary"]){if($J[$Wg]){unset($J[$Wg]);break;}$J[$Wg]=$o;}}}}return$J;}function
textarea($C,$Y,$L=10,$rb=80){global$w;echo"<textarea name='$C' rows='$L' cols='$rb' class='sqlarea jush-$w' spellcheck='false' wrap='off'>";if(is_array($Y)){foreach($Y
as$X)echo
h($X[0])."\n\n\n";}else
echo
h($Y);echo"</textarea>";}function
edit_type($x,$o,$qb,$Rc=array()){global$Og,$Eh,$Lh,$Qe;$U=$o["type"];echo'<td><select name="',h($x),'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);"',on_help("getTarget(event).value",1),'>';if($U&&!isset($Eh[$U])&&!isset($Rc[$U]))array_unshift($Og,$U);if($Rc)$Og[lang(86)]=$Rc;echo
optionlist($Og,$U),'</select>
<td><input name="',h($x),'[length]" value="',h($o["length"]),'" size="3" onfocus="editingLengthFocus(this);"',(!$o["length"]&&preg_match('~var(char|binary)$~',$U)?" class='required'":""),' onchange="editingLengthChange(this);" onkeyup="this.onchange();"><td class="options">';echo"<select name='".h($x)."[collation]'".(preg_match('~(char|text|enum|set)$~',$U)?"":" class='hidden'").'><option value="">('.lang(87).')'.optionlist($qb,$o["collation"]).'</select>',($Lh?"<select name='".h($x)."[unsigned]'".(!$U||preg_match('~((^|[^o])int|float|double|decimal)$~',$U)?"":" class='hidden'").'><option>'.optionlist($Lh,$o["unsigned"]).'</select>':''),(isset($o['on_update'])?"<select name='".h($x)."[on_update]'".(preg_match('~timestamp|datetime~',$U)?"":" class='hidden'").'>'.optionlist(array(""=>"(".lang(88).")","CURRENT_TIMESTAMP"),$o["on_update"]).'</select>':''),($Rc?"<select name='".h($x)."[on_delete]'".(preg_match("~`~",$U)?"":" class='hidden'")."><option value=''>(".lang(89).")".optionlist(explode("|",$Qe),$o["on_delete"])."</select> ":" ");}function
process_length($y){global$sc;return(preg_match("~^\\s*\\(?\\s*$sc(?:\\s*,\\s*$sc)*+\\s*\\)?\\s*\$~",$y)&&preg_match_all("~$sc~",$y,$he)?"(".implode(",",$he[0]).")":preg_replace('~^[0-9].*~','(\0)',preg_replace('~[^-0-9,+()[\]]~','',$y)));}function
process_type($o,$pb="COLLATE"){global$Lh;return" $o[type]".process_length($o["length"]).(preg_match('~(^|[^o])int|float|double|decimal~',$o["type"])&&in_array($o["unsigned"],$Lh)?" $o[unsigned]":"").(preg_match('~char|text|enum|set~',$o["type"])&&$o["collation"]?" $pb ".q($o["collation"]):"");}function
process_field($o,$Ch){global$w;$Ob=$o["default"];return
array(idf_escape(trim($o["field"])),process_type($Ch),($o["null"]?" NULL":" NOT NULL"),(isset($Ob)?" DEFAULT ".((preg_match('~time~',$o["type"])&&preg_match('~^CURRENT_TIMESTAMP$~i',$Ob))||($w=="sqlite"&&preg_match('~^CURRENT_(TIME|TIMESTAMP|DATE)$~i',$Ob))||($o["type"]=="bit"&&preg_match("~^([0-9]+|b'[0-1]+')\$~",$Ob))||($w=="pgsql"&&preg_match("~^[a-z]+\\(('[^']*')+\\)\$~",$Ob))?$Ob:q($Ob)):""),(preg_match('~timestamp|datetime~',$o["type"])&&$o["on_update"]?" ON UPDATE $o[on_update]":""),(support("comment")&&$o["comment"]!=""?" COMMENT ".q($o["comment"]):""),($o["auto_increment"]?auto_increment():null),);}function
type_class($U){foreach(array('char'=>'text','date'=>'time|year','binary'=>'blob','enum'=>'set',)as$x=>$X){if(preg_match("~$x|$X~",$U))return" class='$x'";}}function
edit_fields($p,$qb,$U="TABLE",$Rc=array(),$vb=false){global$h,$xd;$p=array_values($p);echo'<thead><tr class="wrap">
...
<td><input type="radio" name="auto_increment_col" value=""><acronym title="',lang(56),'">AI</acronym>',doc_link(array('sql'=>"example-auto-increment.html",'sqlite'=>"autoinc.html",'pgsql'=>"datatype.html#DATATYPE-SERIAL",'mssql'=>"ms186775.aspx",)),'<td>',lang(95),(support("comment")?"<td".($vb?"":" class='hidden'").">".lang(96):"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($p))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.2.3' alt='+' title='".lang(97)."'>",'<script type="text/javascript">row_count = ',count($p),';</script>
</thead>
<tbody onkeydown="return editingKeydown(event);">
';foreach($p
as$s=>$o){$s++;$ef=$o[($_POST?"orig":"field")];$Vb=(isset($_POST["add"][$s-1])||(isset($o["field"])&&!$_POST["drop_col"][$s]))&&(support("drop_col")||$ef=="");echo'<tr',($Vb?"":" style='display: none;'"),'>
',($U=="PROCEDURE"?"<td>".html_select("fields[$s][inout]",explode("|",$xd),$o["inout"]):""),'<th>';if($Vb){echo'<input name="fields[',$s,'][field]" value="',h($o["field"]),'" onchange="editingNameChange(this);',($o["field"]!=""||count($p)>1?'':' editingAddRow(this);" onkeyup="if (this.value) editingAddRow(this);'),'" maxlength="64" autocapitalize="off">';}echo'<input type="hidden" name="fields[',$s,'][orig]" value="',h($ef),'">
';edit_type("fields[$s]",$o,$qb,$Rc);if($U=="TABLE"){echo'<td>',checkbox("fields[$s][null]",1,$o["null"],"","","block"),'<td><label class="block"><input type="radio" name="auto_increment_col" value="',$s,'"';if($o["auto_increment"]){echo' checked';}?> onclick="var field = this.form['fields[' + this.value + '][field]']; if (!field.value) { field.value = 'id'; field.onchange(); }"></label><td><?php
echo
...
',(support("comment")?"<td".($vb?"":" class='hidden'")."><input name='fields[$s][comment]' value='".h($o["comment"])."' maxlength='".($h->server_info>=5.5?1024:255)."'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$s]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.2.3' alt='+' title='".lang(97)."' onclick='return !editingAddRow(this, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$s]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=4.2.3' alt='^' title='".lang(98)."'>&nbsp;"."<input type='image' class='icon' name='down[$s]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=4.2.3' alt='v' title='".lang(99)."'>&nbsp;":""),($ef==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$s]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=4.2.3' alt='x' title='".lang(100)."' onclick=\"return !editingRemoveRow(this, 'fields\$1[field]');\">":""),"\n";}}function
process_fields(&$p){ksort($p);$D=0;if($_POST["up"]){$Td=0;foreach($p
as$x=>$o){if(key($_POST["up"])==$x){unset($p[$x]);array_splice($p,$Td,0,array($o));break;}if(isset($o["field"]))$Td=$D;$D++;}}elseif($_POST["down"]){$Tc=false;foreach($p
as$x=>$o){if(isset($o["field"])&&$Tc){unset($p[key($_POST["down"])]);array_splice($p,$D,0,array($Tc));break;}if(key($_POST["down"])==$x)$Tc=$o;$D++;}}elseif($_POST["add"]){$p=array_values($p);array_splice($p,key($_POST["add"]),0,array(array()));}elseif(!$_POST["drop_col"])return
false;return
true;}function
normalize_enum($B){return"'".str_replace("'","''",addcslashes(stripcslashes(str_replace($B[0][0].$B[0][0],$B[0][0],substr($B[0],1,-1))),'\\'))."'";}function
grant($Yc,$Kf,$f,$Pe){if(!$Kf)return
true;if($Kf==array("ALL PRIVILEGES","GRANT OPTION"))return($Yc=="GRANT"?queries("$Yc ALL PRIVILEGES$Pe WITH GRANT OPTION"):queries("$Yc ALL PRIVILEGES$Pe")&&queries("$Yc GRANT OPTION$Pe"));return
queries("$Yc ".preg_replace('~(GRANT OPTION)\\([^)]*\\)~','\\1',implode("$f, ",$Kf).$f).$Pe);}function
drop_create($bc,$j,$cc,$ih,$ec,$A,$se,$qe,$re,$Me,$Ce){if($_POST["drop"])query_redirect($bc,$A,$se);elseif($Me=="")query_redirect($j,$A,$re);elseif($Me!=$Ce){$Fb=queries($j);queries_redirect($A,$qe,$Fb&&queries($bc));if($Fb)queries($cc);}else
queries_redirect($A,$qe,queries($ih)&&queries($ec)&&queries($bc)&&queries($j));}function
create_trigger($Pe,$K){global$w;$nh=" $K[Timing] $K[Event]".($K["Event"]=="UPDATE OF"?" ".idf_escape($K["Of"]):"");return"CREATE TRIGGER ".idf_escape($K["Trigger"]).($w=="mssql"?$Pe.$nh:$nh.$Pe).rtrim(" $K[Type]\n$K[Statement]",";").";";}function
create_routine($lg,$K){global$xd;$O=array();$p=(array)$K["fields"];ksort($p);foreach($p
as$o){if($o["field"]!="")$O[]=(preg_match("~^($xd)\$~",$o["inout"])?"$o[inout] ":"").idf_escape($o["field"]).process_type($o,"CHARACTER SET");}return"CREATE $lg ".idf_escape(trim($K["name"]))." (".implode(", ",$O).")".(isset($_GET["function"])?" RETURNS".process_type($K["returns"],"CHARACTER SET"):"").($K["language"]?" LANGUAGE $K[language]":"").rtrim("\n$K[definition]",";").";";}function
...
tar_file($Lc,$sh){$J=pack("a100a8a8a8a12a12",$Lc,644,0,0,decoct($sh->size),decoct(time()));$ib=8*32;for($s=0;$s<strlen($J);$s++)$ib+=ord($J[$s]);$J.=sprintf("%06o",$ib)."\0 ";echo$J,str_repeat("\0",512-strlen($J));$sh->send();echo
str_repeat("\0",511-($sh->size+511)%512);}function
ini_bytes($wd){$X=ini_get($wd);switch(strtolower(substr($X,-1))){case'g':$X*=1024;case'm':$X*=1024;case'k':$X*=1024;}return$X;}function
doc_link($vf){global$w,$h;$Ph=array('sql'=>"http://dev.mysql.com/doc/refman/".substr($h->server_info,0,3)."/en/",'sqlite'=>"http://www.sqlite.org/",'pgsql'=>"http://www.postgresql.org/docs/".substr($h->server_info,0,3)."/static/",'mssql'=>"http://msdn.microsoft.com/library/",'oracle'=>"http://download.oracle.com/docs/cd/B19306_01/server.102/b14200/",);return($vf[$w]?"<a href='$Ph[$w]$vf[$w]' target='_blank' rel='noreferrer'><sup>?</sup></a>":"");}function
...
as$C=>$u){ksort($u["columns"]);$Hf=array();foreach($u["columns"]as$x=>$X)$Hf[]="<i>".h($X)."</i>".($u["lengths"][$x]?"(".$u["lengths"][$x].")":"").($u["descs"][$x]?" DESC":"");echo"<tr title='".h($C)."'><th>$u[type]<td>".implode(", ",$Hf)."\n";}echo"</table>\n";}echo'<p class="links"><a href="'.h(ME).'indexes='.urlencode($a).'">'.lang(122)."</a>\n";}if(fk_support($R)){echo"<h3 id='foreign-keys'>".lang(86)."</h3>\n";$Rc=foreign_keys($a);if($Rc){echo"<table cellspacing='0'>\n","<thead><tr><th>".lang(123)."<td>".lang(124)."<td>".lang(89)."<td>".lang(88)."<td>&nbsp;</thead>\n";foreach($Rc
as$C=>$q){echo"<tr title='".h($C)."'>","<th><i>".implode("</i>, <i>",array_map('h',$q["source"]))."</i>","<td><a href='".h($q["db"]!=""?preg_replace('~db=[^&]*~',"db=".urlencode($q["db"]),ME):($q["ns"]!=""?preg_replace('~ns=[^&]*~',"ns=".urlencode($q["ns"]),ME):ME))."table=".urlencode($q["table"])."'>".($q["db"]!=""?"<b>".h($q["db"])."</b>.":"").($q["ns"]!=""?"<b>".h($q["ns"])."</b>.":"").h($q["table"])."</a>","(<i>".implode("</i>, <i>",array_map('h',$q["target"]))."</i>)","<td>".nbsp($q["on_delete"])."\n","<td>".nbsp($q["on_update"])."\n",'<td><a href="'.h(ME.'foreign='.urlencode($a).'&name='.urlencode($C)).'">'.lang(125).'</a>';}echo"</table>\n";}echo'<p class="links"><a href="'.h(ME).'foreign='.urlencode($a).'">'.lang(126)."</a>\n";}}if(support(is_view($R)?"view_trigger":"trigger")){echo"<h3 id='triggers'>".lang(127)."</h3>\n";$Bh=triggers($a);if($Bh){echo"<table cellspacing='0'>\n";foreach($Bh
as$x=>$X)echo"<tr valign='top'><td>".h($X[0])."<td>".h($X[1])."<th>".h($x)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($x))."'>".lang(125)."</a>\n";echo"</table>\n";}echo'<p class="links"><a href="'.h(ME).'trigger='.urlencode($a).'">'.lang(128)."</a>\n";}}elseif(isset($_GET["schema"])){page_header(lang(62),"",array(),h(DB.($_GET["ns"]?".$_GET[ns]":"")));$Yg=array();$Zg=array();$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE["adminer_schema-".str_replace(".","_",DB)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$he,PREG_SET_ORDER);foreach($he
as$s=>$B){$Yg[$B[1]]=array($B[2],$B[3]);$Zg[]="\n\t'".js_escape($B[1])."': [ $B[2], $B[3] ]";}$uh=0;$Ta=-1;$rg=array();$Zf=array();$Xd=array();foreach(table_status('',true)as$Q=>$R){if(is_view($R))continue;$_f=0;$rg[$Q]["fields"]=array();foreach(fields($Q)as$C=>$o){$_f+=1.25;$o["pos"]=$_f;$rg[$Q]["fields"][$C]=$o;}$rg[$Q]["pos"]=($Yg[$Q]?$Yg[$Q]:array($uh,0));foreach($b->foreignKeys($Q)as$X){if(!$X["db"]){$Vd=$Ta;if($Yg[$Q][1]||$Yg[$X["table"]][1])$Vd=min(floatval($Yg[$Q][1]),floatval($Yg[$X["table"]][1]))-1;else$Ta-=.1;while($Xd[(string)$Vd])$Vd-=.0001;$rg[$Q]["references"][$X["table"]][(string)$Vd]=array($X["source"],$X["target"]);$Zf[$X["table"]][$Q][(string)$Vd]=$X["target"];$Xd[(string)$Vd]=true;}}$uh=max($uh,$rg[$Q]["pos"][0]+2.5+$_f);}echo'<div id="schema" style="height: ',$uh,'em;" onselectstart="return false;">
<script type="text/javascript">
var tablePos = {',implode(",",$Zg)."\n",'};
var em = document.getElementById(\'schema\').offsetHeight / ',$uh,';
...
';foreach($rg
as$C=>$Q){echo"<div class='table' style='top: ".$Q["pos"][0]."em; left: ".$Q["pos"][1]."em;' onmousedown='schemaMousedown(this, event);'>",'<a href="'.h(ME).'table='.urlencode($C).'"><b>'.h($C)."</b></a>";foreach($Q["fields"]as$o){$X='<span'.type_class($o["type"]).' title="'.h($o["full_type"].($o["null"]?" NULL":'')).'">'.h($o["field"]).'</span>';echo"<br>".($o["primary"]?"<i>$X</i>":$X);}foreach((array)$Q["references"]as$fh=>$ag){foreach($ag
as$Vd=>$Wf){$Wd=$Vd-$Yg[$C][1];$s=0;foreach($Wf[0]as$Gg)echo"\n<div class='references' title='".h($fh)."' id='refs$Vd-".($s++)."' style='left: $Wd"."em; top: ".$Q["fields"][$Gg]["pos"]."em; padding-top: .5em;'><div style='border-top: 1px solid Gray; width: ".(-$Wd)."em;'></div></div>";}}foreach((array)$Zf[$C]as$fh=>$ag){foreach($ag
as$Vd=>$f){$Wd=$Vd-$Yg[$C][1];$s=0;foreach($f
as$eh)echo"\n<div class='references' title='".h($fh)."' id='refd$Vd-".($s++)."' style='left: $Wd"."em; top: ".$Q["fields"][$eh]["pos"]."em; height: 1.25em; background: url(".h(preg_replace("~\\?.*~","",ME))."?file=arrow.gif) no-repeat right center;&amp;version=4.2.3'><div style='height: .5em; border-bottom: 1px solid Gray; width: ".(-$Wd)."em;'></div></div>";}}echo"\n</div>\n";}foreach($rg
as$C=>$Q){foreach((array)$Q["references"]as$fh=>$ag){foreach($ag
as$Vd=>$Wf){$we=$uh;$le=-10;foreach($Wf[0]as$x=>$Gg){$Af=$Q["pos"][0]+$Q["fields"][$Gg]["pos"];$Bf=$rg[$fh]["pos"][0]+$rg[$fh]["fields"][$Wf[1][$x]]["pos"];$we=min($we,$Af,$Bf);$le=max($le,$Af,$Bf);}echo"<div class='references' id='refl$Vd' style='left: $Vd"."em; top: $we"."em; padding: .5em 0;'><div style='border-right: 1px solid Gray; margin-top: 1px; height: ".($le-$we)."em;'></div></div>\n";}}}echo'</div>
<p class="links"><a href="',h(ME."schema=".urlencode($ea)),'" id="schema-link">',lang(129),'</a>
';}elseif(isset($_GET["dump"])){$a=$_GET["dump"];if($_POST&&!$n){$Cb="";foreach(array("output","format","db_style","routines","events","table_style","auto_increment","triggers","data_style")as$x)$Cb.="&$x=".urlencode($_POST[$x]);cookie("adminer_export",substr($Cb,1));$S=array_flip((array)$_POST["tables"])+array_flip((array)$_POST["data"]);$Dc=dump_headers((count($S)==1?key($S):DB),(DB==""||count($S)>1));$Ed=preg_match('~sql~',$_POST["format"]);if($Ed){echo"-- Adminer $ia ".$ac[DRIVER]." dump\n\n";if($w=="sql"){echo"SET NAMES utf8;
SET time_zone = '+00:00';
".($_POST["data_style"]?"SET foreign_key_checks = 0;
SET sql_mode = 'NO_AUTO_VALUE_ON_ZERO';
":"")."
";$h->query("SET time_zone = '+00:00';");}}$Pg=$_POST["db_style"];$l=array(DB);if(DB==""){$l=$_POST["databases"];if(is_string($l))$l=explode("\n",rtrim(str_replace("\r","",$l),"\n"));}foreach((array)$l
as$m){$b->dumpDatabase($m);if($h->select_db($m)){if($Ed&&preg_match('~CREATE~',$Pg)&&($j=$h->result("SHOW CREATE DATABASE ".idf_escape($m),1))){set_utf8mb4($j);if($Pg=="DROP+CREATE")echo"DROP DATABASE IF EXISTS ".idf_escape($m).";\n";echo"$j;\n";}if($Ed){if($Pg)echo
use_sql($m).";\n\n";$jf="";if($_POST["routines"]){foreach(array("FUNCTION","PROCEDURE")as$lg){foreach(get_rows("SHOW $lg STATUS WHERE Db = ".q($m),null,"-- ")as$K){$j=remove_definer($h->result("SHOW CREATE $lg ".idf_escape($K["Name"]),2));set_utf8mb4($j);$jf.=($Pg!='DROP+CREATE'?"DROP $lg IF EXISTS ".idf_escape($K["Name"]).";;\n":"")."$j;;\n\n";}}}if($_POST["events"]){foreach(get_rows("SHOW EVENTS",null,"-- ")as$K){$j=remove_definer($h->result("SHOW CREATE EVENT ".idf_escape($K["Name"]),3));set_utf8mb4($j);$jf.=($Pg!='DROP+CREATE'?"DROP EVENT IF EXISTS ".idf_escape($K["Name"]).";;\n":"")."$j;;\n\n";}}if($jf)echo"DELIMITER ;;\n\n$jf"."DELIMITER ;\n\n";}if($_POST["table_style"]||$_POST["data_style"]){$bi=array();foreach(table_status('',true)as$C=>$R){$Q=(DB==""||in_array($C,(array)$_POST["tables"]));$Hb=(DB==""||in_array($C,(array)$_POST["data"]));if($Q||$Hb){if($Dc=="tar"){$sh=new
TmpFile;ob_start(array($sh,'write'),1e5);}$b->dumpTable($C,($Q?$_POST["table_style"]:""),(is_view($R)?2:0));if(is_view($R))$bi[]=$C;elseif($Hb){$p=fields($C);$b->dumpData($C,$_POST["data_style"],"SELECT *".convert_fields($p,$p)." FROM ".table($C));}if($Ed&&$_POST["triggers"]&&$Q&&($Bh=trigger_sql($C,$_POST["table_style"])))echo"\nDELIMITER ;;\n$Bh\nDELIMITER ;\n";if($Dc=="tar"){ob_end_flush();tar_file((DB!=""?"":"$m/")."$C.csv",$sh);}elseif($Ed)echo"\n";}}foreach($bi
as$ai)$b->dumpTable($ai,$_POST["table_style"],1);if($Dc=="tar")echo
pack("x512");}}}if($Ed)echo"-- ".$h->result("SELECT NOW()")."\n";exit;}page_header(lang(65),$n,($_GET["export"]!=""?array("table"=>$_GET["export"]):array()),h(DB));echo'
<form action="" method="post">
<table cellspacing="0">
';$Lb=array('','USE','DROP+CREATE','CREATE');$ah=array('','DROP+CREATE','CREATE');$Ib=array('','TRUNCATE+INSERT','INSERT');if($w=="sql")$Ib[]='INSERT+UPDATE';parse_str($_COOKIE["adminer_export"],$K);if(!$K)$K=array("output"=>"text","format"=>"sql","db_style"=>(DB!=""?"":"CREATE"),"table_style"=>"DROP+CREATE","data_style"=>"INSERT");if(!isset($K["events"])){$K["routines"]=$K["events"]=($_GET["dump"]=="");$K["triggers"]=$K["table_style"];}echo"<tr><th>".lang(130)."<td>".html_select("output",$b->dumpOutput(),$K["output"],0)."\n";echo"<tr><th>".lang(131)."<td>".html_select("format",$b->dumpFormat(),$K["format"],0)."\n";echo($w=="sqlite"?"":"<tr><th>".lang(35)."<td>".html_select('db_style',$Lb,$K["db_style"]).(support("routine")?checkbox("routines",1,$K["routines"],lang(132)):"").(support("event")?checkbox("events",1,$K["events"],lang(133)):"")),"<tr><th>".lang(112)."<td>".html_select('table_style',$ah,$K["table_style"]).checkbox("auto_increment",1,$K["auto_increment"],lang(56)).(support("trigger")?checkbox("triggers",1,$K["triggers"],lang(127)):""),"<tr><th>".lang(134)."<td>".html_select('data_style',$Ib,$K["data_style"]),'</table>
<p><input type="submit" value="',lang(65),'">
<input type="hidden" name="token" value="',$T,'">

<table cellspacing="0">
';$Ef=array();if(DB!=""){$gb=($a!=""?"":" checked");echo"<thead><tr>","<th style='text-align: left;'><label class='block'><input type='checkbox' id='check-tables'$gb onclick='formCheck(this, /^tables\\[/);'>".lang(112)."</label>","<th style='text-align: right;'><label class='block'>".lang(134)."<input type='checkbox' id='check-data'$gb onclick='formCheck(this, /^data\\[/);'></label>","</thead>\n";$bi="";$bh=tables_list();foreach($bh
as$C=>$U){$Df=preg_replace('~_.*~','',$C);$gb=($a==""||$a==(substr($a,-1)=="%"?"$Df%":$C));$Hf="<tr><td>".checkbox("tables[]",$C,$gb,$C,"checkboxClick(event, this); formUncheck('check-tables');","block");if($U!==null&&!preg_match('~table~i',$U))$bi.="$Hf\n";else
echo"$Hf<td align='right'><label class='block'><span id='Rows-".h($C)."'></span>".checkbox("data[]",$C,$gb,"","checkboxClick(event, this); formUncheck('check-data');")."</label>\n";$Ef[$Df]++;}echo$bi;if($bh)echo"<script type='text/javascript'>ajaxSetHtml('".js_escape(ME)."script=db');</script>\n";}else{echo"<thead><tr><th style='text-align: left;'><label class='block'><input type='checkbox' id='check-databases'".($a==""?" checked":"")." onclick='formCheck(this, /^databases\\[/);'>".lang(35)."</label></thead>\n";$l=$b->databases();if($l){foreach($l
as$m){if(!information_schema($m)){$Df=preg_replace('~_.*~','',$m);echo"<tr><td>".checkbox("databases[]",$m,$a==""||$a=="$Df%",$m,"formUncheck('check-databases');","block")."\n";$Ef[$Df]++;}}}else
echo"<tr><td><textarea name='databases' rows='10' cols='20'></textarea>";}echo'</table>
</form>
';$Nc=true;foreach($Ef
as$x=>$X){if($x!=""&&$X>1){echo($Nc?"<p>":" ")."<a href='".h(ME)."dump=".urlencode("$x%")."'>".h($x)."</a>";$Nc=false;}}}elseif(isset($_GET["privileges"])){page_header(lang(63));$I=$h->query("SELECT User, Host FROM mysql.".(DB==""?"user":"db WHERE ".q(DB)." LIKE Db")." ORDER BY Host, User");$Yc=$I;if(!$I)$I=$h->query("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', 1) AS User, SUBSTRING_INDEX(CURRENT_USER, '@', -1) AS Host");echo"<form action=''><p>\n";hidden_fields_get();echo"<input type='hidden' name='db' value='".h(DB)."'>\n",($Yc?"":"<input type='hidden' name='grant' value=''>\n"),"<table cellspacing='0'>\n","<thead><tr><th>".lang(33)."<th>".lang(32)."<th>&nbsp;</thead>\n";while($K=$I->fetch_assoc())echo'<tr'.odd().'><td>'.h($K["User"])."<td>".h($K["Host"]).'<td><a href="'.h(ME.'user='.urlencode($K["User"]).'&host='.urlencode($K["Host"])).'">'.lang(10)."</a>\n";if(!$Yc||DB!="")echo"<tr".odd()."><td><input name='user' autocapitalize='off'><td><input name='host' value='localhost' autocapitalize='off'><td><input type='submit' value='".lang(10)."'>\n";echo"</table>\n","</form>\n",'<p class="links"><a href="'.h(ME).'user=">'.lang(135)."</a>";}elseif(isset($_GET["sql"])){if(!$n&&$_POST["export"]){dump_headers("sql");$b->dumpTable("","");$b->dumpData("","table",$_POST["query"]);exit;}restart_session();$id=&get_session("queries");$hd=&$id[DB];if(!$n&&$_POST["clear"]){$hd=array();redirect(remove_from_uri("history"));}page_header((isset($_GET["import"])?lang(64):lang(55)),$n);if(!$n&&$_POST){$Vc=false;if(!isset($_GET["import"]))$H=$_POST["query"];elseif($_POST["webfile"]){$Vc=@fopen((file_exists("adminer.sql")?"adminer.sql":"compress.zlib://adminer.sql.gz"),"rb");$H=($Vc?fread($Vc,1e6):false);}else$H=get_file("sql_file",true);if(is_string($H)){if(function_exists('memory_get_usage'))@ini_set("memory_limit",max(ini_bytes("memory_limit"),2*strlen($H)+memory_get_usage()+8e6));if($H!=""&&strlen($H)<1e6){$Of=$H.(preg_match("~;[ \t\r\n]*\$~",$H)?"":";");if(!$hd||reset(end($hd))!=$Of){restart_session();$hd[]=array($Of,time());set_session("queries",$id);stop_session();}}$Hg="(?:\\s|/\\*.*\\*/|(?:#|-- )[^\n]*\n|--\r?\n)";$Qb=";";$D=0;$pc=true;$i=connect();if(is_object($i)&&DB!="")$i->select_db(DB);$tb=0;$uc=array();$be=0;$of='[\'"'.($w=="sql"?'`#':($w=="sqlite"?'`[':($w=="mssql"?'[':''))).']|/\\*|-- |$'.($w=="pgsql"?'|\\$[^$]*\\$':'');$vh=microtime(true);parse_str($_COOKIE["adminer_export"],$_a);$gc=$b->dumpFormat();unset($gc["sql"]);while($H!=""){if(!$D&&preg_match("~^$Hg*DELIMITER\\s+(\\S+)~i",$H,$B)){$Qb=$B[1];$H=substr($H,strlen($B[0]));}else{preg_match('('.preg_quote($Qb)."\\s*|$of)",$H,$B,PREG_OFFSET_CAPTURE,$D);list($Tc,$_f)=$B[0];if(!$Tc&&$Vc&&!feof($Vc))$H.=fread($Vc,1e5);else{if(!$Tc&&rtrim($H)=="")break;$D=$_f+strlen($Tc);if($Tc&&rtrim($Tc)!=$Qb){while(preg_match('('.($Tc=='/*'?'\\*/':($Tc=='['?']':(preg_match('~^-- |^#~',$Tc)?"\n":preg_quote($Tc)."|\\\\."))).'|$)s',$H,$B,PREG_OFFSET_CAPTURE,$D)){$pg=$B[0][0];if(!$pg&&$Vc&&!feof($Vc))$H.=fread($Vc,1e5);else{$D=$B[0][1]+strlen($pg);if($pg[0]!="\\")break;}}}else{$pc=false;$Of=substr($H,0,$_f);$tb++;$Hf="<pre id='sql-$tb'><code class='jush-$w'>".shorten_utf8(trim($Of),1000)."</code></pre>\n";if(!$_POST["only_errors"]){echo$Hf;ob_flush();flush();}$Kg=microtime(true);if($h->multi_query($Of)&&is_object($i)&&preg_match("~^$Hg*USE\\b~isU",$Of))$i->query($Of);do{$I=$h->store_result();$lh=" <span class='time'>(".format_time($Kg).")</span>".(strlen($Of)<1000?" <a href='".h(ME)."sql=".urlencode(trim($Of))."'>".lang(10)."</a>":"");if($h->error){echo($_POST["only_errors"]?$Hf:""),"<p class='error'>".lang(136).($h->errno?" ($h->errno)":"").": ".error()."\n";$uc[]=" <a href='#sql-$tb'>$tb</a>";if($_POST["error_stops"])break
2;}elseif(is_object($I)){$z=$_POST["limit"];$df=select($I,$i,array(),$z);if(!$_POST["only_errors"]){echo"<form action='' method='post'>\n";$Ge=$I->num_rows;echo"<p>".($Ge?($z&&$Ge>$z?lang(137,$z):"").lang(138,$Ge):""),$lh;$md="export-$tb";$Cc=", <a href='#$md' onclick=\"return !toggle('$md');\">".lang(65)."</a><span id='$md' class='hidden'>: ".html_select("output",$b->dumpOutput(),$_a["output"])." ".html_select("format",$gc,$_a["format"])."<input type='hidden' name='query' value='".h($Of)."'>"." <input type='submit' name='export' value='".lang(65)."'><input type='hidden' name='token' value='$T'></span>\n";if($i&&preg_match("~^($Hg|\\()*SELECT\\b~isU",$Of)&&($Bc=explain($i,$Of))){$md="explain-$tb";echo", <a href='#$md' onclick=\"return !toggle('$md');\">EXPLAIN</a>$Cc","<div id='$md' class='hidden'>\n";select($Bc,$i,$df);echo"</div>\n";}else
echo$Cc;echo"</form>\n";}}else{if(preg_match("~^$Hg*(CREATE|DROP|ALTER)$Hg+(DATABASE|SCHEMA)\\b~isU",$Of)){restart_session();set_session("dbs",null);stop_session();}if(!$_POST["only_errors"])echo"<p class='message' title='".h($h->info)."'>".lang(139,$h->affected_rows)."$lh\n";}$Kg=microtime(true);}while($h->next_result());$be+=substr_count($Of.$Tc,"\n");$H=substr($H,$D);$D=0;}}}}if($pc)echo"<p class='message'>".lang(140)."\n";elseif($_POST["only_errors"]){echo"<p class='message'>".lang(141,$tb-count($uc))," <span class='time'>(".format_time($vh).")</span>\n";}elseif($uc&&$tb>1)echo"<p class='error'>".lang(136).": ".implode("",$uc)."\n";}else
echo"<p class='error'>".upload_error($H)."\n";}echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
';$zc="<input type='submit' value='".lang(142)."' title='Ctrl+Enter'>";if(!isset($_GET["import"])){$Of=$_GET["sql"];if($_POST)$Of=$_POST["query"];elseif($_GET["history"]=="all")$Of=$hd;elseif($_GET["history"]!="")$Of=$hd[$_GET["history"]][0];echo"<p>";textarea("query",$Of,20);echo($_POST?"":"<script type='text/javascript'>focus(document.getElementsByTagName('textarea')[0]);</script>\n"),"<p>$zc\n",lang(143).": <input type='number' name='limit' class='size' value='".h($_POST?$_POST["limit"]:$_GET["limit"])."'>\n";}else{echo"<fieldset><legend>".lang(144)."</legend><div>",(ini_bool("file_uploads")?"SQL (&lt; ".ini_get("upload_max_filesize")."B): <input type='file' name='sql_file[]' multiple>\n$zc":lang(145)),"</div></fieldset>\n","<fieldset><legend>".lang(146)."</legend><div>",lang(147,"<code>adminer.sql".(extension_loaded("zlib")?"[.gz]":"")."</code>"),' <input type="submit" name="webfile" value="'.lang(148).'">',"</div></fieldset>\n","<p>";}echo
checkbox("error_stops",1,($_POST?$_POST["error_stops"]:isset($_GET["import"])),lang(149))."\n",checkbox("only_errors",1,($_POST?$_POST["only_errors"]:isset($_GET["import"])),lang(150))."\n","<input type='hidden' name='token' value='$T'>\n";if(!isset($_GET["import"])&&$hd){print_fieldset("history",lang(151),$_GET["history"]!="");for($X=end($hd);$X;$X=prev($hd)){$x=key($hd);list($Of,$lh,$kc)=$X;echo'<a href="'.h(ME."sql=&history=$x").'">'.lang(10)."</a>"." <span class='time' title='".@date('Y-m-d',$lh)."'>".@date("H:i:s",$lh)."</span>"." <code class='jush-$w'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$Of)))),80,"</code>").($kc?" <span class='time'>($kc)</span>":"")."<br>\n";}echo"<input type='submit' name='clear' value='".lang(152)."'>\n","<a href='".h(ME."sql=&history=all")."'>".lang(153)."</a>\n","</div></fieldset>\n";}echo'</form>
';}elseif(isset($_GET["edit"])){$a=$_GET["edit"];$p=fields($a);$Z=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0],$p):""):where($_GET,$p));$Mh=(isset($_GET["select"])?$_POST["edit"]:$Z);foreach($p
as$C=>$o){if(!isset($o["privileges"][$Mh?"update":"insert"])||$b->fieldName($o)=="")unset($p[$C]);}if($_POST&&!$n&&!isset($_GET["select"])){$A=$_POST["referer"];if($_POST["insert"])$A=($Mh?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$A))$A=ME."select=".urlencode($a);$v=indexes($a);$Hh=unique_array($_GET["where"],$v);$Rf="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($A,lang(154),$Zb->delete($a,$Rf,!$Hh));else{$O=array();foreach($p
as$C=>$o){$X=process_input($o);if($X!==false&&$X!==null)$O[idf_escape($C)]=$X;}if($Mh){if(!$O)redirect($A);queries_redirect($A,lang(155),$Zb->update($a,$O,$Rf,!$Hh));if(is_ajax()){page_headers();page_messages($n);exit;}}else{$I=$Zb->insert($a,$O);$Ud=($I?last_id():0);queries_redirect($A,lang(156,($Ud?" $Ud":"")),$I);}}}$K=null;if($_POST["save"])$K=(array)$_POST["fields"];elseif($Z){$M=array();foreach($p
as$C=>$o){if(isset($o["privileges"]["select"])){$Ja=convert_field($o);if($_POST["clone"]&&$o["auto_increment"])$Ja="''";if($w=="sql"&&preg_match("~enum|set~",$o["type"]))$Ja="1*".idf_escape($C);$M[]=($Ja?"$Ja AS ":"").idf_escape($C);}}$K=array();if(!support("table"))$M=array("*");if($M){$I=$Zb->select($a,$M,array($Z),$M,array(),(isset($_GET["select"])?2:1));$K=$I->fetch_assoc();if(!$K)$K=false;if(isset($_GET["select"])&&(!$K||$I->fetch_assoc()))$K=null;}}if(!support("table")&&!$p){if(!$Z){$I=$Zb->select($a,array("*"),$Z,array("*"));$K=($I?$I->fetch_assoc():false);if(!$K)$K=array($Zb->primary=>"");}if($K){foreach($K
as$x=>$X){if(!$Z)$K[$x]=null;$p[$x]=array("field"=>$x,"null"=>($x!=$Zb->primary),"auto_increment"=>($x==$Zb->primary));}}}edit_form($a,$p,$K,$Mh);}elseif(isset($_GET["create"])){$a=$_GET["create"];$pf=array();foreach(array('HASH','LINEAR HASH','KEY','LINEAR KEY','RANGE','LIST')as$x)$pf[$x]=$x;$Yf=referencable_primary($a);$Rc=array();foreach($Yf
as$Wg=>$o)$Rc[str_replace("`","``",$Wg)."`".str_replace("`","``",$o["field"])]=$Wg;$gf=array();$R=array();if($a!=""){$gf=fields($a);$R=table_status($a);if(!$R)$n=lang(9);}$K=$_POST;$K["fields"]=(array)$K["fields"];if($K["auto_increment_col"])$K["fields"][$K["auto_increment_col"]]["auto_increment"]=true;if($_POST&&!process_fields($K["fields"])&&!$n){if($_POST["drop"])queries_redirect(substr(ME,0,-1),lang(157),drop_tables(array($a)));else{$p=array();$Ga=array();$Qh=false;$Pc=array();ksort($K["fields"]);$ff=reset($gf);$Da=" FIRST";foreach($K["fields"]as$x=>$o){$q=$Rc[$o["type"]];$Ch=($q!==null?$Yf[$q]:$o);if($o["field"]!=""){if(!$o["has_default"])$o["default"]=null;if($x==$K["auto_increment_col"])$o["auto_increment"]=true;$Mf=process_field($o,$Ch);$Ga[]=array($o["orig"],$Mf,$Da);if($Mf!=process_field($ff,$ff)){$p[]=array($o["orig"],$Mf,$Da);if($o["orig"]!=""||$Da)$Qh=true;}if($q!==null)$Pc[idf_escape($o["field"])]=($a!=""&&$w!="sqlite"?"ADD":" ").format_foreign_key(array('table'=>$Rc[$o["type"]],'source'=>array($o["field"]),'target'=>array($Ch["field"]),'on_delete'=>$o["on_delete"],));$Da=" AFTER ".idf_escape($o["field"]);}elseif($o["orig"]!=""){$Qh=true;$p[]=array($o["orig"]);}if($o["orig"]!=""){$ff=next($gf);if(!$ff)$Da="";}}$rf="";if($pf[$K["partition_by"]]){$sf=array();if($K["partition_by"]=='RANGE'||$K["partition_by"]=='LIST'){foreach(array_filter($K["partition_names"])as$x=>$X){$Y=$K["partition_values"][$x];$sf[]="\n  PARTITION ".idf_escape($X)." VALUES ".($K["partition_by"]=='RANGE'?"LESS THAN":"IN").($Y!=""?" ($Y)":" MAXVALUE");}}$rf.="\nPARTITION BY $K[partition_by]($K[partition])".($sf?" (".implode(",",$sf)."\n)":($K["partitions"]?" PARTITIONS ".(+$K["partitions"]):""));}elseif(support("partitioning")&&preg_match("~partitioned~",$R["Create_options"]))$rf.="\nREMOVE PARTITIONING";$pe=lang(158);if($a==""){cookie("adminer_engine",$K["Engine"]);$pe=lang(159);}$C=trim($K["name"]);queries_redirect(ME.(support("table")?"table=":"select=").urlencode($C),$pe,alter_table($a,$C,($w=="sqlite"&&($Qh||$Pc)?$Ga:$p),$Pc,($K["Comment"]!=$R["Comment"]?$K["Comment"]:null),($K["Engine"]&&$K["Engine"]!=$R["Engine"]?$K["Engine"]:""),($K["Collation"]&&$K["Collation"]!=$R["Collation"]?$K["Collation"]:""),($K["Auto_increment"]!=""?number($K["Auto_increment"]):""),$rf));}}page_header(($a!=""?lang(41):lang(66)),$n,array("table"=>$a),h($a));if(!$_POST){$K=array("Engine"=>$_COOKIE["adminer_engine"],"fields"=>array(array("field"=>"","type"=>(isset($Eh["int"])?"int":(isset($Eh["integer"])?"integer":"")))),"partition_names"=>array(""),);if($a!=""){$K=$R;$K["name"]=$a;$K["fields"]=array();if(!$_GET["auto_increment"])$K["Auto_increment"]="";foreach($gf
as$o){$o["has_default"]=isset($o["default"]);$K["fields"][]=$o;}if(support("partitioning")){$Wc="FROM information_schema.PARTITIONS WHERE TABLE_SCHEMA = ".q(DB)." AND TABLE_NAME = ".q($a);$I=$h->query("SELECT PARTITION_METHOD, PARTITION_ORDINAL_POSITION, PARTITION_EXPRESSION $Wc ORDER BY PARTITION_ORDINAL_POSITION DESC LIMIT 1");list($K["partition_by"],$K["partitions"],$K["partition"])=$I->fetch_row();$sf=get_key_vals("SELECT PARTITION_NAME, PARTITION_DESCRIPTION $Wc AND PARTITION_NAME != '' ORDER BY PARTITION_ORDINAL_POSITION");$sf[""]="";$K["partition_names"]=array_keys($sf);$K["partition_values"]=array_values($sf);}}}$qb=collations();$rc=engines();foreach($rc
as$qc){if(!strcasecmp($qc,$K["Engine"])){$K["Engine"]=$qc;break;}}echo'
<form action="" method="post" id="form">
<p>
';if(support("columns")||$a==""){echo
lang(160),': <input name="name" maxlength="64" value="',h($K["name"]),'" autocapitalize="off">
';if($a==""&&!$_POST){?><script type='text/javascript'>focus(document.getElementById('form')['name']);</script><?php }echo($rc?"<select name='Engine' onchange='helpClose();'".on_help("getTarget(event).value",1).">".optionlist(array(""=>"(".lang(161).")")+$rc,$K["Engine"])."</select>":""),' ',($qb&&!preg_match("~sqlite|mssql~",$w)?html_select("Collation",array(""=>"(".lang(87).")")+$qb,$K["Collation"]):""),' <input type="submit" value="',lang(14),'">
';}echo'
';if(support("columns")){echo'<table cellspacing="0" id="edit-fields" class="nowrap">
';$vb=($_POST?$_POST["comments"]:$K["Comment"]!="");if(!$_POST&&!$vb){foreach($K["fields"]as$o){if($o["comment"]!=""){$vb=true;break;}}}edit_fields($K["fields"],$qb,"TABLE",$Rc,$vb);echo'</table>
<p>
',lang(56),': <input type="number" name="Auto_increment" size="6" value="',h($K["Auto_increment"]),'">
',checkbox("defaults",1,true,lang(162),"columnShow(this.checked, 5)","jsonly");if(!$_POST["defaults"]){echo'<script type="text/javascript">editingHideDefaults()</script>';}echo(support("comment")?"<label><input type='checkbox' name='comments' value='1' class='jsonly' onclick=\"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();\"".($vb?" checked":"").">".lang(96)."</label>".' <input name="Comment" id="Comment" value="'.h($K["Comment"]).'" maxlength="'.($h->server_info>=5.5?2048:60).'"'.($vb?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="',lang(14),'">
';}echo'
';if($a!=""){echo'<input type="submit" name="drop" value="',lang(116),'"',confirm(),'>';}if(support("partitioning")){$qf=preg_match('~RANGE|LIST~',$K["partition_by"]);print_fieldset("partition",lang(163),$K["partition_by"]);echo'<p>
',"<select name='partition_by' onchange='partitionByChange(this);'".on_help("getTarget(event).value.replace(/./, 'PARTITION BY \$&')",1).">".optionlist(array(""=>"")+$pf,$K["partition_by"])."</select>",'(<input name="partition" value="',h($K["partition"]),'">)
',lang(164),': <input type="number" name="partitions" class="size',($qf||!$K["partition_by"]?" hidden":""),'" value="',h($K["partitions"]),'">
...
';}elseif(isset($_GET["indexes"])){$a=$_GET["indexes"];$sd=array("PRIMARY","UNIQUE","INDEX");$R=table_status($a,true);if(preg_match('~MyISAM|M?aria'.($h->server_info>=5.6?'|InnoDB':'').'~i',$R["Engine"]))$sd[]="FULLTEXT";$v=indexes($a);$Ff=array();if($w=="mongo"){$Ff=$v["_id_"];unset($sd[0]);unset($v["_id_"]);}$K=$_POST;if($_POST&&!$n&&!$_POST["add"]&&!$_POST["drop_col"]){$c=array();foreach($K["indexes"]as$u){$C=$u["name"];if(in_array($u["type"],$sd)){$f=array();$Zd=array();$Sb=array();$O=array();ksort($u["columns"]);foreach($u["columns"]as$x=>$e){if($e!=""){$y=$u["lengths"][$x];$Rb=$u["descs"][$x];$O[]=idf_escape($e).($y?"(".(+$y).")":"").($Rb?" DESC":"");$f[]=$e;$Zd[]=($y?$y:null);$Sb[]=$Rb;}}if($f){$_c=$v[$C];if($_c){ksort($_c["columns"]);ksort($_c["lengths"]);ksort($_c["descs"]);if($u["type"]==$_c["type"]&&array_values($_c["columns"])===$f&&(!$_c["lengths"]||array_values($_c["lengths"])===$Zd)&&array_values($_c["descs"])===$Sb){unset($v[$C]);continue;}}$c[]=array($u["type"],$C,$O);}}}foreach($v
as$C=>$_c)$c[]=array($_c["type"],$C,"DROP");if(!$c)redirect(ME."table=".urlencode($a));queries_redirect(ME."table=".urlencode($a),lang(167),alter_indexes($a,$c));}page_header(lang(121),$n,array("table"=>$a),h($a));$p=array_keys(fields($a));if($_POST["add"]){foreach($K["indexes"]as$x=>$u){if($u["columns"][count($u["columns"])]!="")$K["indexes"][$x]["columns"][]="";}$u=end($K["indexes"]);if($u["type"]||array_filter($u["columns"],'strlen'))$K["indexes"][]=array("columns"=>array(1=>""));}if(!$K){foreach($v
...
';if($Ff){echo"<tr><td>PRIMARY<td>";foreach($Ff["columns"]as$x=>$e){echo
select_input(" disabled",$p,$e),"<label><input disabled type='checkbox'>".lang(50)."</label> ";}echo"<td><td>\n";}$Id=1;foreach($K["indexes"]as$u){if(!$_POST["drop_col"]||$Id!=key($_POST["drop_col"])){echo"<tr><td>".html_select("indexes[$Id][type]",array(-1=>"")+$sd,$u["type"],($Id==count($K["indexes"])?"indexesAddRow(this);":1)),"<td>";ksort($u["columns"]);$s=1;foreach($u["columns"]as$x=>$e){echo"<span>".select_input(" name='indexes[$Id][columns][$s]' onchange=\"".($s==count($u["columns"])?"indexesAddColumn":"indexesChangeColumn")."(this, '".h(js_escape($w=="sql"?"":$_GET["indexes"]."_"))."');\"",($p?array_combine($p,$p):$p),$e),($w=="sql"||$w=="mssql"?"<input type='number' name='indexes[$Id][lengths][$s]' class='size' value='".h($u["lengths"][$x])."'>":""),($w!="sql"?checkbox("indexes[$Id][descs][$s]",1,$u["descs"][$x],lang(50)):"")," </span>";$s++;}echo"<td><input name='indexes[$Id][name]' value='".h($u["name"])."' autocapitalize='off'>\n","<td><input type='image' class='icon' name='drop_col[$Id]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=4.2.3' alt='x' title='".lang(100)."' onclick=\"return !editingRemoveRow(this, 'indexes\$1[type]');\">\n";}$Id++;}echo'</table>
<p>
<input type="submit" value="',lang(14),'">
<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["database"])){$K=$_POST;if($_POST&&!$n&&!isset($_POST["add_x"])){$C=trim($K["name"]);if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),lang(171),drop_databases(array(DB)));}elseif(DB!==$C){if(DB!=""){$_GET["db"]=$C;queries_redirect(preg_replace('~\bdb=[^&]*&~','',ME)."db=".urlencode($C),lang(172),rename_database($C,$K["collation"]));}else{$l=explode("\n",str_replace("\r","",$C));$Qg=true;$Td="";foreach($l
as$m){if(count($l)==1||$m!=""){if(!create_database($m,$K["collation"]))$Qg=false;$Td=$m;}}restart_session();set_session("dbs",null);queries_redirect(ME."db=".urlencode($Td),lang(173),$Qg);}}else{if(!$K["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($C).(preg_match('~^[a-z0-9_]+$~i',$K["collation"])?" COLLATE $K[collation]":""),substr(ME,0,-1),lang(174));}}page_header(DB!=""?lang(59):lang(175),$n,array(),h(DB));$qb=collations();$C=DB;if($_POST)$C=$K["name"];elseif(DB!="")$K["collation"]=db_collation(DB,$qb);elseif($w=="sql"){foreach(get_vals("SHOW GRANTS")as$Yc){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$Yc,$B)&&$B[1]){$C=stripcslashes(idf_unescape("`$B[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($C,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($C).'</textarea><br>':'<input name="name" id="name" value="'.h($C).'" maxlength="64" autocapitalize="off">')."\n".($qb?html_select("collation",array(""=>"(".lang(87).")")+$qb,$K["collation"]).doc_link(array('sql'=>"charset-charsets.html",'mssql'=>"ms187963.aspx",)):"");?>
...
';}elseif(isset($_GET["call"])){$da=$_GET["call"];page_header(lang(179).": ".h($da),$n);$lg=routine($da,(isset($_GET["callf"])?"FUNCTION":"PROCEDURE"));$qd=array();$jf=array();foreach($lg["fields"]as$s=>$o){if(substr($o["inout"],-3)=="OUT")$jf[$s]="@".idf_escape($o["field"])." AS ".idf_escape($o["field"]);if(!$o["inout"]||substr($o["inout"],0,2)=="IN")$qd[]=$s;}if(!$n&&$_POST){$bb=array();foreach($lg["fields"]as$x=>$o){if(in_array($x,$qd)){$X=process_input($o);if($X===false)$X="''";if(isset($jf[$x]))$h->query("SET @".idf_escape($o["field"])." = $X");}$bb[]=(isset($jf[$x])?"@".idf_escape($o["field"]):$X);}$H=(isset($_GET["callf"])?"SELECT":"CALL")." ".idf_escape($da)."(".implode(", ",$bb).")";echo"<p><code class='jush-$w'>".h($H)."</code> <a href='".h(ME)."sql=".urlencode($H)."'>".lang(10)."</a>\n";if(!$h->multi_query($H))echo"<p class='error'>".error()."\n";else{$i=connect();if(is_object($i))$i->select_db(DB);do{$I=$h->store_result();if(is_object($I))select($I,$i);else
echo"<p class='message'>".lang(180,$h->affected_rows)."\n";}while($h->next_result());if($jf)select($h->query("SELECT ".implode(", ",$jf)));}}echo'
<form action="" method="post">
';if($qd){echo"<table cellspacing='0'>\n";foreach($qd
as$x){$o=$lg["fields"][$x];$C=$o["field"];echo"<tr><th>".$b->fieldName($o);$Y=$_POST["fields"][$C];if($Y!=""){if($o["type"]=="enum")$Y=+$Y;if($o["type"]=="set")$Y=array_sum($Y);}input($o,$Y,(string)$_POST["function"][$C]);echo"\n";}echo"</table>\n";}echo'<p>
<input type="submit" value="',lang(179),'">
<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["foreign"])){$a=$_GET["foreign"];$C=$_GET["name"];$K=$_POST;if($_POST&&!$n&&!$_POST["add"]&&!$_POST["change"]&&!$_POST["change-js"]){$pe=($_POST["drop"]?lang(181):($C!=""?lang(182):lang(183)));$A=ME."table=".urlencode($a);$K["source"]=array_filter($K["source"],'strlen');ksort($K["source"]);$eh=array();foreach($K["source"]as$x=>$X)$eh[$x]=$K["target"][$x];$K["target"]=$eh;if($w=="sqlite")queries_redirect($A,$pe,recreate_table($a,$a,array(),array(),array(" $C"=>($_POST["drop"]?"":" ".format_foreign_key($K)))));else{$c="ALTER TABLE ".table($a);$bc="\nDROP ".($w=="sql"?"FOREIGN KEY ":"CONSTRAINT ").idf_escape($C);if($_POST["drop"])query_redirect($c.$bc,$A,$pe);else{query_redirect($c.($C!=""?"$bc,":"")."\nADD".format_foreign_key($K),$A,$pe);$n=lang(184)."<br>$n";}}}page_header(lang(185),$n,array("table"=>$a),h($a));if($_POST){ksort($K["source"]);if($_POST["add"])$K["source"][]="";elseif($_POST["change"]||$_POST["change-js"])$K["target"]=array();}elseif($C!=""){$Rc=foreign_keys($a);$K=$Rc[$C];$K["source"][]="";}else{$K["table"]=$a;$K["source"]=array("");}$Gg=array_keys(fields($a));$eh=($a===$K["table"]?$Gg:array_keys(fields($K["table"])));$Xf=array_keys(array_filter(table_status('',true),'fk_support'));echo'
<form action="" method="post">
<p>
';if($K["db"]==""&&$K["ns"]==""){echo
lang(186),':
',html_select("table",$Xf,$K["table"],"this.form['change-js'].value = '1'; this.form.submit();"),'<input type="hidden" name="change-js" value="">
<noscript><p><input type="submit" name="change" value="',lang(187),'"></noscript>
<table cellspacing="0">
<thead><tr><th>',lang(123),'<th>',lang(124),'</thead>
';$Id=0;foreach($K["source"]as$x=>$X){echo"<tr>","<td>".html_select("source[".(+$x)."]",array(-1=>"")+$Gg,$X,($Id==count($K["source"])-1?"foreignAddRow(this);":1)),"<td>".html_select("target[".(+$x)."]",$eh,$K["target"][$x]);$Id++;}echo'</table>
<p>
',lang(89),': ',html_select("on_delete",array(-1=>"")+explode("|",$Qe),$K["on_delete"]),' ',lang(88),': ',html_select("on_update",array(-1=>"")+explode("|",$Qe),$K["on_update"]),doc_link(array('sql'=>"innodb-foreign-key-constraints.html",'pgsql'=>"sql-createtable.html#SQL-CREATETABLE-REFERENCES",'mssql'=>"ms174979.aspx",'oracle'=>"clauses002.htm#sthref2903",)),'<p>
<input type="submit" value="',lang(14),'">
...
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$K=$_POST;if($_POST&&!$n){$C=trim($K["name"]);$Ja=" AS\n$K[select]";$A=ME."table=".urlencode($C);$pe=lang(189);if($_GET["materialized"])$U="MATERIALIZED VIEW";else{$U="VIEW";if($w=="pgsql"){$Lg=table_status($C);$U=($Lg?strtoupper($Lg["Engine"]):$U);}}if(!$_POST["drop"]&&$a==$C&&$w!="sqlite"&&$U!="MATERIALIZED VIEW")query_redirect(($w=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($C).$Ja,$A,$pe);else{$gh=$C."_adminer_".uniqid();drop_create("DROP $U ".table($a),"CREATE $U ".table($C).$Ja,"DROP $U ".table($C),"CREATE $U ".table($gh).$Ja,"DROP $U ".table($gh),($_POST["drop"]?substr(ME,0,-1):$A),lang(190),$pe,lang(191),$a,$C);}}if(!$_POST&&$a!=""){$K=view($a);$K["name"]=$a;if(!$n)$n=error();}page_header(($a!=""?lang(40):lang(192)),$n,array("table"=>$a),h($a));echo'
<form action="" method="post">
...
<tr><th>',lang(200),'<td><input type="number" name="INTERVAL_VALUE" value="',h($K["INTERVAL_VALUE"]),'" class="size"> ',html_select("INTERVAL_FIELD",$_d,$K["INTERVAL_FIELD"]),'<tr><th>',lang(107),'<td>',html_select("STATUS",$Mg,$K["STATUS"]),'<tr><th>',lang(96),'<td><input name="EVENT_COMMENT" value="',h($K["EVENT_COMMENT"]),'" maxlength="64">
...
';}elseif(isset($_GET["procedure"])){$da=$_GET["procedure"];$lg=(isset($_GET["function"])?"FUNCTION":"PROCEDURE");$K=$_POST;$K["fields"]=(array)$K["fields"];if($_POST&&!process_fields($K["fields"])&&!$n){$gh="$K[name]_adminer_".uniqid();drop_create("DROP $lg ".idf_escape($da),create_routine($lg,$K),"DROP $lg ".idf_escape($K["name"]),create_routine($lg,array("name"=>$gh)+$K),"DROP $lg ".idf_escape($gh),substr(ME,0,-1),lang(202),lang(203),lang(204),$da,$K["name"]);}page_header(($da!=""?(isset($_GET["function"])?lang(205):lang(206)).": ".h($da):(isset($_GET["function"])?lang(207):lang(208))),$n);if(!$_POST&&$da!=""){$K=routine($da,$lg);$K["name"]=$da;}$qb=get_vals("SHOW CHARACTER SET");sort($qb);$mg=routine_languages();echo'
<form action="" method="post" id="form">
<p>',lang(170),': <input name="name" value="',h($K["name"]),'" maxlength="64" autocapitalize="off">
',($mg?lang(19).": ".html_select("language",$mg,$K["language"]):""),'<input type="submit" value="',lang(14),'">
<table cellspacing="0" class="nowrap">
';edit_fields($K["fields"],$qb,$lg);if(isset($_GET["function"])){echo"<tr><td>".lang(209);edit_type("returns",$K["returns"],$qb);}echo'</table>
...
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$C=$_GET["name"];$Ah=trigger_options();$K=(array)trigger($C)+array("Trigger"=>$a."_bi");if($_POST){if(!$n&&in_array($_POST["Timing"],$Ah["Timing"])&&in_array($_POST["Event"],$Ah["Event"])&&in_array($_POST["Type"],$Ah["Type"])){$Pe=" ON ".table($a);$bc="DROP TRIGGER ".idf_escape($C).($w=="pgsql"?$Pe:"");$A=ME."table=".urlencode($a);if($_POST["drop"])query_redirect($bc,$A,lang(219));else{if($C!="")queries($bc);queries_redirect($A,($C!=""?lang(220):lang(221)),queries(create_trigger($Pe,$_POST)));if($C!="")queries(create_trigger($Pe,$K+array("Type"=>reset($Ah["Type"]))));}}$K=$_POST;}page_header(($C!=""?lang(222).": ".h($C):lang(223)),$n,array("table"=>$a));echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>',lang(224),'<td>',html_select("Timing",$Ah["Timing"],$K["Timing"],"triggerChange(/^".preg_quote($a,"/")."_[ba][iud]$/, '".js_escape($a)."', this.form);"),'<tr><th>',lang(225),'<td>',html_select("Event",$Ah["Event"],$K["Event"],"this.form['Timing'].onchange();"),(in_array("UPDATE OF",$Ah["Event"])?" <input name='Of' value='".h($K["Of"])."' class='hidden'>":""),'<tr><th>',lang(92),'<td>',html_select("Type",$Ah["Type"],$K["Type"]),'</table>
...
';}elseif(isset($_GET["user"])){$ha=$_GET["user"];$Kf=array(""=>array("All privileges"=>""));foreach(get_rows("SHOW PRIVILEGES")as$K){foreach(explode(",",($K["Privilege"]=="Grant option"?"":$K["Context"]))as$Ab)$Kf[$Ab][$K["Privilege"]]=$K["Comment"];}$Kf["Server Admin"]+=$Kf["File access on server"];$Kf["Databases"]["Create routine"]=$Kf["Procedures"]["Create routine"];unset($Kf["Procedures"]["Create routine"]);$Kf["Columns"]=array();foreach(array("Select","Insert","Update","References")as$X)$Kf["Columns"][$X]=$Kf["Tables"][$X];unset($Kf["Server Admin"]["Usage"]);foreach($Kf["Tables"]as$x=>$X)unset($Kf["Databases"][$x]);$Be=array();if($_POST){foreach($_POST["objects"]as$x=>$X)$Be[$X]=(array)$Be[$X]+(array)$_POST["grants"][$x];}$Zc=array();$Ne="";if(isset($_GET["host"])&&($I=$h->query("SHOW GRANTS FOR ".q($ha)."@".q($_GET["host"])))){while($K=$I->fetch_row()){if(preg_match('~GRANT (.*) ON (.*) TO ~',$K[0],$B)&&preg_match_all('~ *([^(,]*[^ ,(])( *\\([^)]+\\))?~',$B[1],$he,PREG_SET_ORDER)){foreach($he
as$X){if($X[1]!="USAGE")$Zc["$B[2]$X[2]"][$X[1]]=true;if(preg_match('~ WITH GRANT OPTION~',$K[0]))$Zc["$B[2]$X[2]"]["GRANT OPTION"]=true;}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$K[0],$B))$Ne=$B[1];}}if($_POST&&!$n){$Oe=(isset($_GET["host"])?q($ha)."@".q($_GET["host"]):"''");if($_POST["drop"])query_redirect("DROP USER $Oe",ME."privileges=",lang(226));else{$De=q($_POST["user"])."@".q($_POST["host"]);$tf=$_POST["pass"];if($tf!=''&&!$_POST["hashed"]){$tf=$h->result("SELECT PASSWORD(".q($tf).")");$n=!$tf;}$Fb=false;if(!$n){if($Oe!=$De){$Fb=queries(($h->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $De IDENTIFIED BY PASSWORD ".q($tf));$n=!$Fb;}elseif($tf!=$Ne)queries("SET PASSWORD FOR $De = ".q($tf));}if(!$n){$ig=array();foreach($Be
as$Ie=>$Yc){if(isset($_GET["grant"]))$Yc=array_filter($Yc);$Yc=array_keys($Yc);if(isset($_GET["grant"]))$ig=array_diff(array_keys(array_filter($Be[$Ie],'strlen')),$Yc);elseif($Oe==$De){$Le=array_keys((array)$Zc[$Ie]);$ig=array_diff($Le,$Yc);$Yc=array_diff($Yc,$Le);unset($Zc[$Ie]);}if(preg_match('~^(.+)\\s*(\\(.*\\))?$~U',$Ie,$B)&&(!grant("REVOKE",$ig,$B[2]," ON $B[1] FROM $De")||!grant("GRANT",$Yc,$B[2]," ON $B[1] TO $De"))){$n=true;break;}}}if(!$n&&isset($_GET["host"])){if($Oe!=$De)queries("DROP USER $Oe");elseif(!isset($_GET["grant"])){foreach($Zc
as$Ie=>$ig){if(preg_match('~^(.+)(\\(.*\\))?$~U',$Ie,$B))grant("REVOKE",array_keys($ig),$B[2]," ON $B[1] FROM $De");}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?lang(227):lang(228)),!$n);if($Fb)$h->query("DROP USER $De");}}page_header((isset($_GET["host"])?lang(33).": ".h("$ha@$_GET[host]"):lang(135)),$n,array("privileges"=>array('',lang(63))));if($_POST){$K=$_POST;$Zc=$Be;}else{$K=$_GET+array("host"=>$h->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$K["pass"]=$Ne;if($Ne!="")$K["hashed"]=true;$Zc[(DB==""||$Zc?"":idf_escape(addcslashes(DB,"%_\\"))).".*"]=array();}echo'<form action="" method="post">
<table cellspacing="0">
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'>".lang(63).doc_link(array('sql'=>"grant.html#priv_level"));$s=0;foreach($Zc
as$Ie=>$Yc){echo'<th>'.($Ie!="*.*"?"<input name='objects[$s]' value='".h($Ie)."' size='10' autocapitalize='off'>":"<input type='hidden' name='objects[$s]' value='*.*' size='10'>*.*");$s++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>lang(32),"Databases"=>lang(35),"Tables"=>lang(119),"Columns"=>lang(120),"Procedures"=>lang(230),)as$Ab=>$Rb){foreach((array)$Kf[$Ab]as$Jf=>$ub){echo"<tr".odd()."><td".($Rb?">$Rb<td":" colspan='2'").' lang="en" title="'.h($ub).'">'.h($Jf);$s=0;foreach($Zc
as$Ie=>$Yc){$C="'grants[$s][".h(strtoupper($Jf))."]'";$Y=$Yc[strtoupper($Jf)];if($Ab=="Server Admin"&&$Ie!=(isset($Zc["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$C><option><option value='1'".($Y?" selected":"").">".lang(231)."<option value='0'".($Y=="0"?" selected":"").">".lang(232)."</select>";else
echo"<td align='center'><label class='block'><input type='checkbox' name=$C value='1'".($Y?" checked":"").($Jf=="All privileges"?" id='grants-$s-all'":($Jf=="Grant option"?"":" onclick=\"if (this.checked) formUncheck('grants-$s-all');\""))."></label>";$s++;}}}echo"</table>\n",'<p>
<input type="submit" value="',lang(14),'">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="',lang(116),'"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$n){$Pd=0;foreach((array)$_POST["kill"]as$X){if(queries("KILL ".number($X)))$Pd++;}queries_redirect(ME."processlist=",lang(233,$Pd),$Pd||!$_POST["kill"]);}page_header(lang(105),$n);echo'
<form action="" method="post">
...
';}elseif(isset($_GET["select"])){$a=$_GET["select"];$R=table_status1($a);$v=indexes($a);$p=fields($a);$Rc=column_foreign_keys($a);$Ke="";if($R["Oid"]){$Ke=($w=="sqlite"?"rowid":"oid");$v[]=array("type"=>"PRIMARY","columns"=>array($Ke));}parse_str($_COOKIE["adminer_import"],$Aa);$jg=array();$f=array();$kh=null;foreach($p
as$x=>$o){$C=$b->fieldName($o);if(isset($o["privileges"]["select"])&&$C!=""){$f[$x]=html_entity_decode(strip_tags($C),ENT_QUOTES);if(is_shortable($o))$kh=$b->selectLengthProcess();}$jg+=$o["privileges"];}list($M,$ad)=$b->selectColumnsProcess($f,$v);$Dd=count($ad)<count($M);$Z=$b->selectSearchProcess($p,$v);$af=$b->selectOrderProcess($p,$v);$z=$b->selectLimitProcess();$Wc=($M?implode(", ",$M):"*".($Ke?", $Ke":"")).convert_fields($f,$p,$M)."\nFROM ".table($a);$bd=($ad&&$Dd?"\nGROUP BY ".implode(", ",$ad):"").($af?"\nORDER BY ".implode(", ",$af):"");if($_GET["val"]&&is_ajax()){header("Content-Type: text/plain; charset=utf-8");foreach($_GET["val"]as$Ih=>$K){$Ja=convert_field($p[key($K)]);$M=array($Ja?$Ja:idf_escape(key($K)));$Z[]=where_check($Ih,$p);$J=$Zb->select($a,$M,$Z,$M);if($J)echo
reset($J->fetch_row());}exit;}if($_POST&&!$n){$fi=$Z;if(!$_POST["all"]&&is_array($_POST["check"])){$hb=array();foreach($_POST["check"]as$eb)$hb[]=where_check($eb,$p);$fi[]="((".implode(") OR (",$hb)."))";}$fi=($fi?"\nWHERE ".implode(" AND ",$fi):"");$Ff=$Kh=null;foreach($v
as$u){if($u["type"]=="PRIMARY"){$Ff=array_flip($u["columns"]);$Kh=($M?$Ff:array());break;}}foreach((array)$Kh
as$x=>$X){if(in_array(idf_escape($x),$M))unset($Kh[$x]);}if($_POST["export"]){cookie("adminer_import","output=".urlencode($_POST["output"])."&format=".urlencode($_POST["format"]));dump_headers($a);$b->dumpTable($a,"");if(!is_array($_POST["check"])||$Kh===array())$H="SELECT $Wc$fi$bd";else{$Gh=array();foreach($_POST["check"]as$X)$Gh[]="(SELECT".limit($Wc,"\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$p).$bd,1).")";$H=implode(" UNION ALL ",$Gh);}$b->dumpData($a,"table",$H);exit;}if(!$b->selectEmailProcess($Z,$Rc)){if($_POST["save"]||$_POST["delete"]){$I=true;$Ba=0;$O=array();if(!$_POST["delete"]){foreach($f
as$C=>$X){$X=process_input($p[$C]);if($X!==null&&($_POST["clone"]||$X!==false))$O[idf_escape($C)]=($X!==false?$X:idf_escape($C));}}if($_POST["delete"]||$O){if($_POST["clone"])$H="INTO ".table($a)." (".implode(", ",array_keys($O)).")\nSELECT ".implode(", ",$O)."\nFROM ".table($a);if($_POST["all"]||($Kh===array()&&is_array($_POST["check"]))||$Dd){$I=($_POST["delete"]?$Zb->delete($a,$fi):($_POST["clone"]?queries("INSERT $H$fi"):$Zb->update($a,$O,$fi)));$Ba=$h->affected_rows;}else{foreach((array)$_POST["check"]as$X){$ei="\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$p);$I=($_POST["delete"]?$Zb->delete($a,$ei,1):($_POST["clone"]?queries("INSERT".limit1($H,$ei)):$Zb->update($a,$O,$ei)));if(!$I)break;$Ba+=$h->affected_rows;}}}$pe=lang(237,$Ba);if($_POST["clone"]&&$I&&$Ba==1){$Ud=last_id();if($Ud)$pe=lang(156," $Ud");}queries_redirect(remove_from_uri($_POST["all"]&&$_POST["delete"]?"page":""),$pe,$I);if(!$_POST["delete"]){edit_form($a,$p,(array)$_POST["fields"],!$_POST["clone"]);page_footer();exit;}}elseif(!$_POST["import"]){if(!$_POST["val"])$n=lang(238);else{$I=true;$Ba=0;foreach($_POST["val"]as$Ih=>$K){$O=array();foreach($K
as$x=>$X){$x=bracket_escape($x,1);$O[idf_escape($x)]=(preg_match('~char|text~',$p[$x]["type"])||$X!=""?$b->processInput($p[$x],$X):"NULL");}$I=$Zb->update($a,$O," WHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($Ih,$p),!($Dd||$Kh===array())," ");if(!$I)break;$Ba+=$h->affected_rows;}queries_redirect(remove_from_uri(),lang(237,$Ba),$I);}}elseif(!is_string($Kc=get_file("csv_file",true)))$n=upload_error($Kc);elseif(!preg_match('~~u',$Kc))$n=lang(239);else{cookie("adminer_import","output=".urlencode($Aa["output"])."&format=".urlencode($_POST["separator"]));$I=true;$rb=array_keys($p);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$Kc,$he);$Ba=count($he[0]);$Zb->begin();$yg=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));$L=array();foreach($he[0]as$x=>$X){preg_match_all("~((?>\"[^\"]*\")+|[^$yg]*)$yg~",$X.$yg,$ie);if(!$x&&!array_diff($ie[1],$rb)){$rb=$ie[1];$Ba--;}else{$O=array();foreach($ie[1]as$s=>$ob)$O[idf_escape($rb[$s])]=($ob==""&&$p[$rb[$s]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$ob))));$L[]=$O;}}$I=(!$L||$Zb->insertUpdate($a,$L,$Ff));if($I)$Zb->commit();queries_redirect(remove_from_uri("page"),lang(240,$Ba),$I);$Zb->rollback();}}}$Wg=$b->tableName($R);if(is_ajax()){page_headers();ob_start();}else
page_header(lang(44).": $Wg",$n);$O=null;if(isset($jg["insert"])||!support("table")){$O="";foreach((array)$_GET["where"]as$X){if(count($Rc[$X["col"]])==1&&($X["op"]=="="||(!$X["op"]&&!preg_match('~[_%]~',$X["val"]))))$O.="&set".urlencode("[".bracket_escape($X["col"])."]")."=".urlencode($X["val"]);}}$b->selectLinks($R,$O);if(!$f&&support("table"))echo"<p class='error'>".lang(241).($p?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($M,$f);$b->selectSearchPrint($Z,$f,$v);$b->selectOrderPrint($af,$f,$v);$b->selectLimitPrint($z);$b->selectLengthPrint($kh);$b->selectActionPrint($v);echo"</form>\n";$E=$_GET["page"];if($E=="last"){$Uc=$h->result(count_rows($a,$Z,$Dd,$ad));$E=floor(max(0,$Uc-1)/$z);}$vg=$M;if(!$vg){$vg[]="*";if($Ke)$vg[]=$Ke;}$Bb=convert_fields($f,$p,$M);if($Bb)$vg[]=substr($Bb,2);$I=$Zb->select($a,$vg,$Z,$ad,$af,$z,$E,true);if(!$I)echo"<p class='error'>".error()."\n";else{if($w=="mssql"&&$E)$I->seek($z*$E);$oc=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$L=array();while($K=$I->fetch_assoc()){if($E&&$w=="oracle")unset($K["RNUM"]);$L[]=$K;}if($_GET["page"]!="last"&&+$z&&$ad&&$Dd&&$w=="sql")$Uc=$h->result(" SELECT FOUND_ROWS()");if(!$L)echo"<p class='message'>".lang(12)."\n";else{$Sa=$b->backwardKeys($a,$Wg);echo"<table id='table' cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$ad&&$M?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".lang(242)."</a>");$Ae=array();$Xc=array();reset($M);$Tf=1;foreach($L[0]as$x=>$X){if($x!=$Ke){$X=$_GET["columns"][key($M)];$o=$p[$M?($X?$X["col"]:current($M)):$x];$C=($o?$b->fieldName($o,$Tf):($X["fun"]?"*":$x));if($C!=""){$Tf++;$Ae[$x]=$C;$e=idf_escape($x);$ld=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($x);$Rb="&desc%5B0%5D=1";echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($ld.($af[0]==$e||$af[0]==$x||(!$af&&$Dd&&$ad[0]==$e)?$Rb:'')).'">';echo
apply_sql_function($X["fun"],$C)."</a>";echo"<span class='column hidden'>","<a href='".h($ld.$Rb)."' title='".lang(50)."' class='text'> â†“</a>";if(!$X["fun"])echo'<a href="#fieldset-search" onclick="selectSearch(\''.h(js_escape($x)).'\'); return false;" title="'.lang(47).'" class="text jsonly"> =</a>';echo"</span>";}$Xc[$x]=$X["fun"];next($M);}}$Zd=array();if($_GET["modify"]){foreach($L
as$K){foreach($K
as$x=>$X)$Zd[$x]=max($Zd[$x],min(40,strlen(utf8_decode($X))));}}echo($Sa?"<th>".lang(243):"")."</thead>\n";if(is_ajax()){if($z%2==1&&$E%2==1)odd();ob_end_clean();}foreach($b->rowDescriptions($L,$Rc)as$_e=>$K){$Hh=unique_array($L[$_e],$v);if(!$Hh){$Hh=array();foreach($L[$_e]as$x=>$X){if(!preg_match('~^(COUNT\\((\\*|(DISTINCT )?`(?:[^`]|``)+`)\\)|(AVG|GROUP_CONCAT|MAX|MIN|SUM)\\(`(?:[^`]|``)+`\\))$~',$x))$Hh[$x]=$X;}}$Ih="";foreach($Hh
as$x=>$X){if(($w=="sql"||$w=="pgsql")&&strlen($X)>64){$x=(strpos($x,'(')?$x:idf_escape($x));$x="MD5(".($w=='sql'&&preg_match("~^utf8_~",$p[$x]["collation"])?$x:"CONVERT($x USING ".charset($h).")").")";$X=md5($X);}$Ih.="&".($X!==null?urlencode("where[".bracket_escape($x)."]")."=".urlencode($X):"null%5B%5D=".urlencode($x));}echo"<tr".odd().">".(!$ad&&$M?"":"<td>".checkbox("check[]",substr($Ih,1),in_array(substr($Ih,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").($Dd||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$Ih)."'>".lang(244)."</a>"));foreach($K
as$x=>$X){if(isset($Ae[$x])){$o=$p[$x];if($X!=""&&(!isset($oc[$x])||$oc[$x]!=""))$oc[$x]=(is_mail($X)?$Ae[$x]:"");$_="";if(preg_match('~blob|bytea|raw|file~',$o["type"])&&$X!="")$_=ME.'download='.urlencode($a).'&field='.urlencode($x).$Ih;if(!$_&&$X!==null){foreach((array)$Rc[$x]as$q){if(count($Rc[$x])==1||end($q["source"])==$x){$_="";foreach($q["source"]as$s=>$Gg)$_.=where_link($s,$q["target"][$s],$L[$_e][$Gg]);$_=($q["db"]!=""?preg_replace('~([?&]db=)[^&]+~','\\1'.urlencode($q["db"]),ME):ME).'select='.urlencode($q["table"]).$_;if(count($q["source"])==1)break;}}}if($x=="COUNT(*)"){$_=ME."select=".urlencode($a);$s=0;foreach((array)$_GET["where"]as$W){if(!array_key_exists($W["col"],$Hh))$_.=where_link($s++,$W["col"],$W["val"],$W["op"]);}foreach($Hh
as$Jd=>$W)$_.=where_link($s++,$Jd,$W);}$X=select_value($X,$_,$o,$kh);$md=h("val[$Ih][".bracket_escape($x)."]");$Y=$_POST["val"][$Ih][bracket_escape($x)];$jc=!is_array($K[$x])&&is_utf8($X)&&$L[$_e][$x]==$K[$x]&&!$Xc[$x];$jh=preg_match('~text|lob~',$o["type"]);if(($_GET["modify"]&&$jc)||$Y!==null){$dd=h($Y!==null?$Y:$K[$x]);echo"<td>".($jh?"<textarea name='$md' cols='30' rows='".(substr_count($K[$x],"\n")+1)."'>$dd</textarea>":"<input name='$md' value='$dd' size='$Zd[$x]'>");}else{$ee=strpos($X,"<i>...</i>");echo"<td id='$md' onclick=\"selectClick(this, event, ".($ee?2:($jh?1:0)).($jc?"":", '".h(lang(245))."'").");\">$X";}}}if($Sa)echo"<td>";$b->backwardKeysPrint($Sa,$L[$_e]);echo"</tr>\n";}if(is_ajax())exit;echo"</table>\n";}if(($L||$E)&&!is_ajax()){$yc=true;if($_GET["page"]!="last"){if(!+$z)$Uc=count($L);elseif($w!="sql"||!$Dd){$Uc=($Dd?false:found_rows($R,$Z));if($Uc<max(1e4,2*($E+1)*$z))$Uc=reset(slow_query(count_rows($a,$Z,$Dd,$ad)));else$yc=false;}}if(+$z&&($Uc===false||$Uc>$z||$E)){echo"<p class='pages'>";$ke=($Uc===false?$E+(count($L)>=$z?2:1):floor(($Uc-1)/$z));if($w!="simpledb"){echo'<a href="'.h(remove_from_uri("page"))."\" onclick=\"pageClick(this.href, +prompt('".lang(246)."', '".($E+1)."'), event); return false;\">".lang(246)."</a>:",pagination(0,$E).($E>5?" ...":"");for($s=max(1,$E-4);$s<min($ke,$E+5);$s++)echo
pagination($s,$E);if($ke>0){echo($E+5<$ke?" ...":""),($yc&&$Uc!==false?pagination($ke,$E):" <a href='".h(remove_from_uri("page")."&page=last")."' title='~$ke'>".lang(247)."</a>");}echo(($Uc===false?count($L)+1:$Uc-$E*$z)>$z?' <a href="'.h(remove_from_uri("page")."&page=".($E+1)).'" onclick="return !selectLoadMore(this, '.(+$z).', \''.lang(248).'...\');" class="loadmore">'.lang(249).'</a>':'');}else{echo
lang(246).":",pagination(0,$E).($E>1?" ...":""),($E?pagination($E,$E):""),($ke>$E?pagination($E+1,$E).($ke>$E+1?" ...":""):"");}}echo"<p class='count'>\n",($Uc!==false?"(".($yc?"":"~ ").lang(138,$Uc).") ":"");$Wb=($yc?"":"~ ").$Uc;echo
checkbox("all",1,0,lang(250),"var checked = formChecked(this, /check/); selectCount('selected', this.checked ? '$Wb' : checked); selectCount('selected2', this.checked || !checked ? '$Wb' : checked);")."\n";if($b->selectCommandPrint()){echo'<fieldset',($_GET["modify"]?'':' class="jsonly"'),'><legend>',lang(242),'</legend><div>
...
';}$Sc=$b->dumpFormat();foreach((array)$_GET["columns"]as$e){if($e["fun"]){unset($Sc['sql']);break;}}if($Sc){print_fieldset("export",lang(65)." <span id='selected2'></span>");$kf=$b->dumpOutput();echo($kf?html_select("output",$kf,$Aa["output"])." ":""),html_select("format",$Sc,$Aa["format"])," <input type='submit' name='export' value='".lang(65)."'>\n","</div></fieldset>\n";}echo(!$ad&&$M?"":"<script type='text/javascript'>tableCheck();</script>\n");}if($b->selectImportPrint()){print_fieldset("import",lang(64),!$L);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$Aa["format"],1);echo" <input type='submit' name='import' value='".lang(64)."'>","</div></fieldset>\n";}$b->selectEmailPrint(array_filter($oc,'strlen'),$f);echo"<p><input type='hidden' name='token' value='$T'></p>\n","</form>\n";}}if(is_ajax()){ob_end_clean();exit;}}elseif(isset($_GET["variables"])){$Lg=isset($_GET["status"]);page_header($Lg?lang(107):lang(106));$Xh=($Lg?show_status():show_variables());if(!$Xh)echo"<p class='message'>".lang(12)."\n";else{echo"<table cellspacing='0'>\n";foreach($Xh
as$x=>$X){echo"<tr>","<th><code class='jush-".$w.($Lg?"status":"set")."'>".h($x)."</code>","<td>".nbsp($X);}echo"</table>\n";}}elseif(isset($_GET["script"])){header("Content-Type: text/javascript; charset=utf-8");if($_GET["script"]=="db"){$Tg=array("Data_length"=>0,"Index_length"=>0,"Data_free"=>0);foreach(table_status()as$C=>$R){json_row("Comment-$C",nbsp($R["Comment"]));if(!is_view($R)){foreach(array("Engine","Collation")as$x)json_row("$x-$C",nbsp($R[$x]));foreach($Tg+array("Auto_increment"=>0,"Rows"=>0)as$x=>$X){if($R[$x]!=""){$X=format_number($R[$x]);json_row("$x-$C",($x=="Rows"&&$X&&$R["Engine"]==($Ig=="pgsql"?"table":"InnoDB")?"~ $X":$X));if(isset($Tg[$x]))$Tg[$x]+=($R["Engine"]!="InnoDB"||$x!="Data_free"?$R[$x]:0);}elseif(array_key_exists($x,$R))json_row("$x-$C");}}}foreach($Tg
as$x=>$X)json_row("sum-$x",format_number($X));json_row("");}elseif($_GET["script"]=="kill")$h->query("KILL ".number($_POST["kill"]));else{foreach(count_tables($b->databases())as$m=>$X){json_row("tables-$m",$X);json_row("size-$m",db_size($m));}json_row("");}exit;}else{$ch=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($ch&&!$n&&!$_POST["search"]){$I=true;$pe="";if($w=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"]))queries("SET foreign_key_checks = 0");if($_POST["truncate"]){if($_POST["tables"])$I=truncate_tables($_POST["tables"]);$pe=lang(251);}elseif($_POST["move"]){$I=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$pe=lang(252);}elseif($_POST["copy"]){$I=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$pe=lang(253);}elseif($_POST["drop"]){if($_POST["views"])$I=drop_views($_POST["views"]);if($I&&$_POST["tables"])$I=drop_tables($_POST["tables"]);$pe=lang(254);}elseif($w!="sql"){$I=($w=="sqlite"?queries("VACUUM"):apply_queries("VACUUM".($_POST["optimize"]?"":" ANALYZE"),$_POST["tables"]));$pe=lang(255);}elseif(!$_POST["tables"])$pe=lang(9);elseif($I=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"])))){while($K=$I->fetch_assoc())$pe.="<b>".h($K["Table"])."</b>: ".h($K["Msg_text"])."<br>";}queries_redirect(substr(ME,0,-1),$pe,$I);}page_header(($_GET["ns"]==""?lang(35).": ".h(DB):lang(68).": ".h($_GET["ns"])),$n,true);if($b->homepage()){if($_GET["ns"]!==""){echo"<h3 id='tables-views'>".lang(256)."</h3>\n";$bh=tables_list();if(!$bh)echo"<p class='message'>".lang(9)."\n";else{echo"<form action='' method='post'>\n";if(support("table")){echo"<fieldset><legend>".lang(257)." <span id='selected2'></span></legend><div>","<input type='search' name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".lang(47)."'>\n","</div></fieldset>\n";if($_POST["search"]&&$_POST["query"]!="")search_tables();}echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);">';$Xb=doc_link(array('sql'=>'show-table-status.html'));echo'<th>'.lang(119),'<td>'.lang(258).doc_link(array('sql'=>'storage-engines.html')),'<td>'.lang(111).doc_link(array('sql'=>'charset-mysql.html')),'<td>'.lang(259).$Xb,'<td>'.lang(260).$Xb,'<td>'.lang(261).$Xb,'<td>'.lang(56).doc_link(array('sql'=>'example-auto-increment.html')),'<td>'.lang(262).$Xb,(support("comment")?'<td>'.lang(96).$Xb:''),"</thead>\n";$S=0;foreach($bh
as$C=>$U){$ai=($U!==null&&!preg_match('~table~i',$U));echo'<tr'.odd().'><td>'.checkbox(($ai?"views[]":"tables[]"),$C,in_array($C,$ch,true),"","formUncheck('check-all');"),'<th>'.(support("table")||support("indexes")?'<a href="'.h(ME).'table='.urlencode($C).'" title="'.lang(39).'">'.h($C).'</a>':h($C));if($ai){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($C).'" title="'.lang(40).'">'.(preg_match('~materialized~i',$U)?lang(263):lang(118)).'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($C).'" title="'.lang(38).'">?</a>';}else{foreach(array("Engine"=>array(),"Collation"=>array(),"Data_length"=>array("create",lang(41)),"Index_length"=>array("indexes",lang(122)),"Data_free"=>array("edit",lang(42)),"Auto_increment"=>array("auto_increment=1&create",lang(41)),"Rows"=>array("select",lang(38)),)as$x=>$_){$md=" id='$x-".h($C)."'";echo($_?"<td align='right'>".(support("table")||$x=="Rows"||(support("indexes")&&$x!="Data_length")?"<a href='".h(ME."$_[0]=").urlencode($C)."'$md title='$_[1]'>?</a>":"<span$md>?</span>"):"<td id='$x-".h($C)."'>&nbsp;");}$S++;}echo(support("comment")?"<td id='Comment-".h($C)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".lang(235,count($bh)),"<td>".nbsp($w=="sql"?$h->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$x)echo"<td align='right' id='sum-$x'>&nbsp;";echo"</table>\n";if(!information_schema(DB)){$Uh="<input type='submit' value='".lang(264)."'".on_help("'VACUUM'")."> ";$We="<input type='submit' name='optimize' value='".lang(265)."'".on_help($w=="sql"?"'OPTIMIZE TABLE'":"'VACUUM OPTIMIZE'")."> ";echo"<fieldset><legend>".lang(115)." <span id='selected'></span></legend><div>".($w=="sqlite"?$Uh:($w=="pgsql"?$Uh.$We:($w=="sql"?"<input type='submit' value='".lang(266)."'".on_help("'ANALYZE TABLE'")."> ".$We."<input type='submit' name='check' value='".lang(267)."'".on_help("'CHECK TABLE'")."> "."<input type='submit' name='repair' value='".lang(268)."'".on_help("'REPAIR TABLE'")."> ":"")))."<input type='submit' name='truncate' value='".lang(269)."'".confirm().on_help($w=="sqlite"?"'DELETE'":"'TRUNCATE".($w=="pgsql"?"'":" TABLE'"))."> "."<input type='submit' name='drop' value='".lang(116)."'".confirm().on_help("'DROP TABLE'").">\n";$l=(support("scheme")?$b->schemas():$b->databases());if(count($l)!=1&&$w!="sqlite"){$m=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".lang(270).": ",($l?html_select("target",$l,$m):'<input name="target" value="'.h($m).'" autocapitalize="off">')," <input type='submit' name='move' value='".lang(271)."'>",(support("copy")?" <input type='submit' name='copy' value='".lang(272)."'>":""),"\n";}echo"<input type='hidden' name='all' value='' onclick=\"selectCount('selected', formChecked(this, /^(tables|views)\[/));".(support("table")?" selectCount('selected2', formChecked(this, /^tables\[/) || $S);":"")."\">\n";echo"<input type='hidden' name='token' value='$T'>\n","</div></fieldset>\n";}echo"</form>\n","<script type='text/javascript'>tableCheck();</script>\n";}echo'<p class="links"><a href="'.h(ME).'create=">'.lang(66)."</a>\n",(support("view")?'<a href="'.h(ME).'view=">'.lang(192)."</a>\n":""),(support("materializedview")?'<a href="'.h(ME).'view=&amp;materialized=1">'.lang(273)."</a>\n":"");if(support("routine")){echo"<h3 id='routines'>".lang(132)."</h3>\n";$ng=routines();if($ng){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.lang(170).'<td>'.lang(92).'<td>'.lang(209)."<td>&nbsp;</thead>\n";odd('');foreach($ng
